...
var _a = require('./util'), reemitTrackPublicationEvents = _a.reemitTrackPublicationEvents, trackClass = _a.trackClass, trackPublicationClass = _a.trackPublicationClass, valueToJSON = _a.valueToJSON;
var nInstances = 0;
/**
...
var Participant = /** @class */ (function (_super) {
    ...
    function Participant(signaling, options) {
        ...
        var log = options.log.createLog(_this);
...
function indexTracksById(tracks) {
    var indexedTracks = tracks.map(function (track) { return [track.id || track._trackSender.id, track]; });
    ...
    signaling.on('stateChanged', function stateChanged(state) {
        ...
        if (state === 'disconnected') {
            ...
            participant._tracks.forEach(function (track) {
                ...
                var trackIdentifier = track.sid || track._trackSender.id;
                var reemitters = participant._trackEventReemitters.get(trackIdentifier);
                if (track && reemitters) {
                    reemitters.forEach(function (reemitter, event) {
                        track.removeListener(event, reemitter);
                    });
