...
/**
 * Filter remote video codecs based on the local unified plan SDP.
 * @param {string} remoteSdp
 * @param {string} localSdp
 * @param {Array<string>} codecsToRemove
 * @returns {string} - Updated Remote SDP
 */
function unifiedPlanFilterRemoteVideoCodecs(remoteSdp, localSdp, codecsToRemove) {
  var remoteMediaSections = getMediaSections(remoteSdp);
  var remoteSession = remoteSdp.split('\r\nm=')[0];
  var localMidsToMediaSections = createMidToMediaSectionMap(localSdp);
  return [remoteSession].concat(remoteMediaSections.map(function (remoteSection) {
    if (!/^m=video/.test(remoteSection)) {
      return remoteSection;
    }
    var remoteCodecMap = createCodecMapForMediaSection(remoteSection);

    var remainingRemoteCodecs = difference(Array.from(remoteCodecMap.keys()), codecsToRemove.concat('rtx'));

    // NOTE(mmalavalli): Remove the specified codecs only if at least one non-rtx
    // codec will remain in the remote m= section. This way, we ensure that the remote
    // m= section has at least one non-rtx codec available.
    return unifiedPlanFilterCodecsInMediaSection(remoteSection, localMidsToMediaSections, remainingRemoteCodecs.size > 0 ? codecsToRemove : []);
  })).join('\r\n');
}
...
exports.unifiedPlanFilterRemoteVideoCodecs = unifiedPlanFilterRemoteVideoCodecs;
