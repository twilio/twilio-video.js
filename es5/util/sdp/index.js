...
function unifiedPlanRewriteNewTrackIds(sdp, trackIdsByKind) {
  return Array.from(trackIdsByKind).reduce(function (sdp, _ref) {
    var _ref2 = _slicedToArray(_ref, 2),
        kind = _ref2[0],
        trackIds = _ref2[1];

    var sections = getMediaSections(sdp, kind);
    ...
    // of addition.
    var sdpTrackIds = sections.slice(sections.length - trackIds.length).map(function (section) {
      return (section.match(/^a=msid:.+ (.+)$/m) || [])[1];
    });

    return sdpTrackIds.reduce(function (sdp, sdpTrackId, i) {
      if (sdpTrackId) {
        var msidRegex = new RegExp('msid:(.+) ' + sdpTrackId + '$', 'gm');
