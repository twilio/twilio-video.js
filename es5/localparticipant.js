...
var _a = require('./util'), asLocalTrack = _a.asLocalTrack, asLocalTrackPublication = _a.asLocalTrackPublication, reemitTrackPublicationEvents = _a.reemitTrackPublicationEvents, trackClass = _a.trackClass;
...
var LocalParticipant = /** @class */ (function (_super) {
    ...
    LocalParticipant.prototype._addTrack = function (track, priority) {
        var addedTrack = _super.prototype._addTrack.call(this, track, track._trackSender.id);
    ...
    LocalParticipant.prototype._addLocalTrack = function (track, priority) {
        ...
        this._log.info("Added a new " + trackClass(track, true) + ":", track.name);
    ...
    LocalParticipant.prototype._removeTrack = function (track) {
        var removedTrack = _super.prototype._removeTrack.call(this, track, track._trackSender.id);
        if (removedTrack && this.state !== 'disconnected') {
            this._signaling.removeTrack(track._trackSender);
            this._log.info("Removed a " + trackClass(track, true) + ":", track.name);
            this._log.debug(trackClass(track, true) + ":", track);
        }
        return removedTrack;
    };
    ...
    LocalParticipant.prototype._handleTrackSignalingEvents = function () {
        ...
        var localTrackDisabled = function (localTrack) {
            ...
            if (trackSignaling) {
                ...
                log.debug("Disabled the " + trackClass(localTrack, true) + ":", localTrack.name);
            }
        };
        var localTrackEnabled = function (localTrack) {
            ...
            if (trackSignaling) {
                ...
                log.debug("Enabled the " + trackClass(localTrack, true) + ":", localTrack.name);
            }
        };
    ...
    LocalParticipant.prototype._getOrCreateLocalTrackPublication = function (localTrack) {
        ...
        return new Promise(function (resolve, reject) {
            function updated() {
                ...
                if (!self._tracks.has(localTrack._trackSender.id)) {
                    trackSignaling.removeListener('updated', updated);
    ...
    */ /**
    ...
    LocalParticipant.prototype.publishTrack = function (localTrackOrMediaStreamTrack, options) {
        ...
        var addedLocalTrack = this._addTrack(localTrack, options.priority)
            || this._tracks.get(localTrack._trackSender.id);
    ...
    */
    LocalParticipant.prototype.unpublishTrack = function (track) {
        ...
        var localTrack = this._tracks.get(track.id || track._trackSender.id);
        if (!localTrack) {
            return null;
        }
        var trackSignaling = this._signaling.getPublication(localTrack._trackSender);
 ...
 * @property {?number} [maxAudioBitrate] - Max outgoing audio bitrate (kbps);
 ...
 *   12 kbps to 256 kbps for Opus codec; This parameter has no effect on iSAC, PCMU
 *   and PCMA codecs
 * @property {?number} [maxVideoBitrate] - Max outgoing video bitrate (kbps);
 ...
 *   an average bitrate range of 20 kbps (20 kbps) to 8000 kbps for a 720p VideoTrack.
