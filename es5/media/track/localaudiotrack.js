...
var LocalAudioTrack = function (_LocalMediaAudioTrack) {
  ...
  function LocalAudioTrack(mediaStreamTrack, options) {
    ...
    var _this = _possibleConstructorReturn(this, (LocalAudioTrack.__proto__ || Object.getPrototypeOf(LocalAudioTrack)).call(this, mediaStreamTrack, options));
    ...
    if (options.workaroundWebKitBug1208516) {
      restartWhenInadvertentlyStopped(_this);
    }
    return _this;
  }
...
function restartWhenInadvertentlyStopped(localAudioTrack) {
  var getUserMedia = localAudioTrack._getUserMedia,
      log = localAudioTrack._log;


  function shouldReacquireTrack() {
    return document.visibilityState === 'visible' && (localAudioTrack.mediaStreamTrack.muted || localAudioTrack.isStopped) && !localAudioTrack._didCallStop;
  }

  function reacquireTrack(constraints) {
    return getUserMedia({ audio: constraints, video: false }).then(function (mediaStream) {
      return mediaStream.getAudioTracks()[0];
    });
  }

  function handleTrackStateChange() {
    Promise.race([waitForEvent(localAudioTrack.mediaStreamTrack, 'unmute'), waitForSometime(50)]).then(function () {
      if (!shouldReacquireTrack()) {
        return;
      }
      var constraints = localAudioTrack.mediaStreamTrack.getConstraints();
      log.info('Re-acquiring the MediaStreamTrack.');
      log.debug('Constraints:', constraints);
      reacquireTrack(constraints).then(function (newTrack) {
