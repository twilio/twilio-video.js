...
function playIfPausedWhileInBackground(remoteMediaTrack) {
  ...
  function onVisibilityChanged() {
    ...
    if (!isVisible) {
      // NOTE(mpatwardhan): when document goes to background, remember currently playing elements.
      elementsPlaying = [].concat(_toConsumableArray(remoteMediaTrack._attachments.values())).filter(function (el) {
        return !el.paused;
      });
    } else {
      // NOTE(mpatwardhan): when document regains foreground restore playing elements.
      elementsPlaying.forEach(function (el) {
        if (remoteMediaTrack._attachments.has(el) && el.paused) {
          try {
