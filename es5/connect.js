...
var MediaStreamEventPublisher = require('./insights/mediastreameventpublisher');
var setDefaultMediaStreamEventPublisher = require('./createlocaltracks').setDefaultMediaStreamEventPublisher;
...
function connect(token, options) {
    ...
    if (options.insights && eventObserver) {
        var mediaStreamEventPublisher = new MediaStreamEventPublisher(eventObserver, log);
        options.mediaStreamEventPublisher = mediaStreamEventPublisher;
        setDefaultMediaStreamEventPublisher(mediaStreamEventPublisher);
    }
    ...
    cancelableRoomPromise.then(function (room) {
        ...
        log.debug('Room:', room);
        room.once('disconnected', function () {
            if (options.mediaStreamEventPublisher) {
