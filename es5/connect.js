...
var _require3 = require('./util/endpoints'),
    getSignalingEndpoint = _require3.getSignalingEndpoint;

var EncodingParametersImpl = require('./encodingparameters');
var LocalParticipant = require('./localparticipant');

var _require4 = require('./media/track/es5'),
    LocalAudioTrack = _require4.LocalAudioTrack,
    LocalDataTrack = _require4.LocalDataTrack,
    LocalVideoTrack = _require4.LocalVideoTrack;
...
var _require5 = require('./util'),
    asLocalTrack = _require5.asLocalTrack,
    buildLogLevels = _require5.buildLogLevels,
    filterObject = _require5.filterObject,
    isNonArrayObject = _require5.isNonArrayObject;

var _require6 = require('./util/constants'),
    DEFAULT_ENVIRONMENT = _require6.DEFAULT_ENVIRONMENT,
    DEFAULT_LOG_LEVEL = _require6.DEFAULT_LOG_LEVEL,
    DEFAULT_REALM = _require6.DEFAULT_REALM,
    DEFAULT_REGION = _require6.DEFAULT_REGION,
    E = _require6.typeErrors;

var CancelablePromise = require('./util/cancelablepromise');
var Log = require('./util/log');

var _require7 = require('./util/validate'),
    validateBandwidthProfile = _require7.validateBandwidthProfile;
...
var deprecatedProps = [{
  ...
  process: function process(options) {
    options.enableDscp = options.dscpTagging;
    delete options.dscpTagging;
  },
  shouldWarn: false,
  didWarn: false,
  warnText: ['The ConnectOptions flag "dscpTagging" is deprecated and scheduled for removal.', 'Please use "enableDscp" instead.'].join(' ')
}, {
  name: 'abortOnIceServersTimeout',
  process: function process(options) {
    delete options.abortOnIceServersTimeout;
  },
  shouldWarn: false,
  didWarn: false,
  warnText: 'The ConnectOptions flag "abortOnIceServersTimeout" is deprecated and is not used anymore.'
}, {
  name: 'iceServersTimeout',
  process: function process(options) {
    delete options.iceServersTimeout;
  },
  shouldWarn: false,
  didWarn: false,
  warnText: 'The ConnectOptions flag "iceServersTimeout" is deprecated and is not used anymore.'
}];
...
function connect(token, options) {
  ...
  deprecatedProps.forEach(function (prop) {
    prop.shouldWarn = false;
    if (!prop.didWarn && prop.name in options) {
      prop.process(options);
      prop.shouldWarn = true;
    }
  });
  ...
  /* eslint new-cap:0 */
  var wsServer = getSignalingEndpoint(options.environment, options.region);
  ...
  // NOTE(mmalavalli): Print the deprecation warnings once if the log-level
  // is at least "warn", i.e. neither "error" nor "off".
  if (log.level !== 'error' && log.level !== 'off') {
    deprecatedProps.forEach(function (prop) {
      if (prop.shouldWarn) {
        log.warn(prop.warnText);
        prop.didWarn = true;
      }
    });
  }
  ...
  cancelableRoomPromise.then(function (room) {
    ...
    if (cancelableRoomPromise._isCanceled) {
      ...
      log.error('Error while connecting to a Room:', error);
    }
  });
...
function createRoomSignaling(token, options, signaling, encodingParameters, preferredCodecs, localParticipant) {
  ...
  var roomSignalingParams = { token: token };
