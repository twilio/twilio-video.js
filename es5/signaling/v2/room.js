...
var TranscriptionSignaling = require('./transcriptionsignaling');
...
var TrackWarningPublisher = require('../../insights/trackwarningpublisher');
var QualityEventPublisher = require('../../insights/qualityeventpublisher');
...
var STATS_COLLECTION_INTERVAL_MS = 1000;
/**
    ...
    function RoomV2(localParticipant, initialState, transport, peerConnectionManager, options) {
        ...
        Object.defineProperties(_this, {
            ...
            _transcriptionSignaling: {
                value: new options.TranscriptionSignaling(getTrackReceiver, { log: log })
            },
        ...
        periodicallyCollectAndPublishStats(_this, transport, options.statsPublishIntervalMs);
...
/**
 * Collect room stats periodically for processing and publish them at intervals.
 * This optimized version calls getStats() only once per collection interval,
 * reusing the same stats for both processing and publishing.
 * @private
 * @param {RoomV2} roomV2
 * @param {Transport} transport
 * @param {Number} publishIntervalMs
 */
function periodicallyCollectAndPublishStats(roomV2, transport, publishIntervalMs) {
    var _this = this;
    var movingAverageDeltas = new Map();
    var oddPublishCount = false;
    var collectionCount = 0;
    var publishFrequency = Math.floor(publishIntervalMs / STATS_COLLECTION_INTERVAL_MS);
    var interval = setInterval(function () { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, roomV2.getStats()];
                case 1:
                    stats = _b.sent();
                    collectionCount++;
                    // Always process stats for quality events and track warnings
                    stats.forEach(function (response) {
                        if (roomV2._qualityEventPublisher) {
                            roomV2._qualityEventPublisher.processStats(response.localVideoTrackStats);
                        }
                        if (roomV2._trackWarningPublisher) {
                            roomV2._trackWarningPublisher.processStats(response.remoteVideoTrackStats);
                        }
                    });
                    // Publish stats only at the specified publish interval (e.g., every 10 collections for 10 seconds)
                    if (collectionCount % publishFrequency === 0) {
    ...
    roomV2.on('stateChanged', function onStateChanged(state) {
        if (state === 'disconnected') {
            ...
            if (roomV2._qualityEventPublisher) {
                roomV2._qualityEventPublisher.cleanup();
            }
            if (roomV2._trackWarningPublisher) {
                roomV2._trackWarningPublisher.cleanup();
            }
