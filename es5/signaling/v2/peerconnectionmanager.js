...
var PeerConnectionManager = /** @class */ (function (_super) {
     ...
     */
    PeerConnectionManager.prototype.getRemoteTrackStats = function (mediaStreamTrack) {
        var peerConnections = Array.from(this._peerConnections.values());
        return Promise.all(peerConnections.map(function (peerConnection) { return peerConnection.getRemoteTrackStats(mediaStreamTrack).then(function (stats) { return [
            peerConnection.id,
            stats
        ]; }); })).then(function (reports) { return new Map(reports); });
    };
    /**
