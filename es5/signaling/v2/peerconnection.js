...
var PeerConnectionV2 = function (_StateMachine) {
  ...
  _createClass(PeerConnectionV2, [{
    ...
    value: function _setLocalDescription(description) {
      ...
      var descriptionToApply = description.type !== 'rollback' && this._shouldApplyDtx ? new this._RTCSessionDescription({
        sdp: enableDtxForOpus(description.sdp),
        type: description.type
      }) : description;

      return this._peerConnection.setLocalDescription(descriptionToApply).catch(function (error) {
        _this12._log.warn('Calling setLocalDescription with an RTCSessionDescription of type "' + descriptionToApply.type + '" failed with the error "' + error.message + '".');
        if (descriptionToApply.sdp) {
          _this12._log.warn('The SDP was ' + descriptionToApply.sdp);
        }
        throw new MediaClientLocalDescFailedError();
      }).then(function () {
