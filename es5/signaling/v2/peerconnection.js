...
var _require5 = require('../../util/sdp'),
    ...
    unifiedPlanFilterRemoteVideoCodecs = _require5.unifiedPlanFilterRemoteVideoCodecs;
    ...
    value: function _setRemoteDescription(description) {
      ...
      if (description.sdp) {
        ...
        if (isFirefox) {
          ...
          // preferred codec is selected does not work on Mobile Firefox due to this behavior:
          // https://bugzilla.mozilla.org/show_bug.cgi?id=1683258. So, we remove H264 from the
          // remote sdp in order to fix VIDEO-3736.
          var preferredVideoCodec = this._preferredVideoCodecs[0];
          var peerSdp = this._peerConnection.localDescription ? this._peerConnection.localDescription.sdp : description.sdp;
          if (preferredVideoCodec && preferredVideoCodec.codec.toLowerCase() !== 'h264') {
            description.sdp = unifiedPlanFilterRemoteVideoCodecs(description.sdp, peerSdp, ['h264']);
          }
        }
