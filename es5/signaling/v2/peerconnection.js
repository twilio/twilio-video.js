...
var PeerConnectionV2 = function (_StateMachine) {
  ...
  _createClass(PeerConnectionV2, [{
    ...
    value: function _addOrUpdateTransceiver(track) {
      var transceiver = this._peerConnection.getTransceivers().find(function (_ref) {
        var currentDirection = _ref.currentDirection,
            direction = _ref.direction,
            mid = _ref.mid,
            receiver = _ref.receiver,
            stopped = _ref.stopped;
        return currentDirection && mid && !stopped && direction === 'recvonly' && receiver && receiver.track && receiver.track.kind === track.kind;
      });

      if (transceiver && transceiver.sender) {
        this._replaceTrackPromises.push(transceiver.sender.replaceTrack(track).catch(function () {
          // Do nothing.
        }));
    ...
    value: function _rewriteLocalTrackIds(description) {
      var midsToTrackIds = new Map(this._peerConnection.getTransceivers().filter(function (_ref2) {
        var mid = _ref2.mid,
            sender = _ref2.sender,
    ...
    value: function _setLocalDescription(description) {
      ...
      return Promise.resolve().then(function () {
        if (description.sdp) {
          ...
          description = {
            type: description.type,
            sdp: isChrome && vp8SimulcastRequested ? _this8._setSimulcast(description.sdp, sdpFormat, _this8._trackIdsToAttributes) : description.sdp
          };
        }
