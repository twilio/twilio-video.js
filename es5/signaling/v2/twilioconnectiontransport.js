...
var TwilioConnectionTransport = function (_StateMachine) {
  ...
  function TwilioConnectionTransport(name, accessToken, localParticipant, peerConnectionManager, wsServer, options) {
    ...
    var iceServersStatus = options.overrideIceServers ? 'override' : 'acquire';
    Object.defineProperties(_this, {
      ...
      _eventPublisher: {
        value: new EventPublisher(accessToken, SDK_NAME, SDK_VERSION, options.environment, options.region, eventPublisherOptions)
      },
      _iceServersStatus: {
        value: iceServersStatus
      },
      _localParticipant: {
  ...
  _createClass(TwilioConnectionTransport, [{
    ...
    key: 'resumeConnect',
    value: function resumeConnect() {
      this._sendConnectOrSyncOrDisconnectMessage();
    }
  }, {
    key: '_sendIceMessage',
    value: function _sendIceMessage() {
      this._twilioConnection.sendMessage({
    ...
    value: function _startConnect() {
      if (this._iceServersStatus === 'acquire') {
...
function setupEventListeners(transport) {
  ...
  function startConnect() {
    ...
    twilioConnection.once('open', function () {
      return transport._startConnect();
    });
  }
  ...
  function handleMessage(message) {
    ...
    switch (transport.state) {
      ...
      case 'connecting':
        switch (message.type) {
          case 'iced':
            transport._iceServers = message.ice_servers;
            transport.emit('iced', transport._iceServers);
            return;
          case 'connected':
