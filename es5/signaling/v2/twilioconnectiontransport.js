...
function setupEventListeners(transport) {
  function createOrResetTwilioConnection() {
    ...
    var twilioConnection = new TwilioConnection(_wsServer, Object.assign({
      helloBody: transport._createConnectOrSyncOrDisconnectMessage()
    }, _options));
  ...
  function disconnect(error) {
    ...
    if (!reconnectTimer) {
      var twilioError = error.code === TwilioConnection.CloseCode.SERVER_BUSY ? new SignalingServerBusyError() : new SignalingConnectionError();
