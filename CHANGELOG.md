...
The [PreflightTestReport](https://sdk.twilio.com/js/video/releases/2.17.0/docs/global.html#PreflightTestReport) generated by `preflightTest` on `completed` provides statistics that can be used to determine the quality of the connection. Some of the useful stats and their optimal values are listed below.

| Stat                     |  Description | Optimal values
| -------------------------|--------------|------------------|
...
The API also calculates an overall `qualityScore` using jitter, packet loss, and rtt values observed. The quality score can be one of:
