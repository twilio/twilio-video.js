...
function runPreflightTest(publisherToken, subscriberToken, options, preflightTest) {
  ...
  function collectStats(collectedStats) {
    return subscriberRoom._signaling.getStats().then(responses => {
      ...
      if (resp.length > 0) {
        ...
        if (activeIceCandidatePair) {
          collectedStats.rtt.push(activeIceCandidatePair.totalRoundTripTime);
          ...
          if (!collectedStats.selectedLocalIceCandidate) {
            collectedStats.selectedLocalIceCandidate = activeIceCandidatePair.localCandidate;
          }
          if (!collectedStats.selectedRemoteIceCandidate) {
            collectedStats.selectedRemoteIceCandidate = activeIceCandidatePair.remoteCandidate;
          }
        }
  ...
  function generatePreflightReport(collectedStats) {
    ...
    return {
      ...
      selectedLocalIceCandidate: collectedStats.selectedLocalIceCandidate,
      selectedRemoteIceCandidate: collectedStats.selectedRemoteIceCandidate,
      localIceCandidates: collectedStats.localIceCandidates,
    };
  }

  function collectStatsPeriodically(duration, collectedStats) {
    const startTime = Date.now();
    const STAT_INTERVAL = 1000;
    const firstCall = !collectedStats;
    ...
    if (firstCall) {
      ...
      // add initial network quality level
      if (localParticipant.networkQualityLevel) {
        collectedStats.networkQuality.push(localParticipant.networkQualityLevel);
      }
      // and listen for updates to networkQuality
      networkQualityCallback = () => collectedStats.networkQuality.push(localParticipant.networkQualityLevel);
      localParticipant.addListener('networkQualityLevelChanged', networkQualityCallback);
    }
    return collectStats(collectedStats).then(() => {
      const remainingDuration = duration - (Date.now() - startTime);
      if (remainingDuration > 0) {
        return waitForSometime(STAT_INTERVAL).then(() => {
          return collectStatsPeriodically(remainingDuration - 1000, collectedStats);
        });
      }
