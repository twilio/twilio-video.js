    ...
    return Promise.all([subscriberRoom, publisherRoom].map(room => room._signaling.getStats()))
      .then(([subscriberStats, publisherStats]) => {
        ...
        if (publisherStatValues.length > 0) {
          ...
          if (activeIceCandidatePair && activeIceCandidatePair.availableOutgoingBitrate) {
...
/**
 * Emitted to report any warning encountered during the the test
 * @param {string} name - name for the warning
