...
Endpoint.prototype.unlisten = function unlisten() {
  ...
  // NOTE(mroberts): Unregister only if we are not active (and not about to be
  // active in) Conversations.
  return this._maybeUnregister();
};
...
Endpoint.prototype._maybeUnregister = function _maybeUnregister() {
  if (this.isListening
    || this._pendingConversations.size
