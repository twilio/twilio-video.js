...
/**
 * Constructs a new {@link Endpoint} with a token.
 * @class
 ...
 * @property {bool} isListening - Whether the {@link Endpoint} is listening for
 *   {@link Invite}s to {@link Conversation}s
 * @property {Set<Invite>} invites - The active {@link Invite}s this
 *   {@link Endpoint} has received
 * @property {AccessToken} token - The {@link Endpoint}'s active {@link AccessToken}
  ...
  function inviteAddressesFailed(reason) {
    ...
    switch (reason.message) {
      case 'canceled':
        ...
        break;
      case 'rejected':
        message = 'Conversation invite was rejected or Participant does not exist';
        break;
      case 'ignored':
...
/**
 * Sets the active token of this {@link Endpoint}
 * @instance
 * @private
 * @returns {AccessToken}
 */
Endpoint.prototype._createToken = function _createToken(token, log) {
  ...
  // Fire Endpoint#tokenExpired if this token
...
/**
 * Your {@link Endpoint} has run into an error.
