...
function Endpoint(token, options) {
  ...
  var log = new Log('Endpoint', options.logLevel);
  ...
  var thenPromise = stream.then(function(stream) {
    ...
    var ictPromises = icts.map(function(ict) {
      return ict.then(function(dialog) {
        ...
        if (reason.canceled) {
          message = 'Invite to remote endpoint was canceled';
        } else if (reason.rejected) {
          message = 'Remote endpoint rejected invite';
        } else if(reason === 'ignored') {
          message = 'Invite to remote endpoint timed out';
        } else if (reason.failed) {
          message = 'Remote endpoint unreachable';
        }

        self._log.throw(E.SESSION_CONNECT_FAILED, message);
      });
    });
