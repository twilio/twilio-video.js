...
Endpoint.prototype.register = function register() {
  ...
  return this._userAgent.register();
};
...
Endpoint.prototype.createSession = function createSession(participants, options) {
  ...
  var session = new Session(this, participants, options);
  session._endpoints.add(this);
  this.sessions.add(session);
  return session.invite(participants).then(util.return(session));
};
 ...
 */
// TODO(mroberts): I'm pretty sure constraints do not work right now.
Endpoint.prototype.join = function join(session, constraints) {
  var self = this;
  ...
  if (pending.length === 0) {
    return session.invite(this).then(util.return(session));
  }
  ...
  return Q.all(pending.map(function(dialog) {
    return self._userAgent.accept(dialog);
...
Endpoint.prototype.muteAudio = function muteAudio(mute, session) {
  var streams = [];
  if (!session) {
    this.streams.values.forEach(function(_streams) {
