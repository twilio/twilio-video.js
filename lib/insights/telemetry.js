...
class Telemetry {
   ...
   * @param {number} [config.connectTimestamp] - Connection timestamp (defaults to Date.now())
   * @returns {Telemetry}
   */
  configure({ publisher, log, connectTimestamp }) {
    if (!publisher || !log) {
      throw new Error('Telemetry.configure requires publisher and log');
    }
    ...
    this._log = log;
    this._connectTimestamp = connectTimestamp || Date.now();
