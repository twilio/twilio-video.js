...
class TrackWarningPublisher {
  ...
  processStats(remoteVideoTrackStats) {
    ...
    remoteVideoTrackStats.forEach(({ trackSid, frameRateReceived }) => {
      // NOTE(lrivas): frameRateReceived may be null or undefined when not available (i.e., no frames received, receiving less than 1 frame per second).
      // We convert it to 0 to simplify the stalled detection logic and comply with our specification.
