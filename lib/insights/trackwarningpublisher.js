...
/**
 * @typedef {import('../../tsdef/types').RemoteVideoTrackStats} RemoteVideoTrackStats
 */
/**
...
class TrackWarningPublisher {
   ...
   * @param {RemoteVideoTrackStats[]} remoteVideoTrackStats
   */
  processStats(remoteVideoTrackStats) {
    ...
    remoteVideoTrackStats.forEach(({ trackSid, frameRate }) => {
      if (trackSid && typeof frameRate === 'number') {
   ...
   * @private
   * @param {RemoteVideoTrackStats['trackSid']} trackSid - The Track Sid
   * @param {RemoteVideoTrackStats['frameRate']} frameRate - Current frame rate
   */
