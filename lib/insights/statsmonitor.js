...
const MovingAverageMetrics = require('./movingaveragemetrics');
 ...
 *
 * @example
 * const statsMonitor = new StatsMonitor(eventObserver, log, {
...
class StatsMonitor {
  ...
  constructor(eventObserver, log, options = {}) {
    ...
    this._movingAverageMetrics = new MovingAverageMetrics();

    // Stats collection and publishing state
    this._interval = null;
  ...
  async _collectAndAnalyzeStats() {
    try {
      ...
      // Determine if we should publish stats-report (every 10 seconds by default)
      const shouldPublishStatsReport = (this._statsCollectionCount % this._collectionsPerPublish) === 0;

      // Toggle ICE candidate publishing to publish every other stats-report cycle (every 20 seconds by default)
      ...
      // Do nothing - stats collection failures are expected occasionally
    }
  }
  ...
  _analyzeStats(stats, shouldPublishStatsReport, shouldPublishIceCandidate) {
    ...
    stats.forEach((response, id) => {
      // Analyze stats every collection cycle
      ...
      // Publish stats reports only on publish cycles
      if (shouldPublishStatsReport) {
        this._publishStatsReport(id, response);

        if (shouldPublishIceCandidate) {
          this._publishActiveIceCandidatePair(response);
        }
      }
    });
  }
    ...
    this._eventObserver.emit('event', {
      ...
      payload: {
        audioTrackStats: report.remoteAudioTrackStats.map((trackStat, i) =>
          this._movingAverageMetrics.addRemoteTrackMetrics(trackStat, response.remoteAudioTrackStats[i])),
        localAudioTrackStats: report.localAudioTrackStats.map((trackStat, i) =>
          this._movingAverageMetrics.addLocalTrackMetrics(trackStat, response.localAudioTrackStats[i])),
        localVideoTrackStats: report.localVideoTrackStats.map((trackStat, i) =>
          this._movingAverageMetrics.addLocalTrackMetrics(trackStat, response.localVideoTrackStats[i])),
        peerConnectionId: report.peerConnectionId,
        videoTrackStats: report.remoteVideoTrackStats.map((trackStat, i) =>
          this._movingAverageMetrics.addRemoteTrackMetrics(trackStat, response.remoteVideoTrackStats[i])),
      }
    });

    this._movingAverageMetrics.cleanupFromReport(report);
  }
   ...
   * @param {Object} response - StandardizedStatsResponse
   */
  _publishActiveIceCandidatePair(response) {
    const peerConnectionId = response.peerConnectionId || '';
  ...
  cleanup() {
    ...
    this._movingAverageMetrics.clear();
