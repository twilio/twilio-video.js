...
/**
 * @typedef {import('../signaling/v2/room')} RoomSignalingV2
 */

/**
 * @typedef {import('../util/log')} Log
 */
...
class StatsMonitor {
  _stallThreshold = 0.5;
  _resumeThreshold = 5;
  _movingAverageDeltas = new Map();
  /** @type {number | NodeJS.Timeout | null} */
  _interval = null;
  _statsCollectionCount = 0;
  _iceCandidatePublishToggle = false;
  _hasSeenActivePair = false;
  _lastNetworkType = null;
  _lastQualityLimitationReasonByTrackSid = new Map();
  _stalledTrackSids = new Set();

  /**
   * Create a StatsMonitor
   * @param {RoomSignalingV2} roomSignalingV2 - Room signaling instance that provides getStats() method
   ...
   */
  constructor(roomSignalingV2, log, options = {}) {
    if (!roomSignalingV2) {
      throw new Error('StatsMonitor requires a room signaling instance');
    }
    if (!log) {
      throw new Error('StatsMonitor requires a log instance');
    }

    this._roomSignalingV2 = roomSignalingV2;
    this._log = log;
  ...
  async _collectAndAnalyzeStats() {
    try {
      const stats = await this._roomSignalingV2.getStats();
   ...
   * @private
   * @param {Map<string, StandardizedStatsResponse>} stats - Map or Array of StandardizedStatsResponse objects
   ...
   * @private
   * @param {StandardizedStatsResponse} statsResponse - StandardizedStatsResponse
   */
  _checkNetworkTypeChanges(statsResponse) {
    const { activeIceCandidatePair } = statsResponse;
   ...
   * @private
   * @param {StandardizedStatsResponse['localVideoTrackStats']} localVideoTrackStats - Local video track statistics
   */
  _checkQualityLimitations(localVideoTrackStats) {
    localVideoTrackStats.forEach(({ trackSid, qualityLimitationReason }) => {
      if (!trackSid || !qualityLimitationReason) {
        return;
      }
   ...
   * @private
   * @param {StandardizedStatsResponse['remoteVideoTrackStats']} remoteVideoTrackStats - Remote video track statistics
   */
  _checkTrackStalls(remoteVideoTrackStats) {
    ...
    remoteVideoTrackStats.forEach(({ trackSid, frameRate: frameRateReceived }) => {
