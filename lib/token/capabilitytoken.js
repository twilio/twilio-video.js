...
/**
 * Parses a JWT to a {@link CapabilityToken}.
 *
 * @classdesc A {@link CapabilityToken} wraps a JSON Web Token (JWT)
 * representing Twilio Client capabilities (e.g. the ability to make incoming
 * or outgoing calls).
 *
 * @constructor
 *
 ...
 */
function CapabilityToken(capabilityTokenString) {
  if (!(this instanceof CapabilityToken)) {
    return new CapabilityToken(capabilityTokenString);
  }
  ...
  if (supportsOutgoing && !outgoingAppSid) {
    throw constants.errors.OUTGOING_NO_APPSID;
  }
  ...
  if (supportsIncoming && !incomingClientName) {
    throw constants.INCOMING_NO_CLIENT_NAME;
  }
...
function parsePayload(jwt) {
  var segs = jwt.split('.');
  if (segs.length !== 3) {
    throw constants.errors.WRONG_NUMBER_OF_SEGMENTS;
  }
...
function parseScope(uri) {
    ...
    if (!(parts && parts.length === 4)) {
        throw constants.errors.BAD_SCOPE_URI;
    }
    return {
        service: parts[1],
        privilege: parts[2],
