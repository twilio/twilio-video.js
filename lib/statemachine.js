...
/**
 * Throw an Error if the {@link StateMachine} was preempted.
 * @param {StateMachine#Key} key
 * @param {?Error} [error]
 * @throws {Error}
 */
StateMachine.prototype.throwIfPreempted = function throwIfPreempted(key,
  error) {
  error = error || new Error('StateMachine was preempted');
  if (!this.hasLock(key)) {
    throw error;
  }
};
