...
MIDTrackMatcher.prototype.match = function match(event) {
  return this._midToTrackIds.get(event.transceiver.mid) || null;
};
...
MIDTrackMatcher.prototype.update = function update(sdp) {
  ...
  this._midToTrackIds = sections.reduce(function(midToTrackIds, section) {
    ...
    return mid && trackId ? midToTrackIds.set(mid, trackId) : midToTrackIds;
  }, this._midToTrackIds);
};
