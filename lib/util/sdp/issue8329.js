...
function mediaSectionWorkaround(mediaSection) {
  ...
  var unassociatedPts = knownCodecNames.reduce((unassociatedPts, codecName) => {
...
function createAssociatedPtToRtxPt(rtxPtToAssociatedPt, invalidRtxPts) {
  ...
  var associatedPtToRtxPts = Array.from(rtxPtToAssociatedPt).reduce((associatedPtToRtxPts, pair) => {
    var rtxPt = pair[0];
    var pt = pair[1];
