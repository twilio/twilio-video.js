...
class TrackAttributes {
   ...
   */
  toSdpLines() {
    const rtxPairs = Array.from(this.rtxPairs.entries()).map(rtxPair => rtxPair.reverse());
...
function setSimulcastInMediaSection(section, trackIdsToAttributes) {
  ...
  const relevantSdpLines = flatMap(relevantTrackAttributes, trackAttributes => trackAttributes.toSdpLines());
