...
var simulcast = require('./simulcast');
 ...
 * @param {string} sdp
 * @param {Array<VideoCodecSettings>} preferredVideoCodecs
 * @returns {string} Updated SDP string
 */
function setSimulcast(sdp, preferredVideoCodecs, options) {
  options = Object.assign({
    guessBrowser: guessBrowser
  }, options);

  var vp8CodecSettings = preferredVideoCodecs.find(function(codecSettings) {
    return codecSettings.codec.toLowerCase() === 'vp8';
  }) || { codec: 'VP8', simulcast: false };

  if (!(options.guessBrowser() === 'chrome' && vp8CodecSettings.simulcast)) {
    return sdp;
  }
  var mediaSections = getMediaSections(sdp);
  var session = sdp.split('\r\nm=')[0];
  ...
  return [session].concat(mediaSections.map(function(section) {
    ...
    var codecMap = createCodecMapForMediaSection(section);
    var payloadTypes = getPayloadTypesInMediaSection(section);
    return payloadTypes[0] === codecMap.get('vp8') ? simulcast(section) : section;
  })).join('\r\n');
}
