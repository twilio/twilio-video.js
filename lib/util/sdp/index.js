...
function enableDtxForOpus(sdp) {
  ...
  return [session].concat(mediaSections.map(section => {
    ...
    // Add "usedtx=1" to opus's a=fmtp: line.
    const opusFmtpRegex = new RegExp(`a=fmtp:${opusPt} (.+)\r\n`);
    return section.replace(opusFmtpRegex, `a=fmtp:${opusPt} $1;usedtx=1\r\n`);
  })).join('\r\n');
}
