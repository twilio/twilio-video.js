 ...
 * @returns {string}
 */
function unifiedPlanFilterCodecsInLocalMediaSection(localSection, remoteMidsToMediaSections, codecsToRemove) {
  ...
  let localPts = flatMap(Array.from(remotePtToCodecs), ([remotePt, remoteCodec]) =>
    remoteCodec !== 'rtx' && !codecsToRemove.includes(remoteCodec)
 ...
 */
function unifiedPlanFilterLocalCodecs(localSdp, remoteSdp, codecsToRemove = []) {
  ...
  return [localSession].concat(localMediaSections.map(localSection => {
    return unifiedPlanFilterCodecsInLocalMediaSection(localSection, remoteMidsToMediaSections, codecsToRemove);
  })).join('\r\n');
}
