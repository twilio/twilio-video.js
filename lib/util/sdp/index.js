...
function getReorderedPayloadTypes(codecMap, preferredCodecs) {
  ...
  var preferredPayloadTypes = flatMap(preferredCodecs, codecName => codecMap.get(codecName) || []);

  var remainingCodecs = difference(Array.from(codecMap.keys()), preferredCodecs);
  var remainingPayloadTypes = flatMap(remainingCodecs, codecName => codecMap.get(codecName));
...
function setCodecPreferences(sdp, preferredAudioCodecs, preferredVideoCodecs) {
  ...
  return [session].concat(mediaSections.map(section => {
    ...
    var preferredCodecs = kind === 'audio' ? preferredAudioCodecs : preferredVideoCodecs.map(codec => codec.codec);
...
function setSimulcast(sdp, trackIdsToAttributes) {
  ...
  return [session].concat(mediaSections.map(section => {
    ...
    var hasVP8PayloadType = payloadTypes.some(payloadType => vp8PayloadTypes.has(payloadType));
