...
function unifiedPlanFilterCodecsInMediaSection(section, peerMidsToMediaSections, codecsToRemove) {
  ...
  let pts = flatMap(Array.from(peerPtToCodecs), ([peerPt, peerCodec]) =>
    peerCodec !== 'rtx' && !codecsToRemove.includes(peerCodec)
      ? unifiedPlanGetMatchingPayloadTypes(
        peerCodec,
        peerPt,
        codecsToPts,
        section,
...
function unifiedPlanFilterRemoteVideoCodecs(remoteSdp, localSdp, codecsToRemove) {
  ...
  const remoteSession = remoteSdp.split('\r\nm=')[0];
  const localMidsToMediaSections = createMidToMediaSectionMap(localSdp);
  return [remoteSession].concat(remoteMediaSections.map(remoteSection => {
    if (!/^m=video/.test(remoteSection)) {
      return remoteSection;
    }
    const remoteCodecMap = createCodecMapForMediaSection(remoteSection);

    const remainingRemoteCodecs = difference(
      Array.from(remoteCodecMap.keys()),
      codecsToRemove.concat('rtx'));
    ...
    return unifiedPlanFilterCodecsInMediaSection(
      remoteSection,
      localMidsToMediaSections,
      remainingRemoteCodecs.size > 0 ? codecsToRemove : []);
  })).join('\r\n');
}
