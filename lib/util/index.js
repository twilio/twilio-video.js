 ...
 * @param {object} stunTurnToken
 * @param {?string} session
 * @returns {Array}
 */
function makeInviteHeaders(info, capabilityToken, params, stunTurnToken, session) {
  var cmg = [
  ...
  var vss = [
    headers.X_TWILIO_USERNAME      + ': ' + stunTurnToken.username,
    headers.X_TWILIO_PASSWORD      + ': ' + stunTurnToken.password
  ];
  if (session) {
    vss.push(headers.X_TWILIO_SESSION + ': ' + session);
  }
  return cmg.concat(vss);
}
 ...
 * @returns {Array}
 */
function makeRegisterHeaders(info, capabilityToken, stunTurnToken) {
  var cmg = [
    headers.X_TWILIO_TOKEN         + ': ' + capabilityToken.capabilityTokenString,
    headers.X_TWILIO_CLIENT        + ': ' + JSON.stringify(info),
    headers.X_TWILIO_CLIENTVERSION + ': ' + constants.CLIENT_VERSION
