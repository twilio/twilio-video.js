...
const { typeErrors: E, trackPriority, DEPRECATED_OPTIONS } = constants;
...
/**
 * Log deprecation warnings for the given options object
 * @param {Object} options
 * @param {Log} log
 */
function deprecateOptions(options, log) {
  Object.keys(options)
    .map(option => DEPRECATED_OPTIONS[option])
    .filter(warning => !!warning)
    .forEach(warning => log.deprecated(warning));
}
