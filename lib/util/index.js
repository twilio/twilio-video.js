 ...
 * @param {CapabilityToken} capabilityToken - the {@link CapabilityToken}
 * @param {?object} params - the client parameters
 * @param {?string} session
 * @returns {Array}
 */
function makeInviteHeaders(info, capabilityToken, params, session) {
  var cmg = [
    headers.X_TWILIO_TOKEN         + ': ' + capabilityToken.capabilityTokenString,
...
function any(promises) {
  promises = promises.map(cancelify);
  
  var deferred = Q.defer();
  deferred.promise = cancelify(deferred.promise);
  ...
  var cancelDeferred = deferred.promise.cancel;
...
function emitNext() {
 ...
 /**
  * Error to be thrown when a promise is canceled
  * @class
  * @classdesc Error to be thrown when a promise is canceled.
  * @param {String} message - Content of error message
 */
...
/**
 * Wrap a promise with a deferred so that it may be canceled with .cancel()
 * @param {Promise} original - The original, uncancelable promise.
 * @returns {Promise} A promise with the .cancel() method attached.
 */
function cancelify(original) {
  ...
  wrapped.promise.cancel = function(reason) {
    cancelDeferred.reject(new CanceledError(reason));
  };
...
module.exports.cancelify = cancelify;
module.exports.CanceledError = CanceledError;
