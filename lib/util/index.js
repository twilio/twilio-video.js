...
function createBandwidthProfilePayload(bandwidthProfile) {
  return typeof bandwidthProfile.video === 'object' ? [
    ['dominantSpeakerPriority', 'string', 'active_speaker_priority'],
    ['maxSubscriptionBitrate', 'number', 'max_subscription_bandwidth', x => Math.round(x / 1024)],
    ['maxTracks', 'number', 'max_tracks'],
    ['mode', 'string'],
    ['renderDimensions', 'object', 'render_dimensions', createRenderDimensionsPayload]
  ].reduce((payload, [prop, dataType, payloadProp = prop, transform = x => x]) => {
    const value = bandwidthProfile.video[prop];
    if (typeof value === dataType) {
      payload.video = Object.assign({
        [payloadProp]: transform(value)
      }, payload.video);
    }
    return payload;
  }, {}) : {};
...
function createDimensionsPayload(dimensions) {
  return ['height', 'width'].reduce((payload, dimension) => {
...
function createRenderDimensionsPayload(renderDimensions) {
  return [
    ['dominantSpeaker', 'active_speaker']
  ].concat(Object.values(constants.trackPriority).map(priority => {
    return [priority];
  })).reduce((payload, [prop, payloadProp = prop]) => {
    const value = renderDimensions[prop];
    if (typeof value === 'object') {
      payload = Object.assign({
        [payloadProp]: createDimensionsPayload(value)
      }, payload);
    }
    return payload;
  }, {});
