...
function createBandwidthProfileVideoPayload(bandwidthProfileVideo) {
  return createRSPPayload(bandwidthProfileVideo, [
    ...
    { prop: 'maxSubscriptionBitrate', type: 'number', payloadProp: 'max_subscription_bandwidth', transform: x => Math.round(x / 1024) },
...
function createRenderDimensionsPayload(renderDimensions) {
  const { PRIORITY_HIGH, PRIORITY_LOW, PRIORITY_STANDARD } = constants.trackPriority;
  return createRSPPayload(renderDimensions, [
    { prop: 'dominantSpeaker', type: 'object', payloadProp: 'active_speaker', transform: createDimensionsPayload },
 ...
 * @param {object} object
 * @param {Array<object>} propConversions
 * @returns {*}
 */
