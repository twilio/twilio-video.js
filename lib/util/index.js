...
function createBandwidthProfilePayload(bandwidthProfile) {
  const payload = {};
  if (typeof bandwidthProfile.video !== 'object') {
    return payload;
  }

  const mode = bandwidthProfile.video.mode;
  if (typeof mode === 'string') {
    payload.video = Object.assign({ mode }, payload.video);
  }

  const maxSubscriptionBitrate = bandwidthProfile.video.maxSubscriptionBitrate;
  ...
  const maxTracks = bandwidthProfile.video.maxTracks;
