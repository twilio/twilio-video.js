...
function createRoomConnectEventPayload(connectOptions) {
  ...
  // boolean properties.
  [['audio'], ['automaticSubscription'], ['enableDscp'], ['preflight'], ['video'], ['dominantSpeaker', 'enableDominantSpeaker']].forEach(([prop, eventProp]) => {
  ...
  if (connectOptions.bandwidthProfile && connectOptions.bandwidthProfile.video) {
    ...
    ['mode', ['maxSwitchedOnTracks', 'maxVideoTracks'], 'trackSwitchOffMode', 'dominantSpeakerPriority', 'maxSubscriptionBitrate', 'contentPreferencesMode', 'clientTrackSwitchOffControl'].forEach(prop => {
      const destProp = Array.isArray(prop) ? prop[1] : prop;
      const srcProp = Array.isArray(prop) ? prop[0] : prop;

      const srcPropValue = videoBPOptions[srcProp];
      ...
      } else if (typeof videoBPOptions[destProp] === 'object') {
        payload.bandwidthProfileOptions[destProp] = JSON.stringify(srcPropValue);
      }
    });

    const audioBPOptions = connectOptions.bandwidthProfile.audio || {};
    [['maxSwitchedOnTracks', 'maxAudioTracks']].forEach(prop => {
      const destProp = Array.isArray(prop) ? prop[1] : prop;
      const srcProp = Array.isArray(prop) ? prop[0] : prop;

      const srcPropValue = audioBPOptions[srcProp];
      if (typeof srcPropValue === 'number' || typeof srcPropValue === 'string') {
        payload.bandwidthProfileOptions[destProp] = srcPropValue;
      } else if (typeof srcPropValue === 'boolean') {
        payload.bandwidthProfileOptions[destProp] = boolToString(srcPropValue);
...
function createBandwidthProfileVideoPayload(bandwidthProfileVideo) {
  return createRSPPayload(bandwidthProfileVideo, [
    ...
    { prop: 'maxSubscriptionBitrate', type: 'number', payloadProp: 'max_subscription_bandwidth', transform: val => val * 1000 },
    { prop: 'maxSwitchedOnTracks', type: 'number', payloadProp: 'max_tracks' },
