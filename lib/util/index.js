...
/**
 * Ensure that the given function is called once per given duration.
 * @param {function} fn - Function to be executed
 * @param {number} durationMs - Duration in milliseconds
 * @returns {function} - Schedules the given function to be called in the next duration
 */
function oncePerDuration(fn, durationMs) {
  let timeout = null;
  ...
  return function scheduleNextDuration() {
    if (timeout) {
      clearTimeout(timeout);
    }
    timeout = setTimeout(nextDuration, durationMs);
  };
...
exports.oncePerDuration = oncePerDuration;
