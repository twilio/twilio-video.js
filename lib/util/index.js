...
function createRoomConnectEventPayload(connectOptions) {
  ...
  // optional numbers and string properties.
  ['maxVideoBitrate', 'maxAudioBitrate', 'iceTransportPolicy', 'region'].forEach(prop => {
    if (typeof connectOptions[prop] === 'number' || typeof connectOptions[prop] === 'string') {
      payload[prop] = connectOptions[prop];
    }
  });

  // some array props stringified.
  ['preferredAudioCodecs', 'preferredVideoCodecs'].forEach(prop => {
    if (typeof connectOptions[prop] !== 'undefined') {
