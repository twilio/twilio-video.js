...
class Backoff {
  ...
  constructor(fn, min, max, jitter, factor) {
    Object.assign({
      min: min || 100,
      max: max || 10000,
      jitter: jitter > 0 && jitter <= 1 ? jitter : 0,
      factor: factor || 2,
    });
  ...
  */
  duration() {
    let min = this.min * Math.pow(this.factor, this._attempts++);
    if (this.jitter) {
      const rand =  Math.random();
      const deviation = Math.floor(rand * this.jitter * min);
      min = (Math.floor(rand * 10) & 1) === 0  ? min - deviation : min + deviation;
    }
    this._duration = Math.min(min, this.max) | 0;
