...
class Log {
   ...
   */
  constructor(moduleName, component, logLevels, loggerName) {
  ...
  createLog(moduleName, component) {
    return new Log(moduleName, component, this._logLevels, this._loggerName);
  }
...
/* istanbul ignore next */
['DEBUG', 'INFO', 'WARN', 'ERROR', 'OFF'].forEach((level, i) => {
  Object.defineProperty(Log, level, { value: i });
  if (!Log._levels) {
    Object.defineProperty(Log, '_levels', { value: [] });
  }
  Log._levels.push(level);
});

const LOG_LEVELS_SET = {};
const LOG_LEVEL_VALUES = [];

const LOG_LEVEL_NAMES = Log._levels.forEach((level, i) => {
