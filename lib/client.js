...
/**
 * Constructs a new {@link Client} with an {@link AccessToken}.
 * @class
 * @classdesc A {@link Client} is identified by an address and
 *   can create or join {@link Conversation}s.
 * @param {string} token - The {@link Client}'s token
 * @param {Client#ClientOptions} [options] - Options to override the
 *   constructor's default behavior
 * @property {string} address - The {@link Client}'s address (defined by its
 *   token)
 ...
 * @property {AccessToken} token - The {@link Client}'s active {@link AccessToken}
 * @fires Client#invite
 * @fires Client#error
 */ 
 ...
 *
 * var alice = new Twilio.Conversations.Client(originalToken);
 *
 ...
 * }
 */
Client.prototype.listen = function listen(token) {
...
Client.prototype._register = function _register(token) {
 ...
 * });
 */
Client.prototype.createConversation = function createConversation(addresses, options) {
  ...
  this.token._validateAddress(addresses);
...
/**
 * Sets the active token of this {@link Client}
 * @private
 * @returns {AccessToken}
 */
Client.prototype._createToken = function _createToken(token, log) {
  ...
  // Throw an error if the passed token was already expired.
  if (token.isExpired) {
    throw E.INVALID_TOKEN.clone('Supplied token is expired');
  }

  // Fire Client#tokenExpired if this token
