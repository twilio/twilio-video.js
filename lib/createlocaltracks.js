...
const getUserMediaReal = require('@twilio/webrtc').getUserMedia;
let getUserMediaCallId = 0;
const getUserMedia = (...args) => {
  const callId = getUserMediaCallId++;
  console.log(`makarand: ${callId}: calling getUserMedia with `, ...args);
  return getUserMediaReal(...args).then(result => {
    console.log(`makarand: ${callId} resolved`);
    return result;
  }).catch(err => {
    console.log(`makarand: ${callId} rejected`, err);
    throw err;
  });
};
...
function createLocalTracks(options) {
  ...
  if (options.audio && options.audio.workaroundWebKitBug1208516 !== undefined) {
    localTrackNameOptions.audio.workaroundWebKitBug1208516 = options.audio.workaroundWebKitBug1208516;
  }

  // eslint-disable-next-line no-undefined
  if (options.video && options.video.workaroundWebKitBug1208516 !== undefined) {
    localTrackNameOptions.video.workaroundWebKitBug1208516 = options.video.workaroundWebKitBug1208516;
  }
