...
const { subscriptionMode, trackPriority, typeErrors: E } = require('./util/constants');
 ...
 *   specify the default subscribe priority of the Dominant Speaker's {@link RemoteVideoTrack}s.
...
function validateBandwidthProfile(bandwidthProfile) {
  ...
  return [
    ['dominantSpeakerPriority', Object.values(trackPriority), E.INVALID_VALUE],
    ...
    return typeof video[prop] === typeOrValues ? null
      : Error(`options.bandwidthProfile.video.${prop}`, typeOrValues);
  }, validateRenderDimensions(video.renderDimensions));
}

function validateDimensions(prop, dimensions) {
  if (typeof dimensions === 'undefined') {
    return null;
  }
  if (dimensions === null || !isNonArrayObject(dimensions)) {
    return E.INVALID_TYPE(`options.bandwidthProfile.video.renderDimensions.${prop}`, 'object');
  }
  return ['height', 'width'].reduce((error, dimension) => {
    if (error || !(dimension in dimensions)) {
      return error;
    }
    return typeof dimensions[dimension] === 'number' ? null
      : E.INVALID_TYPE(`options.bandwidthProfile.video.renderDimensions.${prop}.${dimension}`, 'number');
  }, null);
}

function validateRenderDimensions(renderDimensions) {
  if (typeof renderDimensions === 'undefined') {
    return null;
  }
  if (renderDimensions === null || !isNonArrayObject(renderDimensions)) {
    return E.INVALID_TYPE('options.bandwidthProfile.video.renderDimensions', 'object');
  }
  return ['dominantSpeaker'].concat(Object.values(trackPriority)).reduce((error, prop) =>
    error || validateDimensions(prop, renderDimensions[prop]), null);
}
