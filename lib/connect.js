...
let didPrintSafariOrEdgeWarning = false;
...
function connect(token, options) {
  ...
  const WS_SERVER = options._useTwilioConnection
    ? constants.WS_SERVER_TWILIOCONNECTION
    : constants.WS_SERVER;

  /* eslint new-cap:0 */
  options = Object.assign({
    wsServer: WS_SERVER(options.environment, options.realm)
  }, options);
  ...
  if (guessBrowser() === 'safari'
    && !didPrintSafariOrEdgeWarning
    && (log.logLevel !== 'error' && log.logLevel !== 'off')) {
    didPrintSafariOrEdgeWarning = true;
    log.warn([
  ...
  if (guessBrowser() === 'edge'
    && !didPrintSafariOrEdgeWarning
    && (log.logLevel !== 'error' && log.logLevel !== 'off')) {
    didPrintSafariOrEdgeWarning = true;
    log.warn([
      'This release of twilio-video.js includes an early experimental support for',
      'Edge build 10547 and newer. Please test this release',
      'and report any issues to https://github.com/twilio/twilio-video.js'
    ].join(' '));
  }
