...
function connect(token, options) {
  ...
  logConnectEvent(options, eventObserver);
...
function logConnectEvent(connectOptions, eventObserver) {
  ...
  const payload = {
    audio: boolToString(!!connectOptions.audio),
    audioTracks: (connectOptions.tracks || []).filter(track => track.kind === 'audio').length,
    automaticSubscription: boolToString(connectOptions.automaticSubscription),
    dataTracks: (connectOptions.tracks || []).filter(track => track.kind === 'data').length,
    enableDominantSpeaker: boolToString(connectOptions.dominantSpeaker),
    enableDscp: boolToString(connectOptions.enableDscp),
    eventListener: boolToString(!!connectOptions.eventListener),
    iceServers: (connectOptions.iceServers || []).length,
    iceTransportPolicy: connectOptions.iceTransportPolicy,
    preferredAudioCodecs: JSON.stringify(connectOptions.preferredAudioCodecs),
    preferredVideoCodecs: JSON.stringify(connectOptions.preferredVideoCodecs),
    preflight: boolToString(!!connectOptions.preflight),
    region: connectOptions.region,
    video: boolToString(!!connectOptions.video),
    videoTracks: (connectOptions.tracks || []).filter(track => track.kind === 'video').length,
    networkQualityConfiguration: JSON.stringify(connectOptions.networkQuality)
  };
  ...
  if (connectOptions.bandwidthProfile) {
    ...
    if (connectOptions.bandwidthProfileOptions.renderDimensions) {
      payload.bandwidthProfileOptions.renderDimensions = JSON.stringify(connectOptions.bandwidthProfileOptions.renderDimensions);
    }
  }
