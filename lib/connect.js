...
const { getSignalingEndpoint } = require('./util/endpoints');
...
function connect(token, options) {
  ...
  /* eslint new-cap:0 */
  const wsServer = getSignalingEndpoint(options.environment, options.region);
...
function createRoomSignaling(token, options, signaling, encodingParameters, preferredCodecs, localParticipant) {
  ...
  const roomSignalingParams = { token };
