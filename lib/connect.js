...
const InsightsPublisher = require('./util/insightspublisher/index');
...
function connect(token, options) {
  ...
  eventObserver.setPublisher(eventPublisher);
  options = Object.assign({ eventObserver, eventPublisher, wsServer }, options);
  ...
  logConnectEvent(options, eventPublisher);
...
function logConnectEvent(connectOptions, eventObserver) {
  // "audio": { "type": "boolean" },
  // "audioTracks": { "type": "integer" },
  // "automaticSubscription": { "type": "boolean" },
  // "dataTracks": { "type": "integer" },
  // "enableIceGatheringOnAnyAddressPorts": { "type": "boolean" },
  // "enableDominantSpeaker": { "type": "boolean" },
  // "enableDscp": { "type": "boolean" },
  // "eventListener": { "type": "boolean" },
  // "iceServers": { "type": "integer" },
  // "iceTransportPolicy": { "type": "string" },
  // "maxVideoBitrate": { "type": "integer" },
  // "maxAudioBitrate": { "type": "integer" },
  // "roomName": { "type": "string" },
  // "networkPrivacyPolicy": { "type": "string" },
  // "preferredAudioCodecs": { "type": "array" },
  // "preferredVideoCodecs": { "type": "array" },
  // "preferredNetworkTypes": { "type": "array" },
  // "preflight": { "type": "boolean" },
  // "region": { "type": "string" },
  // "video": { "type": "boolean" },
  // "videoTracks": { "type": "integer" },
  ...
  //   "properties": {
  //     "mode": { "type": "string" },
  //     "trackSwitchOffMode": { "type": "string" },
  //     "dominantSpeakerPriority": { "type": "string" },
  //     "maxSubscriptionBitrate": { "type": "integer" },
  //     "maxTracks": { "type": "integer" },
  //     "renderDimensions": {
  //       "properties": {
  //         "high": { "$ref": "#/definitions/renderDimensions" },
  //         "low": { "$ref": "#/definitions/renderDimensions" },
  //         "standard": { "$ref": "#/definitions/renderDimensions" }
  //       }
  //     }
  //   }
  // }
  ...
  const payload = {
    ...
    automaticSubscription: connectOptions.automaticSubscription,
    dataTracks: (connectOptions.tracks || []).filter(track => track.kind === 'data').length,
    // enableIceGatheringOnAnyAddressPorts:
    enableDominantSpeaker: connectOptions.dominantSpeaker,
    enableDscp: connectOptions.enableDscp,
    eventListener: !!connectOptions.eventListener,
    iceServers: (connectOptions.iceServers || []).length,
    iceTransportPolicy: connectOptions.iceTransportPolicy,
    preferredAudioCodecs: connectOptions.preferredAudioCodecs,
    preferredVideoCodecs: connectOptions.preferredVideoCodecs,
    preflight: !!connectOptions.preflight,
    ...
    networkQualityConfiguration: connectOptions.networkQuality
  };

  if (connectOptions.maxVideoBitrate) {
    payload.maxVideoBitrate = connectOptions.maxVideoBitrate;
  }

  if (connectOptions.bandwidthProfile) {
    payload.bandwidthProfileOptions = connectOptions.bandwidthProfile;
  }

  if (connectOptions.maxAudioBitrate) {
    payload.maxAudioBitrate = connectOptions.maxAudioBitrate;
  }

  if (connectOptions.name) {
    payload.roomName = connectOptions.name;
  }
...
const deprecatedConnectOptionsProps = new Set([
  { didWarn: false, name: 'abortOnIceServersTimeout' },
  { didWarn: false, name: 'dscpTagging', newName: 'enableDscp' },
  { didWarn: false, name: 'iceServersTimeout' }
]);

function removeDeprecatedConnectOptionsPropsWithWarning(options, log) {
  deprecatedConnectOptionsProps.forEach(prop => {
    const { didWarn, name, newName } = prop;
    if (name in options) {
      if (newName) {
      ...
      if (!didWarn && !['error', 'off'].includes(log.level)) {
        log.warn(`The ConnectOptions flag "${name}" is ${newName
