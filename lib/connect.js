...
function validateBandwidthProfile(bandwidthProfile) {
  ...
  error = error || validateObject(bandwidthProfile.video, 'options.bandwidthProfile.video', [
  ...
  return bandwidthProfile.video
    ? error || validateRenderDimensions(bandwidthProfile.video.renderDimensions)
    : error;
}

function validateRenderDimensions(renderDimensions) {
  const name = 'options.bandwidthProfile.video.renderDimensions';
  const props = ['dominantSpeaker'].concat(Object.values(trackPriority));
  let error = validateObject(renderDimensions, name);
  return renderDimensions ? error || props.reduce((error, prop) => {
