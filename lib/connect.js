...
function connect(token, options) {
  ...
  var preferredCodecs = {
    ...
    video: options.preferredVideoCodecs.map(function(codec) {
      codec = typeof codec === 'string' ? { codec: codec } : codec;
      if (codec.codec.toLowerCase() === 'vp8' && !('simulcast' in codec)) {
        codec.simulcast = false;
      }
      return codec;
    })
  };
 ...
 *   enabled, Twilio's Video SDK will send three partial layers
 */
