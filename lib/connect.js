 ...
 *
 * @example
 * // Access a logger for each individual components
 * var { Logger, connect } = require('twilio-video');
 * var token = getAccessToken();
 *
 * var loggerName = 'twilio-video';
 * var defaultLogger = Logger.getLogger(loggerName + '-default');
 * var signalingLogger = Logger.getLogger(loggerName + '-signaling');
 * var mediaLogger = Logger.getLogger(loggerName + '-media');
 * var webrtcLogger = Logger.getLogger(loggerName + '-webrtc');
 *
 * // Listen for logs to your selected component
 * var originalFactory = defaultLogger.methodFactory;
 * defaultLogger.methodFactory = function (methodName, logLevel, loggerName) {
 *   var method = originalFactory(methodName, logLevel, loggerName);
 *
 *   return function (datetime, logLevel, component, message, data) {
 *     method(datetime, logLevel, component, message, data);
 ...
 *
 * // Specify different levels for each individual components
 * connect(token, {
 *   name: 'my-cool-room',
 *   logLevel: {
 *     default: 'debug',
...
function connect(token, options) {
  ...
  let log;
  try {
    log = new options.Log('default', logComponentName, logLevels, options.loggerName);
  } catch (error) {
 ...
 * @property {LogLevel|LogLevels} [logLevel='warn'] - Set the default log verbosity
 *   of logging. Passing a {@link LogLevel} string will use the same
 *   level for all components. Pass a {@link LogLevels} to set specific log
 *   levels.
 * @property {string} [loggerName='twilio-video'] - The name of the logger. Use this name when accessing the logger
 *   used by the SDK. If you provided a {@link LogLevels} object with different levels via the <code>logLevel</code>
 *   parameter, you can access the loggers used by the SDK using the naming format <code>loggerName + '-' + component</code>.
