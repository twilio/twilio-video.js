 ...
 * @property {string} [mediaRegion=gll] - Allows the Participant to choose the region that is used
 *   for media processing in a Group Room. In order for the property to be honored the Participant
 *   must be the first to connect, and the Room must be either Ad-Hoc with the account's default
 *   media region set to "gll", or created with the "gll" media region via the REST API.
 *   Requests to select a media region are ignored for Peer-to-Peer Rooms, because they do not use a central media server.
