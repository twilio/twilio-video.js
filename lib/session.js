...
function Session(creator, _participants, options) {
  ...
  if (options['invite']) {
    this.invite(_participants, options);
  }
 ...
 * @instance
 * @param {string|Participant} [filter] - a {@link Participant} or
 *   {@link Participant} address to filter by
 * @returns {Array<Stream>}
 */
Session.prototype.getRemoteStreams = function getRemoteStreams(filter) {
  var self = this;
  var remoteStreams = [];
 ...
 * @returns {Array<Stream>}
 */
Session.prototype.getLocalStreams = function getLocalStreams(filter) {
  var self = this;
  var localStreams = [];
 ...
 */
Session.prototype.invite = function invite(address, options) {
  var self = this;

  if (!('stream' in options)) {
    var streamConstraints = (options && 'streamConstraints' in options)
                          ? options['streamConstraints']
                          : null;
    return Stream.getUserMedia(streamConstraints).then(function(stream) {
      options = options || {};
      options['stream'] = stream;
      return self.invite(address, options);
    });
  }
  ...
  var endpointInvitations = invitees.endpoints
    .reduce(function(endpointInvitations, endpoint) {
      ...
      endpoints.forEach(function(caller) {
        ...
        var invitation = caller._userAgent.invite(self, callee, options);
