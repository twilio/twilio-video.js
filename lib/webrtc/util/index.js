 ...
 * @returns {undefined}
 */
const delegateMethod = (source, wrapper, target, methodName) => {
 ...
 * @returns {undefined}
 */
const delegateMethods = (source, wrapper, target) => {
 ...
 * @returns Array<*>
 */
const flatMap = (list, mapFn) => {
  let listArray = list instanceof Map || list instanceof Set
 ...
 */
const guessBrowserVersion = (userAgent) => {
  if (typeof userAgent === 'undefined') {
    userAgent = getUserAgent();
  }
  const prefix = {
  ...
  const match = (userAgent.match(regex) || [])[2];
 ...
 * @returns {void}
 */
const interceptEvent = (target, type) => {
 ...
 */
const legacyPromise = (promise, onSuccess, onFailure) => {
  if (onSuccess) {
 ...
 * @returns {undefined}
 */
const proxyProperties = (source, wrapper, target) => {
 ...
 * @returns {undefined}
 */
const proxyProperty = (source, wrapper, target, propertyName) => {
