...
// enough for this bug.
const needsWorkaroundForBug1480277 = typeof navigator === 'object'
  && navigator.userAgent
...
//
const FirefoxRTCPeerConnection = (configuration) => {
  ...
  peerConnection.addEventListener('signalingstatechange', function onsignalingstatechange() {
    if (!self._rollingBack && self.signalingState !== previousSignalingState) {
      ...
      const dispatchEventToSelf = self.dispatchEvent.apply.bind(self.dispatchEvent, self, arguments);
      if (self._isClosed) {
...
if (needsWorkaroundForBug1480277) {
  FirefoxRTCPeerConnection.prototype.addTrack = function addTrack() {
    ...
    let sender = this._peerConnection.addTrack.apply(this._peerConnection, arguments);
...
FirefoxRTCPeerConnection.prototype.createOffer = function createOffer() {
  const args = [].slice.call(arguments);
  const options = (args.length > 1 ? args[2] : args[0]) || {};
  let promise;
  let self = this;
...
const rollback = (peerConnection, local, onceRolledBack) => {
 ...
 * @returns {undefined}
 */
const saveInitiallyNegotiatedDtlsRole = (peerConnection, description, remote) => {
 ...
 */
const overwriteWithInitiallyNegotiatedDtlsRole = (description, dtlsRole) => {
