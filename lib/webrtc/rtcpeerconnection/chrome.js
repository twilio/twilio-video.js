...
const EventTarget = require('../util/eventtarget');
...
const isIOSChrome = require('../util').isIOSChrome;
...
//
const ChromeRTCPeerConnection = (configuration, constraints) => {
  ...
  let newConfiguration = Object.assign(configuration.iceTransportPolicy
  ...
  let sdpFormat = sdpUtils.getSdpFormat(newConfiguration.sdpSemantics);
  let peerConnection = new RTCPeerConnection(newConfiguration, constraints);
...
if (typeof RTCPeerConnection.prototype.addTrack !== 'function') {
  ...
  ChromeRTCPeerConnection.prototype.addTrack = function addTrack() {
    ...
    const sender = this._senders.get(track);
...
ChromeRTCPeerConnection.prototype.createOffer = function createOffer() {
  let args = [].slice.call(arguments);
  let options = (args.length > 1 ? args[2] : args[0]) || {};
  let self = this;
  ...
  let promise = this._peerConnection.createOffer(options).then(function(offer) {
...
ChromeRTCPeerConnection.prototype.setLocalDescription = function setLocalDescription() {
  ...
  let promise = setDescription(this, true, description);
  return args.length > 1
    ? util.legacyPromise(promise, args[1], args[2])
    : promise;
};
...
ChromeRTCPeerConnection.prototype.setRemoteDescription = function setRemoteDescription() {
  ...
  let promise = setDescription(this, false, description);
  return args.length > 1
    ? util.legacyPromise(promise, args[1], args[2])
    : promise;
};
 ...
 * @returns {boolean}
 */
const needsMaxPacketLifeTimeShim = () => {
 ...
 */
const shimDataChannelInit = (dataChannelDict) => {
 ...
 */
const updateTrackIdsToSSRCs = (sdpFormat, tracksToSSRCs, sdp) => {
