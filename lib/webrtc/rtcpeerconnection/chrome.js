...
class ChromeRTCPeerConnection extends EventTarget {
  constructor(configuration = {}, options = {}) {
    ...
    const constraints = options.chromeSpecificConstraints;
    ...
    Object.defineProperties(this, {
      ...
      _applyCodecPreferences: {
        value: options.applyCodecPreferences || (() => {})
      },
      _audioTransceiver: {
        value: null,
        writable: true
  ...
  setLocalDescription(description) {
    if (description.type === 'rollback') {
      // NOTE(lrivas): Chrome/Safari allow setLocalDescription(rollback) in 'have-remote-offer'
      // but Firefox throws. We throw here for Firefox consistency.
  ...
  setRemoteDescription(description) {
    // NOTE(lrivas): Chrome/Safari allow setRemoteDescription(rollback) in 'have-local-offer'
    // but Firefox throws. We throw here for Firefox consistency.
    if (description.type === 'rollback' && this.signalingState === 'have-local-offer') {
      return Promise.reject(new DOMException('Failed to execute '
        + '\'setRemoteDescription\' on \'RTCPeerConnection\': '
        + 'Called in wrong signalingState: '
        + this.signalingState, 'InvalidStateError'));
    }
