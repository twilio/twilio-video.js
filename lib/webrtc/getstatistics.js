    ...
    switch (name) {
      ...
      case 'googLocalAddress':
        if (value.split) {
          ...
          setProp(transportStats, 'local', 'address', value[0]);
          setProp(transportStats, 'local', 'port', Number(value[1]));
        }
        break;
      ...
      case 'googRemoteAddress':
        if (value.split) {
          var address = value.split('.');
          setProp(transportStats, 'remote', 'address', value[0]);
          setProp(transportStats, 'remote', 'port', Number(value[1]));
        }
        break;
