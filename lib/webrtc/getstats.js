...
const { flatMap, guessBrowser, guessBrowserVersion, getSdpFormat } = require('./util');
...
function standardizeFirefoxActiveIceCandidatePairStats(stats) {
  ...
  const standatdizedLocalCandidateStatsReport = activeLocalCandidateStats
    ? standardizedLocalCandidateStatsKeys.reduce((report, { ffKeys, key, type }) => {
      const key1 = ffKeys && ffKeys.find(key => key in activeLocalCandidateStats) || key;
      report[key] = typeof activeLocalCandidateStats[key1] === type
        ? key1 === 'candidateType'
          ? candidateTypes[activeLocalCandidateStats[key1]] || activeLocalCandidateStats[key1]
          : activeLocalCandidateStats[key1]
        : key1 === 'deleted' ? false : null;
      return report;
    }, {})
    : null;
  ...
  const standardizedRemoteCandidateStatsReport = activeRemoteCandidateStats
    ? standardizedCandidateStatsKeys.reduce((report, { ffKeys, key, type }) => {
      const key1 = ffKeys && ffKeys.find(key => key in activeRemoteCandidateStats) || key;
      report[key] = typeof activeRemoteCandidateStats[key1] === type
        ? key1 === 'candidateType'
          ? candidateTypes[activeRemoteCandidateStats[key1]] || activeRemoteCandidateStats[key1]
          : activeRemoteCandidateStats[key1]
        : null;
      return report;
    }, {})
    : null;
