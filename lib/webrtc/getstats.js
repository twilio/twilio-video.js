...
const guessBrowserVersion = require('./util').guessBrowserVersion;
const getSdpFormat = require('./util/sdp').getSdpFormat;
 ...
 */
const getStats = (peerConnection, options) => {
 ...
 * @returns {Promise.<StandardizedStatsResponse>}
 */
const _getStats = (peerConnection, options) => {
  ...
  const trackStatsPromises = flatMap([
    ...
    const tracks = triple[0];
    const statsArrayName = triple[1];
    const isRemote = triple[2];
 ...
 */
const getActiveIceCandidatePairStats = (peerConnection, options) => {
  options = options || {};
 ...
 */
const standardizeChromeOrSafariActiveIceCandidatePairStats = (stats) => {
  const activeCandidatePairStats = Array.from(stats.values()).find(function(stat) {
  ...
  let standatdizedLocalCandidateStatsReport = activeLocalCandidateStats
  ...
  let standardizedRemoteCandidateStatsReport = activeRemoteCandidateStats
 ...
 * @returns {?StandardizedActiveIceCandidatePairStatsReport}
 */
const standardizeFirefoxActiveIceCandidatePairStats = (stats) => {
  const activeCandidatePairStats = Array.from(stats.values()).find(function(stat) {
    return stat.type === 'candidate-pair' && stat.nominated;
  });
  ...
  let standatdizedLocalCandidateStatsReport = activeLocalCandidateStats
    ? standardizedLocalCandidateStatsKeys.reduce(function(report, keyInfo) {
      const key = keyInfo.ffKeys && keyInfo.ffKeys.find(function(key) {
 ...
 */
const getTracks = (peerConnection, kind, localOrRemote) => {
  let getSendersOrReceivers = localOrRemote === 'local' ? 'getSenders' : 'getReceivers';
  ...
  let getStreams = localOrRemote === 'local' ? 'getLocalStreams' : 'getRemoteStreams';
  return flatMap(peerConnection[getStreams](), function(stream) {
    let getTracks = kind === 'audio' ? 'getAudioTracks' : 'getVideoTracks';
 ...
 */
const getTrackStats = (peerConnection, track, options) => {
  options = options || {};
 ...
 * @returns {Promise.<Array<StandardizedTrackStatsReport>>}
 */
const chromeOrSafariGetTrackStats = (peerConnection, track) => {
 ...
 * @returns {Promise.<Array<StandardizedTrackStatsReport>>}
 */
const firefoxGetTrackStats = (peerConnection, track, isRemote) => {
  return new Promise(function(resolve, reject) {
    peerConnection.getStats(track).then(function(response) {
 ...
 */
const standardizeChromeLegacyStats = (response, track) => {
  const ssrcReport = response.result().find(function(report) {
 ...
 */
const standardizeChromeOrSafariStats = (response) => {
  let inbound = null;
  ...
  let isRemote = track && track.remoteSource;
  let mainSources = isRemote ? [inbound] : outbound;
  let stats = [];
  let remoteSource = isRemote ? remoteOutbound : remoteInbound; // remote rtp stats
  ...
  mainSources.forEach(function(source) {
    ...
    const mimeType = getStatValue('mimeType');
    ...
    const audioLevel = getStatValue('audioLevel');
 ...
 * @returns {StandardizedTrackStatsReport}
 */
const standardizeFirefoxStats = (response, isRemote) => {
  ...
  let first = isRemote ? inbound : outbound;
  let second = isRemote ? outbound : inbound;
