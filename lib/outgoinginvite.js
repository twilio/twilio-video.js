 ...
 *   to a {@link Room} if one or more {@link Participant}s accept the
 *   corresponding {@link IncomingInvite}. An {@link OutgoingInvite} may be
 *   canceled up until a {@link Participant} has accepted it.
 *   <br><br>
 *   {@link OutgoingInvite}s are returned by {@link Client#inviteToRoom}.
 * @extends Promise<Room>
 ...
 */
function OutgoingInvite(identities, getLocalMedia, createOutgoingInviteSignaling, createRoom) {
  if (!(this instanceof OutgoingInvite)) {
    return new OutgoingInvite(identities, getLocalMedia, createOutgoingInviteSignaling, createRoom);
  }
  ...
  function cancel(signaling) {
    if (signaling) {
      ...
      // Just in case it was already accepted, disconnect the RoomSignaling,
      // should it ever resolve.
      signaling.getRoomSignaling().then(function connectSucceeded(roomSignaling) {
  ...
  getLocalMedia(function getLocalMediaSucceeded(localMedia) {
    ...
    return createOutgoingInviteSignaling(localMedia).then(function outgoingInviteSignalingCreated(signaling) {
      ...
      return signaling.getRoomSignaling();
    });
  }).then(function outgoingInviteSignalingAccepted(roomSignaling) {
