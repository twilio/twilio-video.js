...
function runPreflightTest(publisherToken, subscriberToken, options, preflightTest) {
  ...
  function generateWarnings(report) {
    const WARNING_THRESHOLDS = {
      ...
      packetLoss: { max: 1 },
      rtt: { max: 400 },
      outgoingBitrate: { min: 400 },
      incomingBitrate: { min: 400 },
    };

    // extract stats to check
    const stats = {};
    Object.keys(WARNING_THRESHOLDS).forEach(key => {
      if (report.stats[key] && typeof report.stats[key].average === 'number') {
        stats[key] = report.stats[key].average;
      }
    });

    Object.entries(WARNING_THRESHOLDS).forEach(([key, thresholds]) => {
      if (typeof stats[key] === 'number') {
        if (typeof thresholds.max === 'number' && stats[key] >= thresholds.max) {
          preflightTest.emit(`${key}_too_high`, `${key} value was too high (${stats[key]}). max = ${thresholds.max}`);
        }

        if (typeof thresholds.min === 'number' && stats[key] <= thresholds.min) {
          preflightTest.emit('warning', `${key}_too_`, `${key} value was too low (${stats[key]}). min = ${thresholds.max}`);
        }
      }
    });
  }
