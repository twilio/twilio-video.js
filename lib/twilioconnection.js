...
// TODO(mmalavalli): Uncomment this line once VSG accepts TCMP version.
...
class TwilioConnection extends StateMachine {
  ...
  _close({ code, reason }) {
    ...
    if (code !== WS_CLOSE_BUSY_WAIT) {
      this.transition('closed', null, code !== WS_CLOSE_NORMAL
        ? [new Error(`WebSocket Error ${code}: ${reason}`), code, reason]
        : []);
    }
  ...
  _handleBusy({ cookie, keepAlive, retryAfter }) {
    ...
    // TODO(mmalavalli): Remove check for the existence of "cookie" once VSG supports it.
    if (cookie) {
      this._cookie = cookie;
    }
    const log = this._log;
