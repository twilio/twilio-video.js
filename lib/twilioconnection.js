...
const DEFAULT_MAX_CONSECUTIVE_MISSED_HEARTBEATS = 1;
const DEFAULT_MAX_REQUESTED_HEARTBEAT_TIMEOUT = 15000;
const DEFAULT_OPEN_TIMEOUT = 15000;
const DEFAULT_WELCOME_TIMEOUT = 15000;
...
const WS_CLOSE_FAILED_TO_OPEN = 3005;
...
class TwilioConnection extends StateMachine {
  ...
  _connect(serverUrl) {
    ...
    this._openTimeout = new this._options.Timeout(() => {
      const reason = `Failed to open in ${openTimeout} ms`;
      this._close({ code: WS_CLOSE_FAILED_TO_OPEN, reason });
    }, openTimeout);
    ...
    ws.addEventListener('open', () => {
      ...
      this._welcomeTimeout = new this._options.Timeout(() => this._handleWelcomeTimeout(), welcomeTimeout);
      if (this._networkMonitor) {
  ...
  _handleWelcome({ negotiatedTimeout }) {
    ...
    // The offset added to the incoming heartbeat timeout should be removed.
    const heartbeatTimeout = negotiatedTimeout;

    // Outgoing heartbeat should be sent 200 ms earlier than the specified interval to account for network latency.
    const outgoingHeartbeatTimeout = negotiatedTimeout - OUTGOING_HEARTBEAT_OFFSET;

    this._welcomeTimeout.clear();
    this._heartbeatTimeout = new this._options.Timeout(() => this._handleHeartbeatTimeout(), heartbeatTimeout);
    this._messageQueue.splice(0).forEach(message => this._send(message));
    this._sendHeartbeatTimeout = new this._options.Timeout(() => this._sendHeartbeat(), outgoingHeartbeatTimeout);
 ...
 * @property {number} [requestedHeartbeatTimeout=15000] - "heartbeat" timeout (ms) requested by the {@link TwilioConnection}
 * @property {number} [welcomeTimeout=15000] - Time (ms) to wait for the "welcome" message after sending the "hello" message
 */
