...
class TwilioConnection extends StateMachine {
  ...
  _connect(serverUrl) {
    ...
    ws.addEventListener('open', () => {
      ...
      setTimeout(() => this._handleWelcomeTimeout(), welcomeTimeout);
    });
  }
  ...
  _handleWelcome({ negotiatedTimeout }) {
    ...
    this._heartbeatTimeout = {
      reset: () => {
        clearTimeout(this._heartbeatTimeout.timeout);
        this._heartbeatTimeout.timeout = setTimeout(() => {
          this._handleHeartbeatTimeout();
        }, negotiatedTimeout + HEARTBEAT_TIMEOUT_OFFSET);
      },
      timeout: null
    };
    ...
    this._heartbeatTimeout.reset();
    this._sendHeartbeat(negotiatedTimeout);
  ...
  /**
   * Send "heartbeat" message and schedule the next one.
   * @param {number} timeout
   * @private
   */
  _sendHeartbeat(timeout) {
    if (this.state === 'closed') {
      return;
    }
    this._send({ type: 'heartbeat' });
    setTimeout(() => this._sendHeartbeat(timeout), timeout);
  }
