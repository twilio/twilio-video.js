 ...
 * @param {function(IncomingInvite.AcceptOptions, function(LocalMedia): Promise<ConversationImpl>): Promise<ConversationImpl>} getLocalMedia
 * @param {function(IncomingInvite.AcceptOptions, ConversationImpl): Conversation} createConversation
 ...
 */
function IncomingInvite(impl, getLocalMedia, createConversation) {
  if (!(this instanceof IncomingInvite)) {
    return new IncomingInvite(impl, getLocalMedia, createConversation);
  }
...
IncomingInvite.prototype.accept = function accept(options) {
  ...
  return this._getLocalMedia(options, function getLocalMediaSucceeded(localMedia) {
    return self._impl.accept(localMedia, options);
  }).then(function acceptSucceeded(conversationImpl) {
    return self._createConversation(options, conversationImpl);
  });
};
