...
function LocalMedia(options) {
  ...
  options.log = options.log || new Log('media', this, Log.buildLevels(options.logLevel));
...
LocalMedia.getLocalMedia = function getLocalMedia(options) {
  ...
  log.info('MediaStreamConstraints:', options);

  var localMediaPromise = getUserMedia({
    ...
    log.info('GetUserMedia successful; got MediaStream:', mediaStream);
    log.info('Adding MediaStream to LocalMedia');
    return localMedia.addStream(mediaStream);
  });
  localMediaPromise.catch(function(error) {
    log.warn('Error while getting local MediaStream:', error);
  });
...
LocalMedia.prototype.setLogLevel = function setLogLevel(logLevel) {
  this._log.setLevels(Log.buildLevels(logLevel));
  return this;
};
...
LocalMedia.prototype.addMicrophone = function addMicrophone() {
  ...
  var microphonePromise = getUserMedia({ audio: true, video: false })
  ...
  microphonePromise.catch(function(error) {
...
LocalMedia.prototype.addCamera = function addCamera() {
  ...
  var cameraPromise = getUserMedia({ audio: false, video: true })
  ...
  cameraPromise.catch(function(error) {
