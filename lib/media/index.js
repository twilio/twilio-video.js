 ...
 * @instance
 * @param {HTMLElement} [div] - the &lt;div&gt; to attach to
 * @returns {HTMLElement}
 */
  ...
  if (el && this.attachments.has(el)) {
    ...
    this.on('trackAdded', function trackAdded(track) {
      if (!self.attachments.has(el)) {
        return self.removeListener('trackAdded', this);
      }
 ...
 * HTMLElements.
 * @instance
 * @param {HTMLElement} [div] - the &lt;div&gt; to detach from
 * @returns {Media}
 */
  ...
  [this.audioTracks, this.videoTracks].forEach(function(tracks) {
    tracks.forEach(function(_track) {
      if (_track.mediaStream === track.mediaStream) {
        ...
        if (_track.label === 'microphone') {
          this.addMicrophone();
        } else if (_track.label === 'camera') {
          this.addCamera();
        }
      }
    }, this);
