...
const silentTrackWorkaround = require('../../webaudio/workaround180748');
  ...
  return class LocalMediaTrack extends AudioOrVideoTrack {
    ...
    constructor(mediaStreamTrack, options) {
      ...
      Object.defineProperties(this, {
        ...
        _silentTrackWorkaround: {
          value: options.silentTrackWorkaround
        },
...
function restartWhenInadvertentlyStopped(localMediaTrack) {
  const { _getUserMedia: getUserMedia, _silentTrackWorkaround: silentTrackWorkaround, _log: log, kind } = localMediaTrack;
  ...
  function reacquireTrack() {
    ...
    return silentTrackWorkaround(log, getUserMedia, constraints).then(mediaStream => {
  ...
  globalEvents.onVisiblePhase1(handleTrackStateChange);
  mediaStreamTrack.addEventListener('ended', handleTrackStateChange);
  return () => {
    globalEvents.offVisiblePhase1(handleTrackStateChange);
    mediaStreamTrack.removeEventListener('ended', handleTrackStateChange);
  };
}
