...
const globalEvents = require('../../util/globalevents.js');
...
function restartWhenInadvertentlyStopped(localMediaTrack) {
  const { _getUserMedia: getUserMedia, _log: log, _instanceId: instanceId, kind } = localMediaTrack;
  let mediaStreamTrack = localMediaTrack.mediaStreamTrack;
  let trackChangeInProgress = false;

  function shouldReacquireTrack() {
    const { _workaroundWebKitBug1208516Cleanup, isStopped, mediaStreamTrack: { muted, readyState } } = localMediaTrack;
    const isInadvertentlyStopped = isStopped && !!_workaroundWebKitBug1208516Cleanup;
    console.log(`makarand:shouldReacquireTrack:<Local${kind}:${instanceId}> muted=${muted}, readyState=${readyState}, visibilityState=${document.visibilityState}, isInadvertentlyStopped=${isInadvertentlyStopped}`);
  ...
  function handleTrackStateChange(cause) {
    console.log(`makarand:${cause}:<Local${kind}:${instanceId}> handleTrackStateChange`);
    return Promise.race([
      waitForEvent(mediaStreamTrack, 'unmute'),
      waitForSometime(50)
    ]).then(() => {
      console.log(`makarand:${cause}:<Local${kind}:${instanceId}> shouldReacquireTrack`, shouldReacquireTrack());
      if (shouldReacquireTrack()) {
        trackChangeInProgress = true;
        return reacquireTrack().catch(err => {
          console.log(`makarand:${cause}:<Local${kind}:${instanceId}> Failed to re-acquire`, err);
          log.warn('Failed to re-acquire the MediaStreamTrack:', err);
          throw err;
        }).then(newTrack => {
          console.log(`makarand:${cause}:<Local${kind}:${instanceId}> Re-acquired`, newTrack.id);
          ...
          console.log(`makarand:${cause}:<Local${kind}:${instanceId}> _setMediaStreamTrack failed`, err);
        }).finally(() => {
          // reattach listener on updated mediaStream
          console.log(`makarand:${cause}:<Local${kind}:${instanceId}> reattaching ended listener`, mediaStreamTrack.id, localMediaTrack.mediaStreamTrack.id);
          mediaStreamTrack.removeEventListener('ended', onStreamEnded);
          mediaStreamTrack = localMediaTrack.mediaStreamTrack;
          mediaStreamTrack.addEventListener('ended', onStreamEnded);
  ...
  function onStreamEnded() {
    handleTrackStateChange('onStreamEnded');
  }

  function onDocumentVisible() {
    return handleTrackStateChange('onDocumentVisible');
  }


  globalEvents.onVisiblePhase1(onDocumentVisible);
  mediaStreamTrack.addEventListener('ended', onStreamEnded);
  return () => {
    globalEvents.offVisiblePhase1(onDocumentVisible);
    mediaStreamTrack.removeEventListener('ended', onStreamEnded);
  };
}
