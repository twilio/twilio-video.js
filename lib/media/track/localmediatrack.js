...
function mixinLocalMediaTrack(AudioOrVideoTrack) {
  ...
  return class LocalMediaTrack extends AudioOrVideoTrack {
    ...
    _restart(constraints) {
      ...
      if (this._trackChangeInProgress) {
        log.warn('Waiting for the already in-progress re-acquring of the MediaStreamTrack to complete.');
        return this._trackChangeInProgress;
      }

      this._trackChangeInProgress = this._reacquireTrack(constraints).catch(error => {
        ...
        this._trackChangeInProgress = null;
      });
...
function restartWhenInadvertentlyStopped(localMediaTrack) {
  ...
  function shouldReacquireTrack() {
    const {
      _workaroundWebKitBug1208516Cleanup,
      _trackChangeInProgress: trackChangeInProgress,
      isStopped
    } = localMediaTrack;
  ...
  function maybeRestart() {
    return Promise.race([
      ...
      if (shouldReacquire && !localMediaTrack._trackChangeInProgress) {
      ...
      const promise = localMediaTrack._trackChangeInProgress || Promise.resolve();
