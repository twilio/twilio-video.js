...
function restartWhenInadvertentlyStopped(localMediaTrack) {
  ...
  function checkSilence() {
    ...
    log.debug('Begin checking silence');
  ...
  function shouldReacquireTrack() {
    ...
    log.debug(`isInadvertentlyStopped: ${isInadvertentlyStopped}`);
    log.debug(`document.visibilityState: ${document.visibilityState}`);
    log.debug(`muted: ${muted}`);
    log.debug(`trackChangeInProgress: ${!!trackChangeInProgress}`);
  ...
  function maybeRestart() {
    log.debug('Begin for unmute or 50ms');
    return Promise.race([
