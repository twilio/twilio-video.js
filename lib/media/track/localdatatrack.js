 ...
 * {@link LocalParticipant} can publish to a {@link Room}. {@link LocalDataTrack}
 * is currently not supported in Edge and an error is thrown when an attempt is
 * made to create an instance.
 * @extends Track
...
class LocalDataTrack extends Track {
  ...
  constructor(options) {
    ...
    // NOTE(syerrapragada): otalk/peerconnectionshim does not support RTCDataChannels. So
    // DataTracks are not supported in Edge for now.
    if (options.guessBrowser() === 'edge') {
      throw new Error('DataTracks are not supported in Edge');
    }
