...
class LocalAudioTrack extends LocalMediaAudioTrack {
  ...
  _acquireTrack(constraints) {
    const mediaStreamPromise = this._getUserMedia({ audio: constraints, video: false });
  ...
  _handleTrackStateChange() {
    if (this._shouldAttemptRestart()) {
      ...
      setTimeout(() => {
        ...
        if (this._shouldAttemptRestart()) {
          // replace tracks.
          const constraints = this.mediaStreamTrack.getConstraints();
          this._acquireTrack(constraints).then(newTrack => {
            this._log.debug('replacing track with constrains ', newTrack.id, constraints);
            this._setMediaStreamTrack(newTrack).then(() => {
