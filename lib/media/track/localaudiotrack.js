...
class LocalAudioTrack extends LocalMediaAudioTrack {
  ...
  constructor(mediaStreamTrack, options) {
    ...
    const { _log: log } = this;
    let curDefaultDeviceInfo = { deviceId: '', groupId: '', label: '' };
      ...
      _onDeviceChange: {
        value: () => {
          navigator.mediaDevices.enumerateDevices().then(deviceInfos => {
            ...
            if (defaultDeviceInfo && ['deviceId', 'groupId'].some(prop => defaultDeviceInfo[prop] !== curDefaultDeviceInfo[prop])) {
              log.info('Default device changed, restarting the LocalAudioTrack');
              log.debug(`Old default device: "${curDefaultDeviceInfo.deviceId}" => "${curDefaultDeviceInfo.label}"`);
              log.debug(`New default device: "${defaultDeviceInfo.deviceId}" => "${defaultDeviceInfo.label}"`);
              curDefaultDeviceInfo = defaultDeviceInfo;
