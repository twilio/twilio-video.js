...
class LocalAudioTrack extends LocalMediaAudioTrack {
  ...
  *//**
   ...
   */
  stop() {
    // track was stopped, cleanup our 'restart when ended' workaround.
    if (this._cleanupWebKitBug1208516) {
      this._cleanupWebKitBug1208516();
      this._cleanupWebKitBug1208516 = null;
    }
...
function restartWhenInadvertentlyStopped(localAudioTrack) {
  ...
  function handleTrackStateChange() {
    ...
    waitForUnmuteOrTimeout.then(() => {
      ...
      acquireTrack(constraints).then(newTrack => {
        ...
        // stop watching old track and start watching updated track.
        startWatching();
      }).catch(err => {
  ...
  function startWatching() {
    ...
    // start new watch.
    if (track) {
      log.debug('watching for track getting ended', track.id);
      ...
      // TODO: consider using: localAudioTrack.on('stopped', handleTrackStateChange);

      // create a function to clean up localAudioTrack watch.
      stopWatching = () => {
        log.debug('cleaning up watch for track:', track.id);
