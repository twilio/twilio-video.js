...
class LocalAudioTrack extends LocalMediaAudioTrack {
  ...
  constructor(mediaStreamTrack, options) {
    const processor = options?.audio?.processor || null;
    if (processor) {
      delete options.audio.processor;
    }
  ...
  *//**
  ...
  _maybeProcessTrack(track) {
    if (this._processor) {
      this._log.debug('_maybeProcessTrack: ', track);
      return this._processor.process(track).then(processedTrack => {
        this._log.debug('_maybeProcessTrack done: ', processedTrack);
        return processedTrack;
      }).catch(er => {
        this._log.error('_maybeProcessTrack: Error Processing track: ', er);
        return track;
      });
    }
    return track;
  ...
  _reacquireTrack(constraints) {
    this._log.debug('LocalAudioTrack:_reacquireTrack: ', constraints);

    // disconnect the processor.
    if (this._processor) {
      this._processor.disconnect();
    }
    return super._reacquireTrack.call(this, constraints).then(track => {
