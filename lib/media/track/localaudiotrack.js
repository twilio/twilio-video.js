...
class LocalAudioTrack extends LocalMediaAudioTrack {
  ...
  constructor(mediaStreamTrack, options) {
    ...
    const { _log: log } = this;
    let currentDefaultDeviceInfo = { deviceId: '', groupId: '', label: '' };
      ...
      _onDeviceChange: {
        value: () => {
          navigator.mediaDevices.enumerateDevices().then(deviceInfos => {
            const defaultDeviceInfo = deviceInfos.find(({ kind }) => kind === 'audioinput');
            if (defaultDeviceInfo && ['deviceId', 'groupId'].some(prop => defaultDeviceInfo[prop] !== currentDefaultDeviceInfo[prop])) {
              log.info('Default device changed, restarting the LocalAudioTrack');
              log.debug(`Old default device: "${currentDefaultDeviceInfo.deviceId}" => "${currentDefaultDeviceInfo.label}"`);
              log.debug(`New default device: "${defaultDeviceInfo.deviceId}" => "${defaultDeviceInfo.label}"`);
              currentDefaultDeviceInfo = defaultDeviceInfo;
              this.restart().catch(error => log.warn(`Failed to restart: ${error.message}`));
            }
          }, error => {
  ...
  *//**
   ...
   */
  restart() {
    return super.restart.apply(this, arguments).then(() => {
      this._maybeRestartOnDefaultDeviceChange();
      this._log.info('Restart completed');
    });
  }
