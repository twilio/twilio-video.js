...
class LocalAudioTrack extends LocalMediaAudioTrack {
  ...
  constructor(mediaStreamTrack, options) {
    // Note(mpatwardhan): by default we will workaround for WebKitBug1208516 on Safari browsers. Although the
    // but shows up mainly on iOS devices, we do not have a reliable way to tell iOS from MacOs
    // useragent pretends its macOs if Safari is set to request desktop pages.
  ...
  _startWatching(track) {
    ...
    if (track) {
      ...
      // we want to re-acquire track if document becomes visible (and while in background
      // the track was ended)
      ...
      // the page does not receive visibilitychange, but still the audio track goes into ended state.
