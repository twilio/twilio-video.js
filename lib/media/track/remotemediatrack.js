...
function playIfPausedWhileInBackground(audioTrack) {
  ...
  function getCurrentlyPlayingElements() {
    return audioTrack._attachments.values().filter(el => !!el.paused);
  }

  function continuePlaying(elements) {
    elements.forEach(el => {
      if (audioTrack._attachments.has(el)) {
        el.play();
      }
    });
  }
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    if (wasVisible) {
      // when document goes from visible => invisible, record currently playing elements.
      wasPlaying = getCurrentlyPlayingElements();
    } else {
      // when document goes from invisible => visible, restore playing elements.
