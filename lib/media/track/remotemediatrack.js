...
function playIfPausedWhileInBackground(remoteMediaTrack) {
  ...
  // Note(mpatwardhan): listen for document visibility callback on phase2.
  // this ensures that any LocalMediaTrack (which listen on phase1) get executed
  ...
  documentVisibilityMonitor.onVisible(onVisibilityChanged, 'phase2');
  return () => {
    documentVisibilityMonitor.offVisiblePhase2(onVisibilityChanged, 'phase2');
  };
}
