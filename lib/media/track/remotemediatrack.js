  ...
  return class RemoteMediaTrack extends AudioOrVideoTrack {
    ...
    detach(el) {
      ...
      if (this._workaroundWebKitBug212780) {
        ...
        elements.forEach(element => restoreShim(element));
      }
...
const shimmedElements = new Map();
function restoreShim(mediaElement) {
...
function shimMediaElement(mediaElement) {
  ...
  const wasPausedByUser = () => pausedIntentionally;
  shimmedElements.set(mediaElement, { unShim, wasPausedByUser });
}
  ...
  function onVisibilityChanged() {
    if (document.visibilityState === 'visible') {
      ...
      [...remoteMediaTrack._attachments.values()].forEach(el => {
        ...
        if (el.paused && shim && !shim.wasPausedByUser()) {
          el.play().then(() => {
