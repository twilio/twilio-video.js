...
function mixinRemoteMediaTrack(AudioOrVideoTrack) {
  ...
  return class RemoteMediaTrack extends AudioOrVideoTrack {
    ...
    attach(el) {
      ...
      if (this._workaroundWebKitBug212780 && !this._workaroundWebKitBug212780Cleanup) {
        this._workaroundWebKitBug212780Cleanup = playIfPausedWhileInBackground(this);
      }
      return result;
    }
...
function playIfPausedWhileInBackground(audioTrack) {
  const { _log: log } = audioTrack;
  let elementsPlaying = [];
  function onVisibilityChanged() {
    const isVisible = document.visibilityState === 'visible';
    if (!isVisible) {
      // when document goes to background, remember currently playing elements.
      elementsPlaying = [...audioTrack._attachments.values()].filter(el => !el.paused);
    } else {
      // when document regains foreground restore playing elements.
      elementsPlaying.forEach(el => {
        if (audioTrack._attachments.has(el) && el.paused) {
          try {
            log.info('playing paused element');
            el.play();
          } catch (err) {
            log.debug('error playing paused element', err);
          }
        }
      });
    }
