...
function playIfPausedWhileInBackground(audioTrack) {
  // returns true if any of the attached media elements are paused.
  function isAnyElementPaused() {
    console.log('makarand: finding paused:');
    let foundPaused = false;
    audioTrack._attachments.forEach(el => {
      if (el.paused) {
        foundPaused = true;
      }
    });
    console.log('makarand: found paused:', foundPaused);
    return foundPaused;
  }

  function playAll() {
    console.log('makarand: playing them');
    audioTrack._attachments.forEach(el => el.play());
  }

  let wasVisible = document.visibilityState === 'visible';
  let wasPaused = isAnyElementPaused();
  document.addEventListener('visibilitychange', () => {
    const isVisible = document.visibilityState === 'visible';
    let isPaused = isAnyElementPaused();
    console.log(`makarand: Visible = ${wasVisible} => ${isVisible}, Paused: ${wasPaused} => ${isPaused} `);

    // if elements were paused while the app was in background, unpause those elements.
    if (isVisible && !wasVisible && isPaused && !wasPaused) {
      console.log('will play elements');
      playAll();
      isPaused = false;
    }
