...
function mixinRemoteMediaTrack(AudioOrVideoTrack) {
  ...
  return class RemoteMediaTrack extends AudioOrVideoTrack {
    ...
    constructor(sid, mediaTrackReceiver, isEnabled, setPriority, options) {
      ...
      console.log(` makarand: creating RemoteMediaTrack: <${this.kind}>:`);
    }
...
function playIfPausedWhileInBackground(remoteMediaTrack) {
  const { _log: log, kind, _instanceId: instanceId } = remoteMediaTrack;
  function onVisibilityChanged() {
    // NOTE(mpatwardhan): On iOS Safari, play inadvertently paused HTMLAudioElements
    remoteMediaTrack._attachments.forEach(el => {
      ...
      console.log(`makarand:<Remote${kind}:${instanceId}> Processing element. isInadvertentlyPaused: ${isInadvertentlyPaused}`, el);
      if (isInadvertentlyPaused) {
        ...
        console.log(`makarand:<Remote${kind}:${instanceId}> Playing inadvertently paused element`);
        el.play().then(() => {
          console.log(`makarand:<Remote${kind}:${instanceId}> Successfully played inadvertently paused element`);
          log.info(`Successfully played inadvertently paused <${kind}> element`);
          log.debug('Element:', el);
          log.debug('RemoteMediaTrack:', remoteMediaTrack);
        }).catch(err => {
          console.log(`makarand:<Remote${kind}:${instanceId}> Error while playing inadvertently paused element:`, err, el, remoteMediaTrack);
  ...
  globalEvents.onVisiblePhase2(onVisibilityChanged);
  return () => {
    globalEvents.offVisiblePhase2(onVisibilityChanged);
  };
}
