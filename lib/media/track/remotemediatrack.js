...
function playIfPausedWhileInBackground(remoteMediaTrack) {
  ...
  function onVisibilityChanged() {
    ...
    remoteMediaTrack._attachments.forEach(el => {
      ...
      const pausedIntentionally = shim && !shim.isPausedIntentionally();
      console.log(`makarand: Processing Remove <${kind}> element. paused: ${el.paused}, pausedIntentionally: ${pausedIntentionally}`, el);
      if (el.paused && pausedIntentionally) {
        log.info(`Playing inadvertently paused <${kind}> element`);
        log.debug('Element:', el);
        log.debug('RemoteMediaTrack:', remoteMediaTrack);
        console.log(`makarand Playing inadvertently paused <${kind}> element`);
        el.play().then(() => {
          console.log(`makarand Successfully played inadvertently paused <${kind}> element`);
          log.info(`Successfully played inadvertently paused <${kind}> element`);
          log.debug('Element:', el);
          log.debug('RemoteMediaTrack:', remoteMediaTrack);
        }).catch(err => {
          console.log(`makarand: Error while playing inadvertently paused <${kind}> element:`, err, el, remoteMediaTrack);
