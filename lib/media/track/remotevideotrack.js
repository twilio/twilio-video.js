...
class RemoteVideoTrack extends RemoteMediaVideoTrack {
  ...
  _start(dummyEl) {
    ...
    // NOTE(mpatwardhan): after emitting started, update render hints only if subscribedTrackSwitchOffMode is 'auto'.
...
function maybeUpdateRenderHints(removeVideoTrack) {
  ...
  if (removeVideoTrack._subscribedTrackSwitchOffMode !== 'auto' && removeVideoTrack._contentPreferencesMode !== 'auto') {
    return;
  }
