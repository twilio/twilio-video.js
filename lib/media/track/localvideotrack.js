/* globals MediaStreamTrackGenerator, MediaStreamTrackProcessor */

'use strict';

const { transfer, wrap } = require('comlink');
...
class LocalVideoTrack extends LocalMediaVideoTrack {
  ...
  constructor(mediaStreamTrack, options) {
    ...
    const mstGenerator = options.isCreatedByCreateLocalTracks ? (() => {
      ...
      const worker = wrap(new Worker(`${assetsPath}twilio-video-worker.js`));

      const image = new Image();
      image.src = backgroundImage;
      image.onload = () => createImageBitmap(image)
        .then(backgroundBitmap => {
          const { readable } = processor;
          const { writable } = generator;
          worker.virtualBackground(
            transfer(readable, [readable]),
            transfer(writable, [writable]),
            assetsPath,
            transfer(backgroundBitmap, [backgroundBitmap]),
            maskBlurRadius,
            shouldDeferInputResize,
            shouldDebounce
          );
