...
class VideoTrack extends MediaTrack {
  ...
  constructor(mediaTrackTransceiver, options) {
    ...
    super('video', mediaTrackTransceiver, options);
    ...
    Object.defineProperties(this, {
      ...
      _processorEventObserver: {
        value: new options.VideoProcessorEventObserver(this._log),
        writable: true,
      },
      _unmuteHandler: {
        value: null,
  ...
  _captureFrames() {
    ...
    this._dummyEl.play().then(() => {
      const captureFrame = cb => {
        ...
        const { frameRate = DEFAULT_FRAME_RATE } = this.mediaStreamTrack
  ...
  addProcessor(processor) {
    ...
    const { width = 0, height = 0, frameRate = DEFAULT_FRAME_RATE } = this.mediaStreamTrack
      ? this.mediaStreamTrack.getSettings()
      : {};
