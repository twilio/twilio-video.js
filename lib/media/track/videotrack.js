  ...
  _captureFrames() {
    ...
    this._dummyEl.play().then(() => {
      const captureFrame = cb => {
        ...
        const { frameRate = 24 } = this.mediaStreamTrack.getSettings();
      ...
      const process = () => {
        ...
        ((result instanceof Promise) ? result : Promise.resolve(result))
          .then(outputFrame => {
            if (outputFrame && this._outputFrame) {
              this._outputFrame.width = width;
              this._outputFrame.height = height;
              this._outputFrame.getContext('2d').drawImage(outputFrame, 0, 0, width, height);
  ...
  addProcessor(processor) {
    ...
    if (!this._unmuteHandler) {
      this._unmuteHandler = () => {
        ...
        // On certain scenarios where mediaStreamTrack is coming from muted to unmuted state,
