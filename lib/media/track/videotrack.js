/* globals MediaStreamTrackGenerator, MediaStreamTrackProcessor, TransformStream, VideoFrame */
...
class VideoTrack extends MediaTrack {
  ...
  _captureFrames() {
    ...
    const { _benchmark: b } = this.processor;

    const process = async inputVideoFrame => {
      b.end('transformCallDelay');
      b.start('checkIfCanCaptureFramesDelay');
      ...
      b.end('checkIfCanCaptureFramesDelay');

      b.start('initCanvasDimensionsDelay');
      ...
      b.end('initCanvasDimensionsDelay');

      let outputVideoFrame = inputVideoFrame;
      let result = null;
      try {
        ...
        b.start('createOutputFrameDelay');
        if (this._outputFrame && result) {
          outputVideoFrame = new VideoFrame(this._outputFrame, {
            timestamp: inputVideoFrame.timestamp
          });
          inputVideoFrame.close();
        }
        b.end('createOutputFrameDelay');
        this._processorEventObserver.emit('stats');
        b.start('transformCallDelay');
      }
    ...
    const transformer = new TransformStream({
      transform: async (inputVideoFrame, outputController) => {
        const outputVideoFrame = await process(inputVideoFrame);
        outputController.enqueue(outputVideoFrame);
      }
    });
    ...
    this._dummyEl.play().then(
      () => this._mstProcessor
        ...
        .pipeTo(this._mstGenerator.writable)
    ).catch(
      error => this._log.error('Video element cannot be played', { error, track: this })
    );
  }
  ...
  addProcessor(processor, options) {
    ...
    this._mstGenerator = new MediaStreamTrackGenerator({ kind: 'video' });
    this._mstProcessor = new MediaStreamTrackProcessor({ track: this.mediaStreamTrack });
    this.processedTrack = new MediaStream([this._mstGenerator]).getVideoTracks()[0];
  ...
  *//**
  ...
  removeProcessor(processor) {
    ...
    this._mstProcessor.readable.cancel();
    this._mstProcessor = null;
    this._mstGenerator = null;
    this._updateElementsMediaStreamTrack();
    return this;
  }
}
