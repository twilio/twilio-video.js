...
class VideoTrack extends MediaTrack {
  ...
  _captureFrames() {
    ...
    this._dummyEl.play().then(() => {
      ...
      const process = () => {
        ...
        let result = null;
        try {
          const input = this._processorOptions.inputType === 'video' ? this._dummyEl : this._inputFrame;
  ...
  _restartProcessor() {
    ...
    if (processor) {
      const processorOptions = { ...this._processorOptions };
  ...
  addProcessor(processor, options) {
    ...
    let { inputType, renderingContextType } = this._processorOptions;
    if (typeof OffscreenCanvas === 'undefined' && inputType === 'offscreencanvas') {
      throw new Error('OffscreenCanvas is not supported by this browser.');
    }
    if (inputType && inputType !== 'video' && inputType !== 'canvas' && inputType !== 'offscreencanvas') {
      throw new Error(`Invalid inputType of ${inputType}`);
    }
    if (!inputType) {
      inputType = typeof OffscreenCanvas === 'undefined' ? 'canvas' : 'offscreencanvas';
    }

    const { width = 0, height = 0, frameRate = DEFAULT_FRAME_RATE } = this.mediaStreamTrack.getSettings();
    if (inputType === 'offscreencanvas') {
    ...
    const ctx = this._outputFrame.getContext(renderingContextType || '2d');
    if (!ctx) {
      throw new Error(`Cannot get renderingContextType: ${renderingContextType}.`);
    }
    ...
    const targetFps = CanvasCaptureMediaStreamTrack && CanvasCaptureMediaStreamTrack.prototype &&
      typeof CanvasCaptureMediaStreamTrack.prototype.requestFrame === 'function' ? undefined : 0;
 ...
 * @property {string} [inputType="offscreencanvas"] - The type of input the processor will receive.
 * The default is <code>offscreencanvas</code> and will fallback to a regular <code>canvas</code> if the browser does not support it.
 * Possible values include <code>video</code> for a video element input, <code>canvas</code>, and <code>offscreencanvas</code>.
 * @property {string} [renderingContextType="2d"] - The type of rendering <a href="https://developer.mozilla.org/en-US/docs/Web/API/HTMLCanvasElement/getContext" target="_blank">context</a>
 * to use when drawing to the output canvas. Since canvas only allows one context to use, make sure the context type you provide matches
 * the one you will use or else calling <a href="https://developer.mozilla.org/en-US/docs/Web/API/HTMLCanvasElement/getContext" target="_blank">canvas.getContext()</a> will return a `null` value.
 */
