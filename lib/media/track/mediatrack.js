...
const Track = require('./');
const { hasDocumentPiP } = require('../../services/documentPictureInPicture');
...
class MediaTrack extends Track {
  ...
  attach(el) {
    ...
    if (!this._elShims.has(el)) {
      const onUnintentionallyPaused = () => {
        if (this._playPausedElementsIfNotBackgrounded || hasDocumentPiP()) {
