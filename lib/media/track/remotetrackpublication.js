...
class RemoteTrackPublication extends TrackPublication {
  ...
  constructor(signaling, options) {
    ...
    signaling.on('trackswitchedon', () => {
      if (this.track) {
        this.track._setSwitched(signaling.isSwitchedOff);
      }
      // mpatwardhan_fix_before_merge: is this event necessary?
      // this.emit(signaling.isSwitchedOff ? 'trackSwitcheOff' : 'trackSwitchedOn');
    });

    signaling.on('trackswitchedoff', () => {
      if (this.track) {
        this.track._setSwitched(signaling.isSwitchedOff);
      }
      // mpatwardhan_fix_before_merge: is this event necessary?
