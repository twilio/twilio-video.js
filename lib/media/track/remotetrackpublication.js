...
class RemoteTrackPublication extends TrackPublication {
  ...
  constructor(signaling, options) {
    ...
    signaling.on('updated', () => {
      if (signaling.error) {
        const error = signaling.error;
        const twilioError = createTwilioError(error.code, error.message);
        this.emit('subscriptionFailed', twilioError);
        return;
      }
      if (this.track) {
        this.track.setEnabled(signaling.isEnabled);
      }
