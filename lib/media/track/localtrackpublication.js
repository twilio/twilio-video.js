...
class LocalTrackPublication extends TrackPublication {
  ...
  constructor(trackSid, track, unpublish, options) {
    const isEnabled = typeof track.isEnabled === 'boolean'
      ? track.isEnabled
      : true;

    super(isEnabled, track.name, trackSid, options);

    Object.defineProperties(this, {
      _onTrackEvent: {
        value: () => {
          this._isTrackEnabled = track.isEnabled;
        }
      },
      _unpublish: {
        value: unpublish
    ...
    track.on('disabled', this._onTrackEvent);
    track.on('enabled', this._onTrackEvent);
  }
   ...
   */
  unpublish() {
    this.track.removeListener('disabled', this._onTrackEvent);
    this.track.removeListener('enabled', this._onTrackEvent);
