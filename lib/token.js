...
function Token(capabilityTokenString) {
  ...
  if (supportsOutgoing && !outgoingAppSid) {
    throw new Error('Token supports outgoing but is missing an appSid');
  }
  ...
  if (supportsIncoming && !incomingClientName) {
    throw new Error('Token supports incoming but is missing a clientName');
  }
...
function parseScope(uri) {
    ...
    if (!(parts && parts.length === 4)) {
      throw new Error('Token scope URI is invalid or malformed');
    }
    return {
        service: parts[1],
        privilege: parts[2],
