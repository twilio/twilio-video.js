...
function AccessToken(jwt) {
  ...
  grants.forEach((grant) => {
    ...
    if (match) {
      ...
      this._validateAddress(address, host);
    }
  });
...
AccessToken.prototype._validateAddress = function validateAddresses(address, host) {
  host = host || this.host;

  var invalidAddresses = (address.forEach ? address : [address])
