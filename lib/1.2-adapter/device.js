...
function setupPeer(device, peer) {
  device._peer = peer;

  peer.on('error', device.emit.bind(device, 'error'));

  peer.on('incoming', function(incomingCall) {
    console.log('Device emitting "incoming"');
    device.emit('incoming', new Connection(device, incomingCall));
  });

  peer.on('ready', device.emit.bind(device, 'ready', device));

  peer.on('offline', device.emit.bind(device, 'offline', device));
}
...
Device.prototype.setup = function setup(capabilityToken, options) {
  // Convert between 1.2-style options and new Peer options.
  var peerOptions = {};
  ...
  , ['warnings',         'warnings']
    ...
    var peerOption = pair[1];
    if (deviceOption in options) {
      peerOptions[peerOption] = options[deviceOption];
    }
  });
  ...
  if (!('chunderm' in options)) {
    ...
    if (chunder.match(/dev/) || chunderw.match(/dev/)) {
      peerOptions['wsServer'] = 'chunderm.dev.twilio.com';
    } else if (chunder.match(/stage/) || chunderw.match(/stage/)) {
      peerOptions['wsServer'] = 'chunderm.stage.twilio.com';
    } else {
      peerOptions['wsServer'] = 'chunderm.twilio.com';
    }
  }

  setupPeer(this, new Peer(peerOptions));
  this._peer.auth(capabilityToken);
  return this;
};
...
Device.prototype.connect = function connect() {
  ...
  if (args.length === 1 && typeof args[0] === 'function') {
    ...
    return new Connection(this, this._peer.call.apply(this._peer, args));
  } else {
