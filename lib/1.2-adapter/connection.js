...
var IncomingSession = require('../session').IncomingSession;
 ...
 *   around the new {@link Session}.
 * @param {Device} device - the {@link Device} that owns this {@link
 *   Connection}
 * @param {Session} call - the {@link Session} that this {@link Connection} wraps
 */
function Connection(device, call) {
  ...
  var parameters = {};
  if (call.sid) {
    parameters['SessionSid'] = call.sid;
  }
  if (call.accountSid) {
...
Connection.prototype.accept = function accept() {
  ...
  if (this._call instanceof IncomingSession) {
    this._call.accept();
  } else {
