...
LocalParticipant.prototype._handleTrackSignalingEvents = function _handleTrackSignalingEvents() {
  ...
  Object.defineProperty(this, '_initialTracksPublished', {
    value: tracks.map(function(track) {
      ...
      return self._getOrCreatePublishedTrack(track);
    })
  });
...
LocalParticipant.prototype._getOrCreatePublishedTrack = function _getOrCreatePublishedTrack(localTrack) {
  var log = this._log;
  var self = this;
  ...
  return trackSignaling.getSid().then(function(sid) {
    ...
    if (!publishedTrack) {
      publishedTrack = util.asPublishedTrack(localTrack, sid, unpublish, options);
      self.publishedTracks.set(localTrack.id, publishedTrack);
      publishedKindTracks.set(localTrack.id, publishedTrack);
      log.info('Created a new Published' + util.trackClass(localTrack) + ':', sid);
    }
...
LocalParticipant.prototype.publishTrack = function publishTrack(track) {
  ...
  return this._getOrCreatePublishedTrack(localTrack);
};
