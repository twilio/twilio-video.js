...
var PublishedAudioTrack = require('./media/track/publishedaudiotrack');
var PublishedVideoTrack = require('./media/track/publishedvideotrack');
...
function LocalParticipant(signaling, localTracks, options) {
  ...
  Object.defineProperties(this, {
    ...
    _PublishedAudioTrack: {
      value: options.PublishedAudioTrack
    },
    _PublishedVideoTrack: {
      value: options.PublishedVideoTrack
    },
    _tracksToStop: {
      value: tracksToStop
    },
...
LocalParticipant.prototype._publishTrack = function _publishTrack(track) {
  Participant.prototype._addTrack.call(this, track);
  return this._signaling.tracks.get(track.id);
};
...
LocalParticipant.prototype.publishTrack = function publishTrack(track) {
  ...
  if (this.publishedTracks.has(localTrack.id)) {
    return Promise.resolve(this.publishedTracks.get(localTrack.id));
  }

  var log = this._log;
  var self = this;
  var trackSignaling = this._publishTrack(localTrack);

  return trackSignaling.getSid().then(function(sid) {
    if (!self.tracks.has(localTrack.id)) {
      throw new Error('The LocalTrack [id: ' + localTrack.id
        + '] was unpublished');
    }

    var options = {
      log: log,
      PublishedAudioTrack: self._PublishedAudioTrack,
      PublishedVideoTrack: self._PublishedVideoTrack
    };

    var publishedKindTracks = localTrack.kind === 'audio'
      ? self.publishedAudioTracks
      : self.publishedVideoTracks;

    var publishedTrack = self.publishedTracks.get(localTrack.id);
    if (!publishedTrack) {
      var unpublish = self.unpublishTrack.bind(self);
      publishedTrack = util.asPublishedTrack(track, sid, unpublish, options);
      log.info('Created a new Published' + util.trackClass(track) + ':', sid);
    }

    log.debug('Published' + util.trackClass(track) + ':', publishedTrack);
    self.publishedTracks.set(track.id, publishedTrack);
    publishedKindTracks.set(track.id, publishedTrack);
 ...
 * @returns {?PublishedTrack} - The corresponding {@link PublishedTrack} if
...
LocalParticipant.prototype.unpublishTrack = function unpublishTrack(track) {
  ...
  trackSignaling.publishFailed(new Error('The LocalTrack [id: ' + localTrack.id
    + '] was unpublished'));

  var publishedKindTracks = localTrack.kind === 'audio'
    ? this.publishedAudioTracks
    : this.publishedVideoTracks;
  var publishedTrack = this.publishedTracks.get(localTrack.id) || null;

  publishedKindTracks.delete(localTrack.id);
  this.publishedTracks.delete(localTrack.id);
