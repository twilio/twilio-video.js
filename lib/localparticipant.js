...
class LocalParticipant extends Participant {
  ...
  _handleTrackSignalingEvents() {
    ...
    this._tracks.forEach(track => {
      ...
      this._getOrCreateLocalTrackPublication(track).then(publication => {
        const trackSignaling = this._signaling.getPublication(track._trackSender);
        if (trackSignaling) {
          trackSignaling.on('warning', twilioWarningName => {
            publication.emit('warning', twilioWarningName);
            this.emit('trackWarning', twilioWarningName, track);
          });

          trackSignaling.on('warningsCleared', () => {
            publication.emit('warningsCleared');
            this.emit('trackWarningsCleared', track);
          });
        }
      }).catch(error => {
