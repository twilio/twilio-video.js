...
LocalParticipant.prototype._getOrCreateLocalTrackPublication = function _getOrCreateLocalTrackPublication(localTrack) {
  ...
  var promise = trackSignaling.getSid().then(function(sid) {
  ...
  if (self._signaling.state === 'connected') {
    maybeEmitTrackPublishedEvent(self, promise);
  }
  maybeEmitTrackPublicationFailedEvent(self, localTrack, promise);
 ...
 * @param {Promise<LocalTrackPublication>} promise
 * @returns {void}
 */
function maybeEmitTrackPublishedEvent(participant, promise) {
  promise.then(function(publication) {
    setTimeout(function() {
      participant.emit('trackPublished', publication);
    });
  });
}
 ...
 * @private
 * @param {LocalParticipant} participant
 * @paran {LocalTrack} track
 * @param {Promise<LocalTrackPublication>} promise
 * @returns {void}
 */
function maybeEmitTrackPublicationFailedEvent(participant, track, promise) {
  promise.catch(function(error) {
    setTimeout(function() {
      participant.emit('trackPublicationFailed', error, track);
    });
  });
}
