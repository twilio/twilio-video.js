...
class LocalParticipant extends Participant {
  ...
  _getOrCreateLocalTrackPublication(localTrack) {
    ...
    return new Promise((resolve, reject) => {
      ...
      trackSignaling.on('warning', twilioWarningName => {
        localTrackPublication.emit('warning', twilioWarningName);
        this.emit('trackWarning', twilioWarningName, localTrack);
      });

      trackSignaling.on('warningsCleared', () => {
        localTrackPublication.emit('warningsCleared');
        this.emit('trackWarningsCleared', localTrack);
      });
    });
  }
...
/**
 * One of the {@link LocalParticipant}'s {@link LocalTrack}s encounters a {@link TwilioWarning}.
...
/**
 * One of the {@link LocalParticipant}'s {@link LocalTrack}s cleared all {@link TwilioWarning}s.
