...
class LocalParticipant extends Participant {
  ...
  _addTrack(track, priority) {
    const addedTrack = super._addTrack(track, track._trackSender.id);
  ...
  _addTrackPublication(publication) {
    publication = super._addTrackPublication(publication);
    if (publication) {
   ...
   */
  _removeTrack(track) {
    const removedTrack = super._removeTrack(track, track._trackSender.id);
  ...
  */
  unpublishTrack(track) {
    ...
    let localTrack = this._tracks.get(track.id || track._trackSender.id);
    if (!localTrack) {
      return null;
    }
