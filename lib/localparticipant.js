  ...
  _handleTrackSignalingEvents() {
    ...
    signaling.on('stateChanged', function stateChanged(state) {
      ...
      if (state === 'disconnected') {
        ...
        // NOTE(mmalavalli): The LocalTrackPublicationSignaling instances are
        // removed from the LocalParticipantSignaling so that the TrackSenders
        // clear their clones, so that when we restart a LocalMediaTrack, we do
        // not call replaceTrack() the RTCRtpSenders of stale TrackSender clones.
        self._tracks.forEach(track => {
          localTrackStopped(track);
          self._signaling.removeTrack(track._trackSender);
        });
