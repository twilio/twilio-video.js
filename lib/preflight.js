...
const { isNonArrayObject } = require('./util');
const { PreflightTest } = require('./preflighttest');
 ...
 * connectivity. Ensure that access tokens used for the test contain a unique room name which should be different from the
 * room user intents to join. Test will join the room specified in the token. if room does not exist an ad-hoc room will
 ...
 * @property {number} [duration=5000] - number of milliseconds to run test for.
