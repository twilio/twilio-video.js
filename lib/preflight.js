...
const {
  isNonArrayObject, waitForSometime
} = require('./util');
...
function runPreflightTest(publisherToken, subscriberToken, options, preflightTest) {
  ...
  let subscriberConnectTiming = null;
  let publisherConnectTiming = null;
  let subscriberMediaTiming = null;
  ...
  function generatePreflightReport(subscriberRoom) {
    return subscriberRoom.getStats()
      .then(subscriberStats => {
        ...
        return {
          ...
          networkTiming: {
            connect: subscriberConnectTiming,
            media: subscriberMediaTiming,
          },
          ...subscriberStats
        };
      });
  ...
  createLocalTracks()
    .then(tracks => {
      ...
      publisherConnectTiming = new TimeMeasurement();
      return connect(publisherToken, options);
    }).then(room => {
      publisherConnectTiming.stop();
      publisherRoom = room;
      checkIfStopped();
      subscriberConnectTiming = new TimeMeasurement();
      subscriberMediaTiming = new TimeMeasurement();
      return connect(subscriberToken, Object.assign(options, { name: room.sid }));
    }).then(room => {
      subscriberRoom = room;
      subscriberConnectTiming.stop();
      checkIfStopped();
      ...
      checkIfStopped();
      subscriberMediaTiming.stop();
      preflightTest.emit('progress', PreFlightProgress.mediaStarted);
    }).then(() => {
      checkIfStopped();
      return waitForSometime(testDuration);
    }).then(() => {
      checkIfStopped();
      return generatePreflightReport(subscriberRoom);
    }).then(report => {
 ...
 * @property {number} [duration=5000] - duration in milliseconds to run test.
 *   once connected test will run for this duration and generate stats report.
