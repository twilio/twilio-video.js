...
Participant.prototype._handleTrackSignalingEvents = function _handleTrackSignalingEvents() {
  ...
  function trackSignalingAdded(signaling) {
    signaling.getMediaOrDataStreamTrack().then(function(mediaOrDataStreamTrack) {
      var RemoteTrack = {
      ...
      // MediaStreamTrack or DataStreamTrack kinds disagree; however, just in
      // case, we handle it here.
      if (!RemoteTrack || signaling.kind !== mediaOrDataStreamTrack.kind) {
        return;
      }

      var track = new RemoteTrack(mediaOrDataStreamTrack, signaling, { log: log });
      self._addTrack(track);
    });
  }

  function trackSignalingRemoved(signaling) {
    signaling.getMediaOrDataStreamTrack().then(function() {
