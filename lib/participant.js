...
class Participant extends EventEmitter {
  ...
  constructor(signaling, options) {
    ...
    const log = options.log.createLog(this);
  ...
  _handleTrackSignalingEvents() {
    ...
    function trackSignalingAdded(signaling) {
      const reemitEventsToRemoteParticipant = publication => {
      ...
      const publication = new RemoteTrackPublication(signaling, { log, reemitEventsToRemoteParticipant });
    ...
    function trackSignalingSubscribed(signaling) {
      const { isEnabled, name, kind, sid, trackTransceiver, isSwitchedOff, rspVersion, switchOffReason = null } = signaling;
      const RemoteTrack = {
      ...
      const options = { log, name, clientTrackSwitchOffControl, contentPreferencesMode, shouldDeprecateEnabledState: rspVersion >= 3 };
...
function indexTracksById(tracks) {
  const indexedTracks = tracks.map(track => [track.id || track._trackSender.id, track]);
  ...
  signaling.on('stateChanged', function stateChanged(state) {
    ...
    if (state === 'disconnected') {
      ...
      participant._tracks.forEach(track => {
        ...
        const trackIdentifier = track.sid || track._trackSender.id;
        const reemitters = participant._trackEventReemitters.get(trackIdentifier);
        if (track && reemitters) {
          reemitters.forEach((reemitter, event) => {
            track.removeListener(event, reemitter);
          });
