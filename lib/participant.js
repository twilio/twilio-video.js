...
Participant.prototype._handleTrackSignalingEvents = function _handleTrackSignalingEvents() {
  ...
  function trackSignalingAdded(signaling) {
    ...
    signaling.getTrackTransceiver().then(trackReceiver => {
      ...
      var track = new RemoteTrack(trackReceiver, signaling, { log });
...
function indexTracksById(tracks) {
  var indexedTracks = tracks.map(track => [track.id, track]);
  var indexedAudioTracks = indexedTracks.filter(keyValue => keyValue[1].kind === 'audio');
  var indexedVideoTracks = indexedTracks.filter(keyValue => keyValue[1].kind === 'video');
  var indexedDataTracks = indexedTracks.filter(keyValue => keyValue[1].kind === 'data');
