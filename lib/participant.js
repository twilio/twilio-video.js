...
class Participant extends EventEmitter {
  ...
  _handleTrackSignalingEvents() {
    ...
    function trackSignalingSubscribed(signaling) {
      const { id, isEnabled, name, kind, sid, trackTransceiver } = signaling;
      const RemoteTrack = {
      ...
      const publication = self.trackPublications.get(sid);
      ...
      const track = kind === 'data'
        ? new RemoteTrack(sid, trackTransceiver, { log, name })
        : new RemoteTrack(sid, trackTransceiver, isEnabled, { log, name });

      self._addTrack(track, publication, id);
    }
    ...
    function trackSignalingUnsubscribed(signaling) {
      ...
      const publication = self.trackPublications.get(signaling.sid);
      if (track) {
        self._removeTrack(track, publication, signaling.id);
      }
    }
  ...
  signaling.on('stateChanged', function stateChanged(state) {
    ...
    if (state === 'disconnected') {
      ...
      signaling.tracks.forEach(trackSignaling => {
        const track = participant.tracks.get(trackSignaling.id);
