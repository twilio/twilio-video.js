...
function handleSignalingEvents(room, signaling) {
  ...
  signaling.on('stateChanged', function stateChanged(state) {
    var stateChangedEventArgs = [state, room];
    log.info('Transitioned to state:', state);

    if (state === 'disconnected') {
      if (room._options.shouldStopLocalMedia) {
        log.info('Room disconnected. We now stop any automatically acquired '
          + 'LocalMedia');
        room.localParticipant.media.stop();
      }
      if (signaling._didDisconnectUnexpectedly) {
        var error = new TwE.SIGNALING_CONNECTION_ERROR('Connection error');
        stateChangedEventArgs.push(error);
      }
    }

    room.emit.apply(room, stateChangedEventArgs);
  });
}
