...
const ResourceMonitor = require('./insights/resourcemonitor');
const ApplicationMonitor = require('./insights/applicationmonitor');
const NetworkMonitor = require('./insights/networkmonitor');
    ...
    Object.defineProperties(this, {
      ...
      _insightsMonitors: {
        value: options.eventObserver && options.insights ?
          [
            new NetworkMonitor(options.eventObserver, log),
            new ResourceMonitor(options.eventObserver, log),
            new ApplicationMonitor(options.eventObserver, log)
          ] : null
      },
      dominantSpeaker: {
        enumerable: true,
    ...
    switch (state) {
      case 'disconnected':
        ...
        if (room._insightsMonitors) {
          room._insightsMonitors.forEach(monitor => {
