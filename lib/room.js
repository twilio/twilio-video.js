 ...
 * @property {Map<Participant.Identity, OutgoingInvite>} outgoingInvites - Any
 *   pending {@link OutgoingInvite}s you have sent
 ...
 * @param {Participant.Identity} identity - The identity of the {@link Participant} to add
 ...
 * @param {Array<Participant.Identity>} identities - The identities of
 *   {@link Participant}s to add
 * @returns {Array<OutgoingInvite>}
 * @example
 * var initialToken = getAccessToken();
 * var manager = new Twilio.AccessManager(initialToken);
 * var client = new Twilio.Rooms.Client(manager);
...
Room.prototype.invite = function invite(identityOrIdentities) {
  if (!identityOrIdentities) {
    return Promise.reject(new Error('identityOrIdentities is a required argument'));
  }

  if (Array.isArray(identityOrIdentities)) {
    return identityOrIdentities.map(this.invite, this);
  }
  var identity = identityOrIdentities;
  var self = this;

  // We only create a new OutgoingInvite when a pending OutgoingInvite for the
  // same Participant identity does not already exist.
  var outgoingInvite = this.outgoingInvites.get(identity);
  if (outgoingInvite) {
    return outgoingInvite;
  }

  outgoingInvite = new OutgoingInvite(
    this._signaling.invite(identity),
    function getParticipant(participantSignaling) {
      return self.participants.get(participantSignaling.sid);
    });

  // Add the OutgoingInvite to the Map; remove once it resolves or rejects.
  this._outgoingInvites.set(identity, outgoingInvite);
  outgoingInvite.then(function() {
    self._outgoingInvites.delete(identity);
  }, function() {
    self._outgoingInvites.delete(identity);
  });
