...
Room.prototype.disconnect = function disconnect() {
  this._log.info('Disconnecting from room.');
...
function connectParticipant(room, participantSignaling) {
  ...
  log.info('Created new remote participant: ', participant);
  room._participants.set(participant.sid, participant);
  log.info('New list of room participants: ', room._participants);
  room.emit('participantConnected', participant);
  log.info('"participantConnected" emitted.');

  // Reemit Track events from the Participant.
  log.info('Setting up reemitters for track events.');
  var eventListeners = [
    'trackAdded',
  ...
  participant.once('disconnected', function participantDisconnected() {
    ...
    log.info('"participantDisconnected emitted.');
  });
}
...
function handleSignalingEvents(room, signaling) {
  ...
  log.info('Creating ParticipantSignaling for remote participants.');
  signaling.participants.forEach(connectParticipant.bind(null, room));
  log.info('Setting up ParticipantSignaling creation for future participants.');
  ...
  signaling.on('stateChanged', function stateChanged(state) {
    ...
    log.info('"' + state + '" emitted for room object: ', room);
    if (state === 'disconnected' &&
      room._options.shouldStopLocalMedia) {
      log.info('Room disconnected, stopping local media.');
