 ...
 * @param {(string|ScopedAuthenticationToken)} token - {@link ScopedAuthenticationToken}
 ...
 *   registered
 * @property {ScopedAuthenticationToken} token - the {@link ScopedAuthenticationToken}
...
/**
 * Registers this {@link UserAgent} using its {@link ScopedAuthenticationToken}.
 * @instance
 * @param {ScopedAuthenticationToken} [token] - {@link ScopedAuthenticationToken} (defaults to current {@link ScopedAuthenticationToken})
 * @fires UserAgent#registered
 * @fires UserAgent#registrationFailed
 * @returns {Promise<UserAgent>}
 */
...
UserAgent.prototype.invite = function invite(address, options) {
  ...
  options = util.withDefaults(options, {
    ...
    callTimeout: options && options.callTimeout || this.callTimeout
  });
