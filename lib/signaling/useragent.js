...
function UserAgent(token, options) {
  ...
  var log = new Log('UserAgent', options.logLevel);
 ...
 * @param {String|Token} token - A token string or instance of {@link Token}
 * @returns {Token} A new instance of {@link Token}
 * @throws INVALID_TOKEN
 */
UserAgent.prototype.createToken = function(token) {
  try {
    return new Token(token);
  } catch(e) {
    this._log.throw(E.INVALID_TOKEN, e.message);
  }
};
...
UserAgent.prototype.register = function register(token) {
  token = token ? this.createToken(token) : this.token;
