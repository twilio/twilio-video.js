 ...
 */
function SIPJSInviteServerTransaction(userAgent, from, conversationSid, callSid, session, logLevel) {
  if (!(this instanceof SIPJSInviteServerTransaction)) {
    return new SIPJSInviteServerTransaction(userAgent, from, conversationSid, callSid, session, logLevel);
  }
  InviteServerTransaction.call(this, userAgent, from, conversationSid, callSid, logLevel);
  ...
  stream.then(function(localStream) {
    ...
    self.session.once('accepted', function() {
      ...
      if (!self.session.hasOffer) {
        dialog = new SIPJSDialog(self.userAgent, self.from, self.conversationSid, self.callSid, localStream, remoteStream, iceServers, peerConnection, self.session, null);
        self._accepted = true;
        self._deferred.resolve(dialog);
      } else if (self.session.hasOffer && self.session.hasAnswer) {
        // FIXME(mroberts): Not sure about this.
        remoteStream = new Stream(self.session.getRemoteStreams()[0]);
        dialog = new SIPJSDialog(self.userAgent, self.from, self.conversationSid, self.callSid, localStream, remoteStream, iceServers, peerConnection, self.session, null);
        self._accepted = true;
        self._deferred.resolve(dialog);
      } else if (self.session.hasOffer && !self.session.hasAnswer) {
