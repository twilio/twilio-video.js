...
function SIPJSInviteClientTransaction(userAgent, to, options) {
  ...
  stream.then(function(localStream) {
    ...
    var ignoreTimeout = setTimeout(function() {
      self.cancel();
    ...
    session.once('accepted', function(response) {
      ...
      var dialog = new SIPJSDialog(userAgent, to, conversationSid, callSid, localStream, remoteStream, self.iceServers, peerConnection, session, inviteTo180);
