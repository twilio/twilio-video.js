...
SIPJSUserAgent.prototype.invite = function invite(session, participant, options) {
  var self = this;
  var deferred = Q.defer();

  function getUserMedia() {
    var constraints = options && options['streamConstraints'] || { 'audio': true, 'video': true };
  ...
  var first;
  if (options && 'stream' in options && options['stream']) {
    first = inviteWithStream.bind(null, options['stream']);
  } else {
    first = getUserMedia;
  }

  if (!this._ua.isConnected()) {
    this._ua.once('connected', first);
  } else {
    first();
  }
...
SIPJSUserAgent.prototype.accept = function accept(dialog, options) {
  ...
  if (options && 'stream' in options && options['stream']) {
    acceptWithStream(options['stream']);
  } else {
    Stream.getUserMedia(options && options['streamConstraints'] || null)
...
function setupUAListeners(userAgent, ua) {
  ua.on('invite', function(sipjsSession) {
    ...
    dialog._callSid = request.getHeader('X-Twilio-CallSid');
