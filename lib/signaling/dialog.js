...
var Q = require('q');
var conversationInfo = require('./conversationinfo');
...
function Dialog(userAgent, remote, conversationSid, callSid, localMedia, remoteMedia, peerConnection, participantSid) {
  ...
  localMedia.on('trackMuted', function trackMuted(track) {
    if (self.ended) {
      localMedia.removeListener('trackMuted', trackMuted);
      return;
    }
    self.publish(conversationInfo.trackMuted(self.participantSid, track.id));
  });
  localMedia.on('trackUnmuted', function trackUnmuted(track) {
    if (self.ended) {
      return;
    }
    self.publish(conversationInfo.trackUnmuted(self.participantSid, track.id));
  });
  localMedia.on('trackPaused', function trackPaused(track) {
    if (self.ended) {
      localMedia.removeListener('trackPaused', trackPaused);
      return;
    }
    self.publish(conversationInfo.trackPaused(self.participantSid, track.id));
  });
  localMedia.on('trackUnpaused', function trackUnpaused(track) {
    if (self.ended) {
      localMedia.removeListener('trackUnpaused', trackUnpaused);
      return;
    }
    self.publish(conversationInfo.trackUnpaused(self.participantSid, track.id));
  });
...
Dialog.prototype._publish = function _publish(payload) {
  var self = this;
  var deferred = Q.defer();
  setTimeout(function() {
    deferred.resolve(self);
