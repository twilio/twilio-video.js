'use strict';

var EventEmitter = require('events').EventEmitter;
var inherits = require('util').inherits;
var util = require('../util');

/**
 * Construct a {@link TrackSignaling}.
 * @class
 * @classdesc A {@link Track} implementation
 * @param {Track.ID} id
 * @param {Track.Kind} kind
 * @param {boolean} isEnabled
 * @property {Track.ID} id
 * @property {boolean} isEnabled
 * @property {Track.Kind} kind
 * @property {?MediaStreamTrack|DataStreamTrack} mediaOrDataStreamTrack
 * @property {?Track.SID} sid
 */
function TrackSignaling(id, kind, isEnabled) {
  EventEmitter.call(this);
  var mediaOrDataStreamTrack;
  var sid = null;
  Object.defineProperties(this, {
    _isEnabled: {
      value: isEnabled,
      writable: true
    },
    _mediaOrDataStreamTrack: {
      get: function() {
        return mediaOrDataStreamTrack;
      },
      set: function(_mediaOrDataStreamTrack) {
        mediaOrDataStreamTrack = _mediaOrDataStreamTrack;
        this._mediaOrDataStreamTrackDeferred.resolve(mediaOrDataStreamTrack);
      }
    },
    _mediaOrDataStreamTrackDeferred: {
      value: util.defer()
    },
    _sid: {
      get: function() {
        return sid;
      },
      set: function(_sid) {
        sid = _sid;
        this._sidDeferred.resolve(sid);
      }
    },
    _sidDeferred: {
      value: util.defer()
    },
    id: {
      enumerable: true,
      value: id
    },
    isEnabled: {
      enumerable: true,
      get: function() {
        return this._isEnabled;
      }
    },
    kind: {
      enumerable: true,
      value: kind
    },
    mediaOrDataStreamTrack: {
      enumerable: true,
      get: function() {
        return mediaOrDataStreamTrack;
      }
    },
    sid: {
      enumerable: true,
      get: function() {
        return sid;
      }
    }
  });
}

inherits(TrackSignaling, EventEmitter);

/**
 * Disable the {@link TrackSignaling} if it is not already disabled.
 * @return {this}
 */
TrackSignaling.prototype.disable = function disable() {
  return this.enable(false);
};

/**
 * Enable (or disable) the {@link TrackSignaling} if it is not already enabled
 * (or disabled).
 * @param {boolean} [enabled=true]
 * @return {this}
 */
TrackSignaling.prototype.enable = function enable(enabled) {
  enabled = typeof enabled === 'boolean' ? enabled : true;
  if (this.isEnabled !== enabled) {
    this._isEnabled = enabled;
    this.emit('updated');
  }
  return this;
};

/**
 * Get the MediaStreamTrack or {@link DataStreamTrack} on the
 * {@link TrackSignaling}.
 * @returns {Promise<MediaStreamTrack|DataStreamTrack>}
 */
TrackSignaling.prototype.getMediaOrDataStreamTrack = function getMediaOrDataStreamTrack() {
  return this._mediaOrDataStreamTrackDeferred.promise;
};

/**
 * Get the SID on the {@link TrackSignaling}.
 * @returns {Promise<Track.SID>}
 */
TrackSignaling.prototype.getSid = function getSid() {
  return this._sidDeferred.promise;
};

/**
 * Rejects the SID's deferred promise with the given Error.
 * @param {Error} error
 * @returns {this}
 */
TrackSignaling.prototype.publishFailed = function publishFailed(error) {
  this._sidDeferred.reject(error);
  return this;
};

/**
 * Set the MediaStreamTrack or {@link DataStreamTrack} on the {@link TrackSignaling}.
 * @param {MediaStreamTrack|DataStreamTrack} mediaOrDataStreamTrack
 * @returns {this}
 */
TrackSignaling.prototype.setMediaOrDataStreamTrack = function setMediaOrDataStreamTrack(mediaOrDataStreamTrack) {
  this._mediaOrDataStreamTrack = mediaOrDataStreamTrack;
  return this;
};

/**
 * Set the SID on the {@link TrackSignaling} once.
 * @param {string} sid
 * @returns {this}
 */
TrackSignaling.prototype.setSid = function setSid(sid) {
  if (this._sid === null) {
    this._sid = sid;
    this.emit('updated');
  }
  return this;
};

/**
 * Emitted whenever the {@link TrackSignaling} is updated
 * @event TrackSignaling#updated
 */

module.exports = TrackSignaling;
