...
class PeerConnectionV2 extends StateMachine {
  ...
  _offer() {
    ...
    return Promise.all(this._replaceTrackPromises.values()).then(() => {
      this._peerConnection.getTransceivers().forEach(transceiver => this._applyCodecPreferencesToTransceiver(transceiver));
      ...
      updatedSdp = this._peerConnection.remoteDescription
