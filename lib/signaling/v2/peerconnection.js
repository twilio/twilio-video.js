...
class PeerConnectionV2 extends StateMachine {
  ...
  _setPublisherHint(mediaTrackSender, encodings, _fromQueue) {
    if (!_fromQueue && this._mediaTrackSenderToPublisherHints.has(mediaTrackSender)) {
      // abandon/fail any stale hint associated with the mediaTrackSender.
      const queuedHint = this._mediaTrackSenderToPublisherHints.get(mediaTrackSender);
      queuedHint.deferred.resolve('HINT_ABANDONED');
      this._mediaTrackSenderToPublisherHints.delete(mediaTrackSender);
    }

    const sender = this._rtpSenders.get(mediaTrackSender);
    if (!sender || this._peerConnection.signalingState === 'closed') {
    ...
    encodings.forEach(({ enabled, layer_index: layerIndex }) => {
      this._log.warn(`layer:${layerIndex}, active:${parameters.encodings[layerIndex].active} => ${enabled}`);
  ...
  removeMediaTrackSender(mediaTrackSender) {
    ...
    // clean up any publisher hints associated with this mediaTrackSender.
    if (this._mediaTrackSenderToPublisherHints.has(mediaTrackSender)) {
      // abandon/fail this stale hint
      const queuedHint = this._mediaTrackSenderToPublisherHints.get(mediaTrackSender);
      queuedHint.deferred.resolve('HINT_ABANDONED');
      this._mediaTrackSenderToPublisherHints.delete(mediaTrackSender);
    }
