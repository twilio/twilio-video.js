...
PeerConnectionV2.prototype._setLocalDescription = function _setLocalDescription(description) {
  ...
  var self = this;
  var vp8CodecSettings = this._preferredVideoCodecs.find(function(codecSettings) {
    return codecSettings.codec.toLowerCase() === 'vp8';
  }) || { codec: 'VP8', simulcast: false };

  return Promise.resolve().then(function() {
    if (description.sdp) {
      description.sdp = isChrome && vp8CodecSettings.simulcast
        ? self._setSimulcast(description.sdp)
        : description.sdp;
    }
