...
PeerConnectionV2.prototype._answer = function _answer(offer) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var usernameFragment = getUsernameFragment(offer);
    if (usernameFragment) {
      var candidates = self._remoteCandidates.setUsernameFragment(usernameFragment);
...
PeerConnectionV2.prototype._handleIceCandidateEvent = function _handleIceCandidateEvent(event) {
  // TODO(mroberts): Signal end-of-candidates.
  if (event.candidate) {
    this._localCandidates.push(event.candidate);
  }
  var peerConnectionState = {
    ice: {
      candidates: this._localCandidates,
      revision: this._localCandidatesRevision++,
      usernameFragment: this._localUsernameFragment
    },
    id: this.id
  };
...
PeerConnectionV2.prototype._setLocalDescription = function _setLocalDescription(description) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (description.type !== 'rollback') {
      ...
      self._localUsernameFragment = getUsernameFragment(description);
...
PeerConnectionV2.prototype._updateDescription = function _updateDescription(description) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var usernameFragment = getUsernameFragment(description);
    if (usernameFragment) {
      var candidates = self._remoteCandidates.setUsernameFragment(usernameFragment);
...
function getUsernameFragment(description) {
  if (description.sdp) {
