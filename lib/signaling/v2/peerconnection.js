...
class PeerConnectionV2 extends StateMachine {
  ...
  /**
   * we sometimes replace media stream after publishing the track. In such cases published tracks id won't
   * match the id of the mediaStream track, Fix it here to match original track id.
   * @private
   * @param {RTCSessionDescription} description
   * @return {RTCSessionDescription}
   */
  _mapTrackIdToOriginalTrackId(id) {
    const mediaTrackSender = [...this._rtpSenders.keys()].find(ms => ms.track.id === id);
    return mediaTrackSender ? mediaTrackSender.id : id;
  }
  ...
  _addOrRewriteLocalTrackIds(description) {
    ...
    const midsToTrackIds = new Map(assignedTransceivers.map(({ mid, sender }) => [mid, this._mapTrackIdToOriginalTrackId(sender.track.id)]));
    ...
    const newTrackIdsByKind = new Map(['audio', 'video'].map(kind => [
      kind,
      unassignedTransceivers.filter(({ sender }) => sender.track.kind === kind).map(({ sender }) => this._mapTrackIdToOriginalTrackId(sender.track.id))
    ]));
...
function rewriteLocalTrackId(pcv2, stats) {
  const trackId = pcv2._mapTrackIdToOriginalTrackId(stats.trackId);
...
function rewriteTrackId(pcv2, stats) {
  ...
  const output = Object.assign(stats, { trackId });
  return output;
}
