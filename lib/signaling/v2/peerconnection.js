...
class PeerConnectionV2 extends StateMachine {
  ...
  _setLocalDescription(description) {
    const updatedDescription = description.type !== 'rollback' && this._enableDtxForOpus
      ? new this._RTCSessionDescription({ sdp: enableDtxForOpus(description.sdp), type: description.type })
      : description;

    return this._peerConnection.setLocalDescription(updatedDescription).catch(error => {
