...
class PeerConnectionV2 extends StateMachine {
  ...
  _addOrRewriteLocalTrackIds(description) {
    ...
    const midsToTrackIds = new Map(assignedTransceivers.map(({ mid, sender }) => {
      // we sometimes replaceTracks, ensure that we use the original track id.
      const originalTrackId = [...this._rtpSenders.keys()].find(ms => ms.track.id === sender.track.id).id;
      return [mid, originalTrackId];
    }));
    ...
    const newTrackIdsByKind = new Map(['audio', 'video'].map(kind => [
      kind,
      unassignedTransceivers.filter(({ sender }) => sender.track.kind === kind).map(({ sender }) => {
        // we sometimes replaceTracks, ensure that we use the original track id.
        const originalTrackId = [...this._rtpSenders.keys()].find(ms => ms.track.id === sender.track.id).id;
