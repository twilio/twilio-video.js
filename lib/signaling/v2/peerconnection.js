...
class PeerConnectionV2 extends StateMachine {
  ...
  _answer(offer) {
    return Promise.resolve().then(() => {
      ...
      this._publishMediaWarning({
        message: 'Failed to setRemoteDescription offer',
      ...
      if (errorToThrow instanceof MediaClientLocalDescFailedError) {
        this._publishMediaWarning({
          message: 'Failed to setLocalDescription',
          code: errorToThrow.code,
          error
        });
      }
  ...
  _setLocalDescription(description) {
    ...
    return this._peerConnection.setLocalDescription(description).catch(error => {
      const message = `Calling setLocalDescription with an RTCSessionDescription of type "${description.type}" failed`;
      this._log.warn(message, { sdp: description.sdp, error });

      const errorToThrow = new MediaClientLocalDescFailedError();
      this._publishMediaWarning({ message, code: errorToThrow.code, error, sdp: description.sdp });
      throw errorToThrow;
    }).then(() => {
