 ...
 * @returns {boolean}
 */
function shouldRecycleTransceiver(pcv2, transceiver) {
  const track = transceiver.receiver.track;
  return !pcv2._recycledTransceivers[track.kind].includes(transceiver)
    && !transceiver.stopped
...
function updateRecycledTransceivers(pcv2) {
  ...
  pcv2._peerConnection.getTransceivers().forEach(transceiver => {
    if (shouldRecycleTransceiver(pcv2, transceiver)) {
      const track = transceiver.receiver.track;
