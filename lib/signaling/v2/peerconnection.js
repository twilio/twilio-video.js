...
class PeerConnectionV2 extends StateMachine {
  ...
  /**
   * Rewrites the session decription to circumvent browser bugs.
   * @private
   * @param {_RTCSessionDescription} description
   * @returns {_RTCSessionDescription}
   */
  _rewriteDescription(description) {
    if (isFirefox) {
      ...
      description = new this._RTCSessionDescription({
        sdp: disableRtx(description.sdp),
        type: description.type
      });
    } else {
      description = workaroundIssue8329(description);
    }
    ...
    description.sdp = removeSSRCAttributes(description.sdp, ['mslabel', 'label']);
  ...
  _answer(offer) {
    return Promise.resolve().then(() => {
      ...
      answer = this._rewriteDescription(answer)
      let updatedSdp = answer.sdp
  ...
  _offer() {
    ...
    return Promise.all(this._replaceTrackPromises.values()).then(() => {
      ...
      offer = this._rewriteDescription(offer)
