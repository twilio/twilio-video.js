  ...
  pcv2._peerConnection.getSenders().filter(sender => sender.track).forEach(sender => {
    ...
    if (!isFirefox) {
      if (sender.track.kind === 'audio') {
        ...
        // NOTE(mmalavalli): "priority" is a per-sender property and not a per-encoding-layer property.
        // So, we set the value only on the first encoding layer. Any attempt to set the value on
        // subsequent encoding layers (in the case of simulcast) will result in the Promise returned
        // by RTCRtpSender.setParameters() being rejected.
