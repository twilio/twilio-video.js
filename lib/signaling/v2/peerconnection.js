...
class PeerConnectionV2 extends StateMachine {
  ...
  _addOrUpdateTransceiver(track) {
    const transceiver = this._recycledTransceivers[track.kind].length > 0
      ? this._recycledTransceivers[track.kind].pop()
      : null;
      ...
      if (description.type === 'answer' && isUnifiedPlan) {
        this._peerConnection.getTransceivers().forEach(transceiver => {
          if (shouldRecycleTransceiver(transceiver)) {
            ...
            this._recycledTransceivers[track.kind].push(transceiver);
          }
        });
      }
    }, error => {
