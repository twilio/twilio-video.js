...
PeerConnectionV2.prototype.removeMediaStream = function removeMediaStream(mediaStream) {
  if (this._peerConnection.signalingState !== 'closed') {
    var senders = new Map(this._peerConnection.getSenders().filter(function(sender) {
      return sender.track;
    }).map(function(sender) {
      return [sender.track.id, sender];
    }));
    mediaStream.getTracks().forEach(function(track) {
      if (senders.has(track.id)) {
        this._peerConnection.removeTrack(senders.get(track.id));
      }
    }, this);
  }
};
