...
class PeerConnectionV2 extends StateMachine {
  ...
  constructor(id, encodingParameters, preferredCodecs, options) {
    ...
    // TODO: we should limit the properties that we pass as 1st parameter. its dangerous to pass
    // generic properites like region, as they could be intepreted by future RTCPeerConnections
...
function updateEncodingParameters(pcv2) {
  ...
  pcv2._peerConnection.getSenders().filter(sender => sender.track).forEach(sender => {
    ...
    let networkPriority;
    if (pcv2._dscpTagging === true && sender.track.kind === 'audio') {
