...
class PeerConnectionV2 extends StateMachine {
  ...
  _publishPeerConnectionEvent(group, name, level, payload = {}) {
    if (!name || !level || !group) {
      this._log.warn('unknown state: ', name);
    } else {
      this._eventObserver.emit('event', { level, name, group, payload: Object.assign(payload, { peerConnectionId: this.id }) });
    }
  }

  _publishIceCandidate(candidate, isRemote = false) {
    let cost = 0;
    try {
      const parts = candidate.candidate.split('network-cost ');
      if (parts[1]) {
        cost = parseInt(parts[1], 10);
      }
    } catch (e) {
      this._log.warn('Error finding network-cost for ice candidate: ', candidate.candidate);
    }

    this._publishPeerConnectionEvent('ice-candidate', 'ice-candidate', 'debug', {
      // required
      transportId: candidate.sdpMid,
      isRemote: isRemote,
  ...
  _handleIceCandidateEvent(event) {
    if (event.candidate) {
      this._publishIceCandidate(event.candidate, false);
  ...
  _handleIceConnectionStateChange() {
    ...
    this._publishPeerConnectionEvent('ice-connection-state', iceConnectionState, level);
  ...
  _handleIceGatheringStateChange() {
    ...
    this._publishPeerConnectionEvent('ice-gathering-state', iceGatheringState, level);
  ...
  _handleSignalingStateChange() {
    this._publishPeerConnectionEvent('pc-signaling-state', this._peerConnection.signalingState, 'debug');
