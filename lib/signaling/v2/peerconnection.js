...
PeerConnectionV2.prototype._maybeReoffer = function _maybeReoffer(localDescription) {
  ...
  // value > 1.
  if (isFirefox) {
    var senders = this._peerConnection.getSenders().filter(sender => sender.track);
...
PeerConnectionV2.prototype._setLocalDescription = function _setLocalDescription(description) {
  ...
  var self = this;
  var vp8SimulcastRequested = this._preferredVideoCodecs.some(codecSettings => codecSettings.codec.toLowerCase() === 'vp8' && codecSettings.simulcast);
