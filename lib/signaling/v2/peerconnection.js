...
class PeerConnectionV2 extends StateMachine {
  ...
  _addOrUpdateTransceiver(track) {
    ...
    if (transceiver && transceiver.sender) {
      this._log.warn(`Reusing transceiver: ${printTransceiver(transceiver)} for ${track.id}`);
      const oldTrackId = transceiver.sender.track ? transceiver.sender.track.id : null;
      if (oldTrackId) {
        this._log.warn(`Conflict Reusing transceiver: ${transceiver.mid}] ${oldTrackId} => ${track.id}`);
      }
  ...
  _setLocalDescription(description) {
    this._log.warn(`this._setLocalDescription: ${description.type}`);
    return this._peerConnection.setLocalDescription(description).catch(error => {
      ...
      this._log.warn(`done with this._setLocalDescription: ${description.type}`);
  ...
  _setRemoteDescription(description) {
    this._log.warn(`this._setRemoteDescription: ${description.type}`);
    if (description.sdp) {
      if (!this._isRTCRtpSenderParamsSupported) {
    ...
    return Promise.resolve().then(() => {
      ...
      this._log.warn(`done with this._setRemoteDescription: ${description.type}`);
...
function printTransceiver(transceiver) {
  const { mid, currentDirection, direction, receiver, sender } = transceiver;
  const senderTrackId = sender && sender.track && sender.track.id;
  const receiverTrackId = receiver && receiver.track && receiver.track.id;
  return JSON.stringify({ mid, currentDirection, direction, senderTrackId, receiverTrackId });
}
...
function updateRecycledTransceivers(pcv2) {
  ...
  pcv2._peerConnection.getTransceivers().forEach(transceiver => {
    if (shouldRecycleTransceiver(transceiver, pcv2)) {
      const strTransceiver = printTransceiver(transceiver);
      pcv2._log.warn(`recycling Transceiver: ${strTransceiver}`);
      // console.log('recycling Transceiver: ', strTransceiver);
