...
PeerConnectionV2.prototype._handleTrackEvent = function _handleTrackEvent(event) {
  ...
  var mediaTrackReceiver = new MediaTrackReceiver(mediaStreamTrack.id, mediaStreamTrack);
 ...
 * @returns {void}
 */
PeerConnectionV2.prototype.addMediaTrackSender = function addMediaTrackSender(mediaTrackSender, mediaStream) {
  if (this._peerConnection.signalingState === 'closed' || this._mediaSenders.has(mediaTrackSender)) {
    return;
  }
  var sender = this._peerConnection.addTrack(mediaTrackSender.track, mediaStream);
  mediaTrackSender.addSender(sender);
  this._mediaSenders.set(mediaTrackSender, sender);
};
...
PeerConnectionV2.prototype.removeMediaTrackSender = function removeMediaTrackSender(mediaTrackSender) {
  ...
  var sender = this._mediaSenders.get(mediaTrackSender);
  this._peerConnection.removeTrack(sender);
  mediaTrackSender.removeSender(sender);
  this._mediaSenders.delete(mediaTrackSender);
};
