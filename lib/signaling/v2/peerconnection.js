...
class PeerConnectionV2 extends StateMachine {
  ...
  _addOrRewriteLocalTrackIds(description) {
    ...
    // we sometimes replaceTrack...this function returns (mediaTrackSender.id) for a given local mediaStreamTrack
    const mapTrackIdToOriginalTrackId = id => {
    ...
    const midsToTrackIds = new Map(assignedTransceivers.map(({ mid, sender }) => [mid, mapTrackIdToOriginalTrackId(sender.track.id)]));
