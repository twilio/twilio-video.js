...
class PeerConnectionV2 extends StateMachine {
  ...
  _answer(offer) {
    return Promise.resolve().then(() => {
      ...
      // NOTE(mmalavalli): Applying codec preferences to local answer in order to
      // fix VIDEO-3736. Also, this takes care if scenarios in all browsers where
      // the payload types in the answer are ordered differently from the offer.
      ...
      updatedSdp = removeSSRCAttributes(updatedSdp, ['mslabel', 'label']);
