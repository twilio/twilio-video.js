...
class TwilioConnectionTransport extends StateMachine {
  ...
  get _isReconnecting() {
    ...
    return this.state === 'connected'
      ...
      && this._reconnectAttemptsLeft < maxReconnectAttempts;
  }

  _createTwilioConnection() {
    const { TwilioConnection } = this._options;
    this._twilioConnection = new TwilioConnection(this._wsServer, this._options);
    return this._twilioConnection;
  }

  _resetReconnectAttemptsLeft() {
    const { maxReconnectAttempts } = this._options;
    this._reconnectAttemptsLeft = maxReconnectAttempts;
  }
...
function setupEventListeners(transport) {
  ...
  function reconnect() {
    ...
    // TODO(mmalavalli): Refactor withJitter() to utils.
    setTimeout(startConnect, withJitter(backOffMs, transport._reconnectBackOffJitter));
  }

  function startConnect() {
    const twilioConnection = transport._createTwilioConnection();
      ...
      case 'syncing':
        switch (message.type) {
          ...
          case 'update':
            ...
            return;
          case 'synced':
            transport._resetReconnectAttemptsLeft();
            transport.emit('message', message);
            transport.preempt('connected');
            return;
          case 'disconnected':
