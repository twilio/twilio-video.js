...
class TwilioConnectionTransport extends StateMachine {
  ...
  constructor(name, accessToken, localParticipant, peerConnectionManager, wsServer, options) {
    ...
    Object.defineProperties(this, {
      ...
      _iceServers: {
        value: options.overrideIceServers ? options.overrideIceServers : null,
        writable: true
      },
      ecsOptions: {
        value: ecsOptions
  ...
  _sendIceMessage() {
    // makarand: TODO: implement this once room service implements it.
  ...
  connect() {
    ...
    console.group('makarand', 'connect');
    if (!this._iceDone) {
      console.group('makarand', 'connect !this._iceDone');
      if (this._iceServerSource) {
        // ice servers were specified
        console.group('makarand', 'connect this._iceServerSource');
        Promise.resolve().then(() => this.emit('iced', { iceServers: this._iceServerSource.iceServers }));
      } else {
        console.group('makarand', 'connect !this._iceServerSource');
        ...
        switch (message.type) {
          case 'iced':
            transport._iceServerSource = {
              status: 'success', // TODO: change to 'acquire'.
              iceServers: message.ice_servers
            };
            transport.emit('iced', { iceServers: transport._iceServerSource.iceServers });
            return;
          case 'connected':
