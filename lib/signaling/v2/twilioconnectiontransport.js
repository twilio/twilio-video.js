...
class TwilioConnectionTransport extends StateMachine {
  ...
  _sendConnectOrSyncOrDisconnectMessage() {
    ...
    if (message.type === 'connect') {
      message.ice_servers = this._iceServerSourceStatus; // TODO: check this
...
function setupEventListeners(transport) {
  ...
  function startConnect() {
    ...
    const openedPromise = new Promise(resolve => twilioConnection.once('open', resolve));
    const turnServerPromise = getTurnServers(transport._accessToken, transport.options)
      .then(({ status, iceServers, ttl }) => {
        transport._iceServerSourceStatus = status;
        transport.emit('iced', { status, iceServers, ttl });
      });

    Promise.all([turnServerPromise, openedPromise]).then(() => {
      console.log('makarand got turn and open');

      // TODO: we need to wait before connecting so that
      // peer connection has time to setConfiguration and createOffer
