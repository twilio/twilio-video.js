...
class TwilioConnectionTransport extends StateMachine {
  ...
  constructor(name, accessToken, localParticipant, peerConnectionManager, wsServer, options) {
    ...
    // NOTE(mmalavalli): In order to hide the AccessToken from being available
    // to the application code, we define those member functions that make use
    // of it here.
    Object.defineProperties(this, {
      _createConnectOrSyncOrDisconnectMessage: {
        value: () => createConnectOrSyncOrDisconnectMessage(this, accessToken)
      },
      _createIceMessage: {
        value: () => createIceMessage(accessToken)
      }
    });
 ...
 * @private
 * @param {TwilioConnectionTransport} transport
 * @param {string} accessToken
 * @returns {?object}
 */
function createConnectOrSyncOrDisconnectMessage(transport, accessToken) {
  const {
    _adaptiveSimulcast,
    _automaticSubscription,
    _bandwidthProfile,
  ...
  const message = {
    name: _name,
    participant: _localParticipant.getState(),
    peer_connections: _peerConnectionManager.getStates(),
    type,
    version: RSP_VERSION
  };

  if (message.type === 'connect') {
    message.ice_servers = _iceServersStatus;
    ...
    if (_bandwidthProfile) {
      message.bandwidth_profile = createBandwidthProfilePayload(_bandwidthProfile);
    }
    ...
    message.subscribe = createSubscribePayload(_automaticSubscription);

    const { sdpFormat } = _options;
    if (sdpFormat) {
 ...
 * @private
 */
function createIceMessage(accessToken) {
  return {
