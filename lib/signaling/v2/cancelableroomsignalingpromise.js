...
function createCancelableRoomSignalingPromise(token, wsServer, localParticipant, encodingParameters, preferredCodecs, options) {
  ...
  return new CancelablePromise(function onCreate(resolve, reject, isCanceled) {
    console.group('makarand: CancelablePromise');
    ...
    return new Promise((resolve, reject) => {
      ...
      transportOptions = Object.assign({
        ...
        overrideIceServers: Array.isArray(options.iceServers) ? options.iceServers : null,
      ...
      transport.once('iced', ({ iceServers }) => {
        // eslint-disable-next-line no-console
        console.log('makarand: got iced');
        options.iceServers = iceServers;
        peerConnectionManager.setConfiguration(options); // makarand-todo: we need iceServers here.
        ...
        peerConnectionManager.createAndOffer().then(function createAndOfferSucceeded() {
          console.log('makarand: createAndOfferSucceeded');
          ...
          console.log('makarand: createAndOffer failed', e);
        });
      });

      transport.once('connected', function connected(initialState) {
        console.log('makarand: got connected');
        if (isCanceled()) {
          reject(cancellationError);
          return;
        }
