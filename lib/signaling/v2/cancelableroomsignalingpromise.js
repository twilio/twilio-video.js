...
function createCancelableRoomSignalingPromise(accountSid, token, ua, identities, labelOrSid, localMedia, options) {
  ...
  var extraHeaders = [
    ...
    C.headers.X_TWILIO_PARTICIPANTS + ': ' + addresses.join(',') + (labelOrSid ? ';label=' + labelOrSid : ''),
    'Session-Expires: 120'
  ];
    ...
    peerConnectionManager.setConfiguration(options).then(function setConfigurationSucceeded() {
      ...
      return new Promise(function createRoomSignaling(resolve, reject) {
        ...
        session.once('accepted', function accepted(response) {
          ...
          var participantSignaling = new ParticipantV2(participantSid, identity, 'connected', localMedia);
