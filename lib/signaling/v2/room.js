...
RoomV2.prototype.disconnect = function disconnect(isUnexpected) {
  var didDisconnect = RoomSignaling.prototype.disconnect.call(this);
  if (didDisconnect) {
    this._transport.disconnect();
    this._didDisconnectUnexpectedly = isUnexpected || this._didDisconnectUnexpectedly;
  }
...
function handleTransportEvents(roomV2, transport) {
  ...
  transport.on('stateChanged', function stateChanged(state) {
    if (state === 'disconnected') {
      roomV2.disconnect(roomV2.state === 'connected');
