...
function RoomV2(localParticipant, initialState, transport, peerConnectionManager, options) {
  ...
  Object.defineProperties(this, {
    ...
    _mediaAndDataStreamTrackDeferreds: {
      value: new Map()
    }
  });
...
RoomV2.prototype._deleteMediaOrDataStreamTrackDeferred = function _deleteMediaOrDataStreamTrackDeferred(id) {
  return this._mediaAndDataStreamTrackDeferreds.delete(id);
};

RoomV2.prototype._getOrCreateMediaOrDataStreamTrackDeferred = function _getOrCreateMediaOrDataStreamTrackDeferred(id) {
  var deferred = this._mediaAndDataStreamTrackDeferreds.get(id) || util.defer();
  var mediaAndDataStreamTracks = this._peerConnectionManager.getRemoteMediaAndDataStreamTracks();
  ...
  var mediaOrDataStreamTrack = mediaAndDataStreamTracks.find(function(track) {
    return track.id === id && track.readyState !== 'ended';
  });

  if (mediaOrDataStreamTrack) {
    deferred.resolve(mediaOrDataStreamTrack);
  } else {
    ...
    this._mediaAndDataStreamTrackDeferreds.set(id, deferred);
  }

  return deferred;
};

RoomV2.prototype._addMediaOrDataStreamTrack = function _addMediaOrDataStreamTrack(mediaOrDataStreamTrack) {
  var deferred = this._getOrCreateMediaOrDataStreamTrackDeferred(mediaOrDataStreamTrack.id);
  deferred.resolve(mediaOrDataStreamTrack);
  return this;
};
...
RoomV2.prototype._getMediaOrDataStreamTrack = function _getMediaOrDataStreamTrack(id) {
  var self = this;
  return this._getOrCreateMediaOrDataStreamTrackDeferred(id).promise.then(function(mediaOrDataStreamTrack) {
    self._deleteMediaOrDataStreamTrackDeferred(id);
    return mediaOrDataStreamTrack;
  });
};
...
RoomV2.prototype._getOrCreateRemoteParticipant = function _getOrCreateRemoteParticipant(participantState) {
  ...
  var self = this;
  if (!participant) {
    participant = new RemoteParticipantV2(participantState, this._getMediaOrDataStreamTrack.bind(this));
...
function handleLocalParticipantEvents(roomV2, localParticipant) {
  ...
  function renegotiate() {
    var mediaAndDataStreamTracks = util.flatMap(localParticipant.tracks, function(track) {
      return track.mediaOrDataStreamTrack;
    });
    peerConnectionManager.setMediaAndDataStreamTracks(mediaAndDataStreamTracks);
  }
...
function handlePeerConnectionEvents(roomV2, peerConnectionManager) {
  ...
  peerConnectionManager.on('trackAdded', roomV2._addMediaOrDataStreamTrack.bind(roomV2));
  peerConnectionManager.dequeue('trackAdded');

  peerConnectionManager.getRemoteMediaAndDataStreamTracks().forEach(function(mediaOrDataStreamTrack) {
    roomV2._addMediaOrDataStreamTrack(mediaOrDataStreamTrack);
  });
}
