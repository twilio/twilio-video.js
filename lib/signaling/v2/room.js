...
class RoomV2 extends RoomSignaling {
   ...
   * @private
   */
  _getOrCreateTrackReceiverDeferred(id, type) {
    ...
    const trackReceiver = trackReceivers.find(trackReceiver => trackReceiver[type] === id && trackReceiver.readyState !== 'ended');
  ...
  _addTrackReceiver(trackReceiver) {
    const deferredById = this._getOrCreateTrackReceiverDeferred(trackReceiver.id);
    if (deferredById) {
      deferredById.resolve(trackReceiver);
    }
    const deferredByMid = this._getOrCreateTrackReceiverDeferred(trackReceiver.mid);
    if (deferredByMid) {
      deferredByMid.resolve(trackReceiver);
    }
    return this;
  }
   ...
   * @private
   */
  _getTrackReceiver(id, type = 'id') {
    return this._getOrCreateTrackReceiverDeferred(id, type).promise.then(trackReceiver => {
   ...
   * @private
   */
  _getTrackSidsToTrackSignalings(kind) {
    const trackSidsToTrackSignalings = flatMap(this.participants, participant =>
      Array.from(participant[`${kind}Tracks`] || participant.tracks));
  ...
  _initTrackSubscriptionsSignaling() {
    this._trackSubscriptionsSignaling.on('updated', trackStates => {
      // NOTE(mmalavalli): For the POC, only VideoTrack states are signaled using the MSP.
      const trackSidsToTrackSignalings = this._getTrackSidsToTrackSignalings('video');
      const trackSidsToTrackStates = new Map(trackStates.map(state => [state.sid, state]));

      // Update states of subscribed Tracks.
      trackSidsToTrackStates.forEach(({ mid, sid, state }) => {
        const trackSignaling = trackSidsToTrackSignalings.get(sid);
        const isSwitchedOff = state === 'off';
        if (trackSignaling) {
          // NOTE(mmalavalli): For the POC, in order to simplify implementation, we consider a Track
          // that switches MIDs as unsubscribed from and subscribed to again. This way, we get the
          // management of the underlying attached elements for free. Also, just for the POC, a Track
          // which is no longer assigned to an MID is considered unsubscribed.
          if (!mid || (trackSignaling.isSubscribed && trackSignaling.trackTransceiver.mid !== mid)) {
          ...
          // NOTE(mmalavalli): Cache switch off state for Track not yet known by the client.
...
function handleSubscriptions(room) {
  ...
  trackSidsToTrackSignalings.forEach(trackSignaling => {
    // NOTE(mmalavalli): For the POC, the RSP subscribed payload is not used to update
    // the subscription states of VideoTracks if the Track Subscriptions MSP is set up.
    if (trackSignaling.kind === 'video' && room._trackSubscriptionsSignaling.isSetup) {
      return;
    }
