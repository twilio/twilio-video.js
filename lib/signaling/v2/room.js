...
class RoomV2 extends RoomSignaling {
  ...
  _update(roomState) {
    ...
    // MAKARAND todo: add checks for switch_off
  ...
  _teardownTrackSwitchOff() {
    ...
    this._trackSwitchOffMonitor = null;
    ...
    trackSwitchOffSignaling.on('updated', (tracksOff, tracksOn) => {
      this.participants.forEach((participant) => {
        participant.tracks.forEach((track) => {
          if (tracksOff.indexOf(track.sid) >= 0) {
            track.switched(true /* off */);
          }
          if (tracksOn.indexOf(track.sid) >= 0) {
            track.switched(false /* on */);
          }
        });
      });
    });
...
function handleSubscriptions(room) {
  ...
  trackSignalings.forEach(trackSignaling => {
    ...
    if (trackId) {
      room._getTrackReceiver(trackId).then((trackReceiver) => {
        trackSignaling.setTrackTransceiver(trackReceiver);
      });
    }
  });
}
