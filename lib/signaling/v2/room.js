...
class RoomV2 extends RoomSignaling {
  ...
  _setupDataTransportBackedNetworkQualityMonitor(id) {
    ...
    const networkQualityMonitorPromise = this._getTrackReceiver(id).then(receiver => {
      ...
      const networkQualitySignaling = new this._NetworkQualitySignaling(receiver.toDataTransport(), self._options.networkQuality);
...
function handleLocalParticipantEvents(roomV2, localParticipant) {
  ...
  const networkQualityConfigurationUpdated = util.oncePerTick(() => {
    if (roomV2._networkQualityMonitor) {
      roomV2._networkQualityMonitor.setNetworkConfiguration(localParticipant._networkQualityConfiguration);
    }
  });
  ...
  localParticipant.on('networkQualityConfigurationUpdated', networkQualityConfigurationUpdated);
  ...
  roomV2.on('stateChanged', function stateChanged(state) {
    if (state === 'disconnected') {
      ...
      localParticipant.removeListener('networkQualityConfigurationUpdated', networkQualityConfigurationUpdated);
