...
RoomV2.prototype._getOrCreateTrackReceiverDeferred = function _getOrCreateTrackReceiverDeferred(id) {
  ...
  var trackReceiver = trackReceivers.find(trackReceiver => trackReceiver.id === id && trackReceiver.readyState !== 'ended');
...
function handleLocalParticipantEvents(roomV2, localParticipant) {
  ...
  var updateLocalParticipantStateAndRenegotiate = util.oncePerTick(() => {
    roomV2._publishNewLocalParticipantState();
    renegotiate();
  });

  function renegotiate() {
    var trackSenders = util.flatMap(localParticipant.tracks, trackV2 => trackV2.trackTransceiver);
...
function handleSubscriptionFailures(room) {
  var remoteTracks = new Map(util.flatMap(room.participants, participant => Array.from(participant.tracks.values()).map(track => [track.sid, track])));
