...
class RoomV2 extends RoomSignaling {
  ...
  _initPublisherHintSignaling() {
    this._publisherHintsSignaling.on('updated', hints => {
      console.log('makarand: _initPublisherHintSignaling got update');
      try {
        this.localParticipant.setPublisherHints(hints);
      } catch (ex) {
        console.error('makarand: _initPublisherHintSignaling failed to setPublisherHints: ', ex);
      }
    });
  }
