...
var NetworkQuality = require('../../stats/networkquality');
...
function RoomV2(localParticipant, initialState, transport, peerConnectionManager, options) {
  ...
  Object.defineProperties(this, {
    ...
    _networkQuality: {
      value: new options.NetworkQuality()
    },
  ...
  periodicallyMonitorNetworkQuality(this, peerConnectionManager, this._networkQuality);
 ...
 * @param {NetworkQuality} networkQuality
 * @returns {void}
 */
function periodicallyMonitorNetworkQuality(roomV2, peerConnectionManager, networkQuality) {
  var timeout = setTimeout(function computeNetworkQualityLevel() {
    // TODO(mroberts): Don't depend on private API; I'm only doing this to avoid
    // rewriting unit tests ATM.
    var peerConnectionV2s = peerConnectionManager._peerConnections
      ? Array.from(peerConnectionManager._peerConnections.values())
      : [];
    var peerConnections = peerConnectionV2s.map(function(pcv2) {
        return pcv2._peerConnection;
      });
    Promise.all(peerConnections.map(function(peerConnection) {
      if (peerConnection.signalingState === 'closed') {
        return null;
      }
      return peerConnection.getStats().catch(function() {
        return null;
      });
      ...
      if (report) {
        var result = networkQuality.compute(report);
        if (result !== null) {
          var networkQualityLevel = result.qualityLevel;
          if (networkQualityLevel !== null && roomV2._networkQualityLevel !== networkQualityLevel) {
            roomV2.emit('networkQualityChanged', networkQualityLevel);
            roomV2._networkQualityLevel = networkQualityLevel;
          }
        }
      }
      setTimeout(computeNetworkQualityLevel, 1000);
    });
  }, 1000);
