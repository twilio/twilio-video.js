...
class RoomV2 extends RoomSignaling {
  ...
  _setupTrackSwitchOff(trackSwitchOffSignaling) {
    trackSwitchOffSignaling.on('updated', (tracksOff, tracksOn) => {
      console.log("makarand: trackSwitchOff:", tracksOff, tracksOn)
      ...
      this.participants.forEach((participant) => {
        participant.tracks.forEach((track) => {
          if (tracksOff.indexOf(track.id) >= 0) {
            track.switchedOff(true);
          }
          if (tracksOn.indexOf(track.id) >= 0) {
            track.switchedOff(false);
          }
        });
      });
    });
