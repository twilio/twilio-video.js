...
class RoomV2 extends RoomSignaling {
  constructor(localParticipant, initialState, transport, peerConnectionManager, options) {
    ...
    periodicallyCollectAndPublishStats(this, transport, options.statsPublishIntervalMs);
...
function periodicallyCollectAndPublishStats(roomV2, transport, publishIntervalMs) {
  ...
  const publishFrequency = Math.floor(publishIntervalMs / STATS_COLLECTION_INTERVAL_MS);

  const interval = setInterval(async () => {
    try {
      const stats = await roomV2.getStats();
      collectionCount++;
