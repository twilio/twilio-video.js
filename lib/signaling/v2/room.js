...
class RoomV2 extends RoomSignaling {
  ...
  _setupTrackPriorityUpdates(trackPrioritySignaling) {
    ...
    trackPrioritySignaling.on('updated', (trackSid, publishOrSubscribe, priority) => {
      const trackSignaling = getTrackSignalings(this).get(trackSid);
      if (trackSignaling) {
...
function getTrackSignalings(room) {
  return new Map(util.flatMap(room.participants, participant => Array.from(participant.tracks.values()).map(track => [track.sid, track])));
}

function handleSubscriptions(room) {
  const trackSignalings = getTrackSignalings(room);
