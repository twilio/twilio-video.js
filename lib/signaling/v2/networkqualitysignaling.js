...
class NetworkQualitySignaling extends MediaSignaling {
  ...
  constructor(getReceiver, networkQualityConfiguration, options) {
    ...
    Object.defineProperties(this, {
      ...
      _resendTimer: {
        value: new Timeout(() => {
          // and schedule next timer for double the delay..
          this._resendTimer.setDelay(this._resendTimer.delay * 2);
  ...
  _handleNetworkQualityMessage(message) {
    ...
    // once a score is received. reset the timer to default timeout.
    this._resendTimer.setDelay(NETWORK_QUALITY_RESPONSE_TIME_MS);

    // timer is set while we are waiting a response.
    // if it isn't set, that means we are already in the process
    // of sending input, do not schedule another send.
