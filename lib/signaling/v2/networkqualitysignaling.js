...
class NetworkQualitySignaling extends EventEmitter {
   ...
   * @param {NetworkQualityConfiguration} config
   */
  constructor(mediaSignalingTransport, config) {
    super();
    ...
    Object.defineProperties(this, {
      ...
      _config: {
        value: fixIfConfigIsInvalid(config),
        writable: true
      }
    });
  ...
  setNetworkConfiguration(config) {
    this._config = fixIfConfigIsInvalid(config);
  }
  ...
  _sendNetworkQualityInputs() {
    return this._networkQualityInputs.take().then(networkQualityInputs => {
      this._mediaSignalingTransport.publish(createNetworkQualityInputsMessage(networkQualityInputs, this._config));
    });
  }
 ...
 */
function createNetworkQualityInputsMessage(networkQualityInputs, config) {
  return Object.assign({
    type: 'network_quality',
  }, config
    ? { reportLevel: config.local, remoteReportLevel: config.remote }
    : {}, networkQualityInputs);
  }

function fixIfConfigIsInvalid(config) {
  if (typeof config === 'object') {
    return {
      local: typeof config.local === 'number' && inRange(config.local, 1, 3)
        ? config.local
        : DEFAULT_NQ_LEVEL_LOCAL,
      remote: typeof config.remote === 'number' && inRange(config.remote, 0, 3)
        ? config.remote
        : DEFAULT_NQ_LEVEL_REMOTE
    };
  }
  return { local: DEFAULT_NQ_LEVEL_LOCAL, remote: DEFAULT_NQ_LEVEL_REMOTE };
}
