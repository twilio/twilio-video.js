...
PeerConnectionManager.prototype._getOrCreate = function _getOrCreate(id, configuration) {
  ...
  if (!peerConnection) {
    ...
    this._localDataStreamTracks.forEach(peerConnection.addDataStreamTrack, peerConnection);
  }
...
/**
 * Get the remote MediaStreamTracks and {@link RemoteDataStreamTrack}s of all
 * the {@link PeerConnectionV2}s.
 * @returns {Array<MediaStreamTrack|RemoteDataStreamTrack>} mediaAndDataStreamTracks
 */
PeerConnectionManager.prototype.getRemoteMediaAndDataStreamTracks = function getRemoteMediaAndDataStreamTracks() {
  return util.flatMap(this._peerConnections, function(peerConnection) {
    return peerConnection.getRemoteMediaAndDataStreamTracks();
  });
};
...
/**
 * Set the local MediaStreamTracks and {@link LocalDataStreamTrack}s on the
 * {@link PeerConnectionManager}'s underlying {@link PeerConnectionV2}s.
 * @param {Array<MediaStreamTrack|LocalDataStreamTrack>} mediaAndDataStreamTracks
 * @returns {this}
 */
PeerConnectionManager.prototype.setMediaAndDataStreamTracks = function setMediaAndDataStreamTracks(mediaAndDataStreamTracks) {
  var mediaStreamTracks = new Set();
  var dataStreamTracks = new Set();
  mediaAndDataStreamTracks.forEach(function(mediaOrDataStreamTrack) {
    if (mediaOrDataStreamTrack.kind === 'data') {
      dataStreamTracks.add(mediaOrDataStreamTrack);
    } else {
      mediaStreamTracks.add(mediaOrDataStreamTrack);
    }
  });
  ...
  var changes = getMediaAndDataStreamTrackChanges(this, dataStreamTracks, mediaStreamTracks);

  this._localDataStreamTracks = dataStreamTracks;

  applyMediaAndDataStreamTrackChanges(this, changes);
...
/**
 * Apply {@link MediaAndDataStreamTrackChanges}.
 * @param {PeerConnectionManager} peerConnectionManager
 * @param {MediaAndDataStreamTrackChanges} changes
 * @returns {void}
 */
function applyMediaAndDataStreamTrackChanges(peerConnectionManager, changes) {
  if (changes.data.add.size
    ...
    peerConnectionManager._peerConnections.forEach(function(peerConnection) {
      ...
      changes.data.remove.forEach(peerConnection.removeDataStreamTrack, peerConnection);
      changes.data.add.forEach(peerConnection.addDataStreamTrack, peerConnection);
...
/**
 * @interface DataStreamTrackChanges
 * @property {Set<LocalDataStreamTrack>} add
 * @property {Set<LocalDataStreamTrack>} remove
 */

/**
 * Get {@Link LocalDataStreamTrack} changes.
 * @param {PeerConnectionManager} peerConnectionManager
 * @param {Array<LocalDataStreamTrack>} dataStreamTracks
 * @returns {DataStreamTrackChanges} changes
 */
function getDataStreamTrackChanges(peerConnectionManager, dataStreamTracks) {
  var dataStreamTracksToRemove = util.difference(peerConnectionManager._localDataStreamTracks, dataStreamTracks);
  var dataStreamTracksToAdd = util.difference(dataStreamTracks, peerConnectionManager._localDataStreamTracks);
  return {
    add: dataStreamTracksToAdd,
    remove: dataStreamTracksToRemove
  };
}

/**
 * @interface MediaAndDataStreamTrackChanges
 * @property {DataStreamTrackChanges} data
 * @property {MediaStreamTrackChanges} media
 */

/**
 * Get {@link LocalDataStreamTrack} and MediaStreamTrack changes.
 * @param {PeerConnectionManager} peerConnectionManager
 * @param {Array<LocalDataStreamTrack>} dataStreamTracks
 * @param {Array<MediaStreamTrack>} mediaStreamTracks
 * @returns {MediaAndDataStreamTrackChanges} changes
 */
function getMediaAndDataStreamTrackChanges(peerConnectionManager, dataStreamTracks, mediaStreamTracks) {
  return {
    data: getDataStreamTrackChanges(peerConnectionManager, dataStreamTracks),
