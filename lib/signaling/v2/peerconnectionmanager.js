...
PeerConnectionManager.prototype.setMediaAndDataStreamTracks = function setMediaAndDataStreamTracks(mediaAndDataStreamTracks) {
  ...
  var mediaStreamTracksToRemove = util.difference(this._localMediaStream.getTracks(), mediaStreamTracks);
  var mediaStreamTracksToAdd = util.difference(mediaStreamTracks, this._localMediaStream.getTracks());

  var dataStreamTracksToRemove = util.difference(this._localDataStreamTracks, dataStreamTracks);
  var dataStreamTracksToAdd = util.difference(dataStreamTracks, this._localDataStreamTracks);
  this._localDataStreamTracks = dataStreamTracks;

  if (mediaStreamTracksToRemove.size
    || mediaStreamTracksToAdd.size
    || dataStreamTracksToRemove.size
    || dataStreamTracksToAdd.size) {
    mediaStreamTracksToRemove.forEach(this._localMediaStream.removeTrack, this._localMediaStream);
    mediaStreamTracksToAdd.forEach(this._localMediaStream.addTrack, this._localMediaStream);

    this._peerConnections.forEach(function(peerConnection) {
      peerConnection.removeMediaStream(this._localMediaStream);
      peerConnection.addMediaStream(this._localMediaStream);
      dataStreamTracksToRemove.forEach(peerConnection.removeDataStreamTrack, peerConnection);
      dataStreamTracksToAdd.forEach(peerConnection.addDataStreamTrack, peerConnection);
