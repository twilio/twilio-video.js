...
inherits(PeerConnectionManager, EventEmitter);
...
PeerConnectionManager.prototype._getOrCreate = function _getOrCreate(id, configuration) {
  ...
  if (!peerConnection) {
    ...
    peerConnection.on('candidates', this.emit.bind(this, 'candidates'));
    peerConnection.on('description', this.emit.bind(this, 'description'));
    peerConnection.on('trackAdded', this.emit.bind(this, 'trackAdded'));
    self._localMediaStreams.forEach(peerConnection.addMediaStream, peerConnection);
  }
