 ...
 * RTCPeerConnections, can be updated with Room Signaling messages, and emits
 * Room Signaling messages.
 * @emits PeerConnectionManager#roomState
 */
...
/**
 * Add ICE candidates to an RTCPeerConnection with the given ID.
 * @param {string} id
 * @param {Array<RTCIceCandidateInit>} candidates
 * @returns {Promise}
 */
PeerConnectionManager.prototype._addIceCandidates = function _addIceCandidates(id, candidates) {
  var self = this;
  return new Promise(function(resolve) {
    resolve(self._get(id));
...
PeerConnectionManager.prototype._close = function _close(id) {
  ...
  this._onClosePeerConnection.get(id)();
  this._onClosePeerConnection.delete(id);
};
...
PeerConnectionManager.prototype._create = function _create(id) {
  ...
  var removeIceCandidateEventListener = this._onicecandidate(id, peerConnection);
  this._onClosePeerConnection.set(id, removeIceCandidateEventListener);
};
...
/**
 * Re-emit any ICE candidate events on an RTCPeerConnection as Room Signaling
 * messages. Returns a function that, when called, disables this behavior.
 * @private
 * @param {string} id
 * @param {RTCPeerConnection} peerConnection
 * @returns {function}
 */
PeerConnectionManager.prototype._onicecandidate = function _onicecandidate(id, peerConnection) {
  var self = this;
  ...
  function onicecandidate(event) {
    ...
    self.queue('roomState', roomState);
  }

  var removeListener = peerConnection.removeEventListener.bind(
    peerConnection,
    'icecandidate',
    onicecandidate);
...
PeerConnectionManager.prototype._setRemoteDescription = function _setRemoteDescription(id, description) {
  ...
  return new Promise(function setRemoteDescription(resolve, reject) {
    ...
    var candidates = self._pendingCandidates.get(id);
    if (candidates) {
      self._pendingCandidates.delete(id);
      self._addIceCandidates(id, candidates);
    }
...
/**
 * Get a Room Signaling message describing the most recent change to the state
 * of the {@link PeerConnectionManager}.
 * @returns {?object}
 */
PeerConnectionManager.prototype.getState = function getState() {
...
/**
 * Notify the {@link PeerConnectionManager} of a new Room Signaling message.
 * This may or may not trigger changes to the underlying RTCPeerConnections.
 * @param {object} roomState
 * @returns {Promise}
 */
PeerConnectionManager.prototype.update = function update(roomState) {
  ...
  var peerConnections = roomState['peer_connections'];
  ...
  return Promise.all(peerConnections.map(function(peerConnection) {
    ...
    var candidates = peerConnection['candidates'];
    if (candidates instanceof Array) {
      if (!this._has(id)) {
        this._pendingCandidates.get(this.pendingCandidates.get(id) || []).push(candidates);
      } else {
        this._addIceCandidates(id, candidates);
      }
    }
...
/**
 * @event {PeerConnectionManager#roomState}
 * @param {object} roomState
 */
