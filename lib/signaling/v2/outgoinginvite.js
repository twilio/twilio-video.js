...
function handleSessionEvents(outgoingInviteV2, session) {
  session.once('accepted', function accepted(response) {
    ...
    var roomSid = util.parseRoomSIDFromContactHeader(
      response.getHeader('Contact'));

    var roomV2 = new RoomV2(outgoingInviteV2.localMedia, participantSid, roomSid, session, outgoingInviteV2._options);

    outgoingInviteV2._deferred.resolve(roomV2);
  });
