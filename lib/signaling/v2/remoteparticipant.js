...
class RemoteParticipantV2 extends RemoteParticipantSignaling {
   ...
   * @param {function(Track.SID): Promise<MediaTrackReceiver | DataTrackReceiver | null>} getTrackReceiver
   ...
   */
  constructor(participantState, getInitialTrackSwitchOffState, getTrackReceiver, setPriority, setRenderHint, clearTrackHint, options) {
  ...
  _getOrCreateTrack(trackState) {
    ...
    if (!track) {
      ...
      track = new RemoteTrackPublicationV2(trackState, isSwitchedOff, this._getTrackReceiver);
      this.addTrack(track);
    }
    return track;
  }
