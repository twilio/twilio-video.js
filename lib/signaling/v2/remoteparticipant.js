...
class RemoteParticipantV2 extends RemoteParticipantSignaling {
  ...
  addTrack(track) {
    ...
    // NOTE(syerrapragada): RTCDataChannels are not supported in Edge,
    // so there is no TrackReceiver for the corresponding RemoteDataTrackPublication.
    if (guessBrowser() === 'edge' && track.kind === 'data') {
      this._log.warnOnce('DataTracks are not supported in Edge. So you won\'t be able to '
