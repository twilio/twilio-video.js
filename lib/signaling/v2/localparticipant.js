...
class LocalParticipantV2 extends LocalParticipantSignaling {
  ...
  setPublisherHints(hints) {
    hints.forEach(({ track, encodings }) => {
      const trackSignaling = Array.from(this.tracks.values()).find(trackPub => trackPub.sid === track);
      if (trackSignaling) {
        const rtpSender = Array.from(trackSignaling.trackTransceiver._senders)[0];
        const parameters = rtpSender.getParameters();
        encodings.forEach(({ enabled, layer_index: layerIndex }) => {
          this._log.debug(`${track} layer:${layerIndex}, active:${parameters.encodings[layerIndex].active} => ${enabled}`);
          parameters.encodings[layerIndex].active = enabled;
        });
        rtpSender.setParameters(parameters);
      } else {
        this._log.warn('track signaling not found: ', track);
      }
    });
  }
}
