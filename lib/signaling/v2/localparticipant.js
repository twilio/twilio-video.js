...
const { buildLogLevels, isChromeScreenShareTrack, isDeepEqual } = require('../../util');
...
class LocalParticipantV2 extends LocalParticipantSignaling {
   ...
   * @param {string} hint
   */
  setScreenShareContentHint(hint) {
    if (!this._screenShareContentHint) {
      this._screenShareContentHint = hint;
    }
  }
  ...
  _createLocalTrackPublicationSignaling(trackSender, name, priority) {
    if (isChromeScreenShareTrack(trackSender.track)) {
      trackSender.track.contentHint = this._screenShareContentHint;
    }
