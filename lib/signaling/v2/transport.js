...
/**
 * Create a {@link TwilioError} from the header or the body of an incoming message.
 * @param {object} headers - SIP incoming error message headers
 * @param {string} body - SIP incoming error message body
 * @param {boolean} byeOrFailed - true if the SIP incoming error message is the payload of the 'bye' or 'failed' events
 * @returns {TwilioError}
 */
function createTwilioErrorFromHeadersOrBody(headers, body, byeOrFailed) {
  var code;
  var message;
  ...
  else if (headers && headers['X-Twilio-Error']) {
    var twilioErrorHeader = headers['X-Twilio-Error'][0].raw.split(' ');
    code = parseInt(twilioErrorHeader[0], 10);
    message = twilioErrorHeader.slice(1).join(' ');
  }

  // NOTE(mmalavalli): If the SIP incoming error message does not have any error
  // code or message in either the headers or the body, then a TwilioError
  // is not created if the message is the payload for the  underlying session's
  // 'bye' or 'failed' events. On the other hand, if the message is the payload
  // for the underlying session's 'accepted' or 'info' events, then a
  // TwilioError(0, 'Unknown error') is created.
  if (!byeOrFailed || (typeof code === 'number')) {
    ...
    if (requestOrResponse && !(requestOrResponse instanceof SIP.OutgoingRequest)) {
      ...
      if (requestOrResponse.body) {
        try {
          ...
          twilioError = new SignalingIncomingMessageInvalidError();
        }
      }
      twilioError = twilioError || createTwilioErrorFromHeadersOrBody(requestOrResponse.headers, message, true);
    } else {
      switch (cause) {
      ...
      case 'connected':
        switch (message.type) {
          ...
          case 'update':
            ...
            return;
          case 'error':
            error = createTwilioErrorFromHeadersOrBody(requestOrResponse.headers, message);
          case 'disconnected':
