 ...
 * @returns {?object|SignalingIncomingMessageInvalidError}
 */
function parseRequestOrResponseBody(requestOrResponse) {
  if (requestOrResponse.body) {
    try {
      ...
      return new SignalingIncomingMessageInvalidError();
    }
  }
  return null;
}
...
function getTwilioErrorFromRequestOrResponseBody(requestOrResponse) {
  var error = parseRequestOrResponseBody(requestOrResponse);
  return error && error.code && error.message
    ? createTwilioError(error.code, error.message)
    : null;
}
...
function getTwilioErrorFromRequestOrResponse(requestOrResponse) {
  return getTwilioErrorFromRequestOrResponseBody(requestOrResponse)
    || getTwilioErrorFromRequestOrResponseHeaders(requestOrResponse);
}
...
function setupEventListeners(transport, session, ua) {
  ...
  function handleRequestOrResponse(requestOrResponse) {
    ...
    if (message instanceof SignalingIncomingMessageInvalidError) {
      error = message;
    } else if (!message) {
      error = getTwilioErrorFromRequestOrResponseHeaders(requestOrResponse) || createTwilioError();
    }

    if (error) {
      return transport.disconnect(error);
    }
        ...
        switch (message.type) {
          ...
          case 'disconnected':
            if (message.type === 'error') {
              error = getTwilioErrorFromRequestOrResponse(requestOrResponse) || createTwilioError();
            }
