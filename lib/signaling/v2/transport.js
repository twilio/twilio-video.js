...
function Transport(name, accountSid, accessToken, localParticipant, peerConnectionManager, ua, options) {
  ...
  var session = createSession(this, name, accountSid, accessToken, localParticipant, peerConnectionManager, ua);
  Object.defineProperties(this, {
    _session: {
      value: session
    },
    _SIPJSMediaHandler: {
      value: options.SIPJSMediaHandler
    },
    _updatesReceived: {
      value: []
    },
...
function createSession(transport, name, accountSid, accessToken, localParticipant, peerConnectionManager, ua) {
  ...
  return ua.invite(target, {
    ...
    mediaHandlerFactory: function mediaHandlerFactory() {
      var SIPJSMediaHandler = transport._SIPJSMediaHandler;
...
function setupEventListeners(transport, session) {
  ...
  session.once('failed', handleFailure);

  transport.on('stateChanged', function stateChanged(state) {
    switch (state) {
      case 'connected':
        session.removeListener('accepted', handleRequestOrResponse);
        session.removeListener('failed', handleFailure);
        ...
        transport._updatesReceived.splice(0).forEach(transport.emit.bind(transport, 'update'));
