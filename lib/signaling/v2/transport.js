...
function setupEventListeners(transport, session, ua) {
  function disconnect(requestOrResponse, cause) {
    ...
    if (requestOrResponse && !(requestOrResponse instanceof SIP.OutgoingRequest)) {
      ...
      if (!twilioError) {
        var twilioErrorInfo = getTwilioErrorInfo(requestOrResponse.headers, message);
        if (twilioErrorInfo) {
          twilioError = createTwilioError(twilioErrorInfo.code, twilioErrorInfo.message);
        }
      }
    } else {
      switch (cause) {
  ...
  function getTwilioErrorInfo(headers, body) {
  ...
  function createTwilioErrorFromRequestOrResponse(headers, body) {
    var twilioErrorInfo = getTwilioErrorInfo(headers, body) || {};
    return createTwilioError(twilioErrorInfo.code, twilioErrorInfo.message);
  }
      ...
      case 'connected':
        switch (message.type) {
          ...
          case 'update':
            ...
            return;
          case 'error':
            error = createTwilioErrorFromRequestOrResponse(requestOrResponse.headers, message);
          case 'disconnected':
