...
function publishWithRetries(session, payload, attempts) {
  function jitterify(value) {
    var jitter = constants.PUBLISH_BACKOFF_JITTER;
    var range = jitter << 1;
    return Math.floor(value - jitter + Math.random() * range + 0.5);
  }
  ...
  return new Promise(function(resolve, reject) {
      ...
      switch (Math.floor(response.status_code / 100)) {
        ...
        case 5:
          if (attempts < constants.PUBLISH_MAX_ATTEMPTS) {
            ...
            reject(new Error('Transport failed to send INFO message even '
    ...
    setTimeout(sendRequest, jitterify(backOffMs));
  });
}
