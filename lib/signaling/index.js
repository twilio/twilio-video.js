...
// NOTE(mroberts): This is a dummy implementation suitable for testing.
Signaling.prototype._connect = function _connect(identities, nameOrSid, localParticipant, options) {
  localParticipant.setSid('PA00000000000000000000000000000000');
  localParticipant.setIdentity('foo');
 ...
 */
Signaling.prototype.connect = function connect(identities, nameOrSid, localParticipant, options) {
  var self = this;
  return this.bracket('connect', function transition(key) {
    switch (self.state) {
      ...
      case 'open':
        ...
        return self._connect(identities, nameOrSid, localParticipant, options);
      default:
        throw new Error('Unexpected Signaling state "' + self.state + '"');
    }
  });
