...
Signaling.prototype._accept = function _accept(incomingInvite, options, key) {
  var preemptionError = new Error('IncomingInvite StateMachine was preempted');
  if (!incomingInvite._stateMachine.hasLock(key)) {
  ...
  return LocalMedia.getLocalMedia(options).then(
    ...
    return self._createConversation(sid, participantSid, localMedia,
      ...
      if (incomingInvite._stateMachine.hasLock(key)) {
        incomingInvite._stateMachine.transition('failed', key);
      }
      throw error;
    });
...
Signaling.prototype._close = function _close(key) {
  if (!this._stateMachine.hasLock(key)) {
    throw new Error('Signaling StateMachine was preempted');
  }
...
Signaling.prototype._connect = function _connect(conversationSidOrLabel,
  options) {
  var sid = conversationSidOrLabel.match(/^CV[a-z1-9]{32}$/)
    ? conversationSidOrLabel : null;
  var self = this;
  return LocalMedia.getLocalMedia(options).then(
    function getLocalMediaSucceeded(localMedia) {
    ...
    return self._createConversation(sid, localMedia, options);
  });
};
  ...
  return this._stateMachine.bracket('connect', function transition(key) {
    switch (self.state) {
      ...
      case 'listening':
        ...
        while (self._stateMachine.hasLock(key)) {
          self._stateMachine.releaseLock(key);
        }
