...
var IncomingSession = sessions.IncomingSession;
var OutgoingSession = sessions.OutgoingSession;
var util = require('../../util');

/**
 * Constructs a new {@link SipUA}.
 * @class
 * @classdesc A {@link SipUA} is a SIP User Agent that makes and receives
 *   calls on behalf of an {@link Endpoint}.
 * @param {SipUAFactory} factory - the {@link SipUAFactory} that owns this
 *   {@link SipUA}
 * @param {Endpoint} endpoint - the {@link Endpoint} associated with this
 *   {@link SipUA}
 * @property {Array<Session>} sessions - the {@link Session}s active on this
 *   {@link SipUA}
 * @property {Endpoint} endpoint - the {@link Endpoint} that owns this
 *   {@link SipUA}
 */
function SipUA(factory, endpoint) {
  if (!(this instanceof SipUA)) {
    return new SipUA(endpoint, ua);
  }
  ...
  Object.defineProperties(this, {
    ...
    'sessions': {
      get: function() {
        return util.getValues(self._sessions);
      }
    },
    'endpoint': {
      value: endpoint
  ...
  ua.on('invite', function(session) {
    ...
    var session = new IncomingSession(self, session);
...
SipUA.prototype.close = function close() {
  this._factory.removeEndpoint(this.endpoint);
 ...
 */
SipUA.prototype.call = function session(capabilityToken, params) {
  params = params || {};
  ...
  // Wrap the SipUA.js Session in an OutgoingSession object.
  var _session = new OutgoingSession(this, session);
  this._factory.addSession(this, _session);
  var self = this;
  _session.once('error', function() {
    self._factory.removeSession(self, _session);
  });
...
/**
 * Register on behalf of the {@link Endpoint}.
 * @instance
 * @param {CapabilityToken} capabilityToken
 * @returns {SipUA}
 */
...
SipUA.prototype.unregister = function() {
  this._ua.unregister();
  return this;
};
