'use strict';

var EventEmitter = require('events').EventEmitter;
var inherits = require('util').inherits;

var sessions = require('../../session');
var IncomingSession = sessions.IncomingSession;
var OutgoingSession = sessions.OutgoingSession;
var util = require('../../util');

/**
 * Constructs a new {@link SipUA}.
 * @class
 * @classdesc A {@link SipUA} is a SIP User Agent that makes and receives sessions
 *   on behalf of a {@link Endpoint}.
 * @param {SipUAFactory} factory - the {@link SipUAFactory} that constructed
 *   this {@link SipUA}
 * @param {Endpoint} endpoint - the {@link Endpoint} associated with this {@link SipUA}
 * @property {Array<Session>} sessions - the {@link Session}s active on this {@link
 *   SipUA}
 * @property {Endpoint} endpoint - the {@link Endpoint} that owns this {@link SipUA}
 */
function SipUA(factory, endpoint) {
  if (!(this instanceof SipUA)) {
    return new SipUA(endpoint, ua);
  }

  EventEmitter.call(this);

  var ua = new factory._UA({
    'autostart': false,
    'register': false,
    'traceSip': factory._debug,
    'stunServers': 'invalid',
    'wsServers': [factory._wsServer, factory._wsServer],
    'log': {
      'builtinEnabled': factory._debug
    }
  });

  var self = this;
  Object.defineProperties(this, {
    // Private
    _sessions: {
      value: {}
    },
    _factory: {
      value: factory
    },
    _ua: {
      value: ua
    },
    // Public
    'sessions': {
      get: function() {
        return util.getValues(self._sessions);
      }
    },
    'endpoint': {
      value: endpoint
    }
  });

  ua.on('invite', function(session) {
    console.log('SipUA emitting "incoming"');
    var session = new IncomingSession(self, session);
    self.emit('incoming', session);
  });

  ua.on('registered', function() {
    self.emit('registered');
  });

  ua.on('registrationFailed', function() {
    self.emit('registrationFailed');
  });

  setTimeout(ua.start.bind(ua));

  return Object.freeze(this);
}

inherits(SipUA, EventEmitter);

/**
 * Close this {@link SipUA} and remove it from the {@link SipUAFactory}.
 * @instance
 * @returns {SipUA}
 */
SipUA.prototype.close = function close() {
  this._factory.removeEndpoint(this.endpoint);
  setTimeout(this._ua.stop.bind(this._ua));
  return this;
};

/**
 * Make an {@link OutgoingSession}.
 * @instance
 * @param {CapabilityToken} capabilityToken
 * @param {?object} params - outgoing application parameters, if any
 * @returns {OutgoingSession}
 */
SipUA.prototype.call = function session(capabilityToken, params) {
  params = params || {};
  var target = util.makeURI(capabilityToken.accountSid,
                            capabilityToken.outgoingAppSid);
  var deviceInfo = {p:'browser'};
  var inviteHeaders = util.makeInviteHeaders(deviceInfo, capabilityToken, params);
  var session = this._ua.invite(target, {
    'extraHeaders': inviteHeaders,
    'media': {
      'constraints': {
        'audio': true,
        'video': false
      },
      'render': {}
    }
  });

  // Wrap the SipUA.js Session in an OutgoingSession object.
  var _session = new OutgoingSession(this, session);
  this._factory.addSession(this, _session);
  var self = this;
  _session.once('error', function() {
    self._factory.removeSession(self, _session);
  });

  return _session;
};

/**
 * Register on behalf of the {@link Endpoint}.
 * @instance
 * @param {CapabilityToken} capabilityToken
 * @returns {SipUA}
 */
SipUA.prototype.register = function(capabilityToken) {
  var deviceInfo = {p:'browser'};
  var registerHeaders = util.makeRegisterHeaders(deviceInfo, capabilityToken);
  var self = this;
  function register() {
    var registerUri = util.makeURI(capabilityToken.accountSid,
                                   capabilityToken.incomingClientName)
                    + ';rtp_profile=RTP_SAVPF'; 
    self._ua.register({
      'extraHeaders': registerHeaders
    });
  }
  if (this._ua.isConnected()) {
    register();
  } else {
    this._ua.once('connected', register);
  }
  return this;
};

module.exports = SipUA;
