...
class RoomSignaling extends StateMachine {
  ...
  constructor(localParticipant, sid, name, options) {
    ...
    const sessionTimeout = new options.Timeout(() => {
      this.preempt('disconnected', null, [this._reconnectingError]);
    ...
    this.on('connectionStateChanged', () => {
      if (this.connectionState === 'failed' && this.mediaConnectionState !== 'failed') {
        // eslint-disable-next-line
        // TODO(mmalavalli): Raise a DTLSTransportFailedError once it is available.
        this.preempt('disconnected', null, [new MediaConnectionError()]);
      }
    });
