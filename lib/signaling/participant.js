...
class ParticipantSignaling extends StateMachine {
   ...
   * @returns {void}
   */
  updateSubscriberTrackPriority(trackSid, priority, renderHint) {
    // note the most recent priority update for the track.
    this._enqueuedPriorityUpdates.set(trackSid, { priority, renderHint });
    if (this._trackPrioritySignaling) {
      this._trackPrioritySignaling.sendTrackPriorityUpdate(trackSid, 'subscribe', priority, renderHint);
    }
  }
  ...
  setTrackPrioritySignaling(trackPrioritySignaling) {
    ...
    if (trackPrioritySignaling) {
      this._enqueuedPriorityUpdates.forEach(({ priority, renderHint }, trackSid) => {
        this._trackPrioritySignaling.sendTrackPriorityUpdate(trackSid, 'subscribe', priority, renderHint);
      });
