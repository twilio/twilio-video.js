...
class ParticipantSignaling extends StateMachine {
   ...
   * @returns {void}
   */
  updateTrackPriority(trackSid, priority, publishOrSubscribe) {
    if (this._trackPrioritySignaling) {
      this._trackPrioritySignaling.sendTrackPriorityUpdate(trackSid, publishOrSubscribe, priority);
    } else {
      this._enqueuedPriorityUpdates.set(trackSid, { priority, publishOrSubscribe });
    }
  }
  ...
  setTrackPrioritySignaling(trackPrioritySignaling) {
    ...
    if (trackPrioritySignaling) {
      this._enqueuedPriorityUpdates.forEach(({ priority, publishOrSubscribe }, trackSid) => {
        this._trackPrioritySignaling.sendTrackPriorityUpdate(trackSid, publishOrSubscribe, priority);
      });
