 ...
 */
function OutgoingInvite(userAgent, participants, signaling, options) {
  if (!(this instanceof OutgoingInvite)) {
    return new OutgoingInvite(userAgent, participants, signaling, options);
  }
  ...
  function setLocalMedia(_localMedia) {
    ...
    options.shouldStopLocalMediaOnDisconnect = shouldStopLocalMediaOnFailure;
  ...
  function setupConversation(dialog) {
    ...
    var conversation = new ConversationImpl(localMedia, participantSid, sid,
      self._signaling, options);
