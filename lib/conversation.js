...
Conversation.prototype._connectParticipant = function _connectParticipant(participant) {
  ...
  participant.on('trackEnded', function trackEnded(track) {
    if (!self.participants.has(participant.sid)) {
      return participant.removeListener('trackEnded', trackEnded);
    }
    self.emit('trackEnded', participant, track);
  });
  ...
  participant.on('trackStarted', function trackStarted(track) {
    if (!self.participants.has(participant.sid)) {
      return participant.removeListener('trackStarted', trackStarted);
    }
    self.emit('trackStarted', participant, track);
  });
