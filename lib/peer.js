 ...
 * @property {?CapabilityToken} capabilityToken - This {@link Peer}'s {@link CapabilityToken}, set after a call to {@link Peer#auth}
...
function Peer(domain, name, callback) {
  ...
  if (!(this instanceof Peer)) {
    return new Peer(domain, name, callback);
  }
 ...
 * @instance
 * @param {CapabilityToken|string} capabilityToken - A {@link CapabilityToken} or a capability token string to parse
 ...
 * @returns Peer
 */
Peer.prototype.auth = function auth(capabilityToken, callback) {
  ...
  capabilityToken = typeof capabilityToken === 'string'
                  ? new CapabilityToken(capabilityToken)
                  : capabilityToken;
  Object.defineProperty(this, 'capabilityToken', {
 ...
 * @returns Peer
 */
Peer.prototype.connect = function connect(callback) {
  var transport = Transport.getInstance();

  var session = Transport.getInstance().connect(this, callback);
  if (!callback) {
